// Generated by CoffeeScript 1.10.0
(function() {
  angular.module('ajenti.auth.users', ['core']);

  angular.module('ajenti.auth.users').run(function(customization) {
    var base;
    customization.plugins.auth_users = {};
    return (base = customization.plugins.auth_users).forceUID != null ? base.forceUID : base.forceUID = null;
  });

}).call(this);

// Generated by CoffeeScript 1.10.0
(function() {
  angular.module('core').config(function($routeProvider) {
    return $routeProvider.when('/view/auth-users', {
      templateUrl: '/auth_users:resources/partial/index.html',
      controller: 'AuthUsersIndexController'
    });
  });

}).call(this);

// Generated by CoffeeScript 1.10.0
(function() {
  angular.module('ajenti.auth.users').controller('AuthUsersIndexController', function($scope, $http, notify, pageTitle, config, passwd, customization, gettext) {
    pageTitle.set('Users');
    $scope.config = config;
    $scope.defaultRootPassword = '';
    config.load().then(function() {
      var base;
      if ((base = config.data.auth).users == null) {
        base.users = {
          root: {
            password: $scope.defaultRootPassword,
            uid: 0
          }
        };
      }
      return config.getPermissions().then(function(data) {
        var results, username;
        $scope.permissions = data;
        results = [];
        for (username in config.data.auth.users) {
          $scope.resetPermissions(username);
          results.push(angular.extend($scope.userPermissions[username], config.data.auth.users[username].permissions || {}));
        }
        return results;
      });
    })["catch"](function() {
      return notify.error('Could not load config');
    });
    $scope.userPermissions = {};
    $scope.resetPermissions = function(username) {
      var i, len, permission, ref, results;
      $scope.userPermissions[username] = {};
      ref = $scope.permissions;
      results = [];
      for (i = 0, len = ref.length; i < len; i++) {
        permission = ref[i];
        results.push($scope.userPermissions[username][permission.id] = permission["default"]);
      }
      return results;
    };
    $scope.removeUser = function(username) {
      return delete config.data.auth.users[username];
    };
    passwd.list().then(function(l) {
      $scope.systemUsers = l;
      return $scope.getSystemUser = function(uid) {
        var i, len, ref, u;
        ref = $scope.systemUsers;
        for (i = 0, len = ref.length; i < len; i++) {
          u = ref[i];
          if (u.uid === uid) {
            return u;
          }
        }
      };
    });
    $scope.save = function() {
      var i, len, permission, ref, username, v;
      for (username in $scope.userPermissions) {
        if (!config.data.auth.users[username]) {
          continue;
        }
        config.data.auth.users[username].permissions = {};
        ref = $scope.permissions;
        for (i = 0, len = ref.length; i < len; i++) {
          permission = ref[i];
          v = $scope.userPermissions[username][permission.id];
          if (v !== permission["default"]) {
            config.data.auth.users[username].permissions[permission.id] = v;
          }
        }
      }
      return config.save().then(function() {
        return notify.success('Saved');
      });
    };
    $scope.setPassword = function(username, password) {
      return config.save().then(function() {
        return $http.post("/api/auth-users/set-password/" + username, password).then(function() {
          notify.success(gettext('Password updated'));
          return config.load();
        });
      });
    };
    $scope.addUser = function(username) {
      config.data.auth.users[username] = {
        uid: customization.plugins.auth_users.forceUID || 0
      };
      $scope.resetPermissions(username);
      return $scope.newUsername = '';
    };
    return $scope.isDangerousSetup = function() {
      var safe, username;
      if (!config.data) {
        return;
      }
      safe = false;
      for (username in config.data.auth.users) {
        if (config.data.auth.users[username].uid === 0) {
          safe = true;
        }
      }
      return !safe;
    };
  });

}).call(this);

