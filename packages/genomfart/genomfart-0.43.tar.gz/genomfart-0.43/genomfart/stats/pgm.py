...
class discrete_factor_graph(nx.Graph):
    ...
    def run_LBP_rbp(self, maxit=1000):
            ...
            for n in self.neighbors(to_node):
                ...
                try:
                    ...
                    for v in message_queue:
                        if all((v[1] == (to_node,n),v[2] == neighbor_message_name)):
                            message_queue.remove(v)
                            break
                # Update message
        ...
        print "Terminated at %d iterations" % it
    def run_synchronous_update(self):
        # Create dictionary of ((from_node,to_node),message_type) -> message
        message_dict = {}
        for from_node, to_node in self.edges_iter():
            if self.node[from_node]['bipartite'] == 'v':
                message_dict[((from_node,to_node),'to_f_message')] = \
                  self.send_message_to_factor(from_node,to_node,update=False)
            else:
                message_dict[((from_node,to_node),'to_v_message')] = \
                  self.send_message_to_var(from_node, to_node,update=False)
        for to_node, from_node in self.edges_iter():
            if self.node[from_node]['bipartite'] == 'v':
                message_dict[((from_node,to_node),'to_f_message')] = \
                  self.send_message_to_factor(from_node,to_node,update=False)
