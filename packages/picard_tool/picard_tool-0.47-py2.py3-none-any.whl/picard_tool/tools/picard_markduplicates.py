...
def bam_markduplicates(uuid, bam_path, engine, logger, be_lenient):
    ...
    if pipe_util.already_step(step_dir, 'picard_markduplicates', logger):
        ...
        cmd = ['java', '-d64', '-jar', os.path.join(home_dir, 'tools/picard-tools/picard.jar'), 'MarkDuplicates', 'INPUT=' + bam_path, 'OUTPUT=' + outbam, 'METRICS_FILE=' + metrics_file, 'TMP_DIR=' + md_dir ,'CREATE_INDEX=true'] # 'MINIMUM_DISTANCE=' + minimum_distance,
        if be_lenient:
