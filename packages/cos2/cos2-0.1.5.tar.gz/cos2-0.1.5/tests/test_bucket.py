...
class TestBucket(CosTestCase):
    ...
    def test_website(self):
        ...
        bucket_anonymousAuth = cos2.Bucket(cos2.AnonymousAuth(), 'http://cos-beta-website.chinac.com',COS_BUCKET)
        result = bucket_anonymousAuth.get_object('')
        self.assertEqual(result.read(), index_content)

        # 验证获取不存在的对象，会重定向到error页面
        result = bucket_anonymousAuth.get_object(key)
