...
class Service(object):
    ...
    def _getType(name):
        ...
        if os.path.isdir("/etc/init.d"):
            print "INIT" # TODO DELETE
            logging.debug("Service control is via init.d")
            return "init.d"
        else:
            print "Not init" # TODO DELETE
            result = -1

        try:
            print "Not try service" # TODO DELETE
            p = (Popen(["service", "--status-all"],
                       stdout=subprocess.PIPE))
            print "open" # TODO DELETE
            ret = p.communicate()
            result = p.returncode
            print "result" # TODO DELETE

            if result == 0 or result == 1:
                print "SERVICE" # TODO DELETE
                logging.debug("Service control is via service")
                return "service"
        except:
            print "except" # TODO DELETE
            result = -1
        print result # TODO DELETE
        return result
