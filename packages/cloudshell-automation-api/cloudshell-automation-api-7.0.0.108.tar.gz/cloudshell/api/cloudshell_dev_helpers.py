...
CONNECTIVITY_DETAILS_TEMPLATE = '"tsAPIPort": "{cloudshell_api_port}",'\
	...
	'"{server_address}", "adminAuthToken" : "{token}"'
...
def _bootstrap_data(user, password, domain, reservation_id,
    ...
    if is_dev_mode():
        ...
        token = CloudShellAPISession(server_address, user,password,domain,port=cloudshell_api_port).Logon(user, password, domain).Token.Token
