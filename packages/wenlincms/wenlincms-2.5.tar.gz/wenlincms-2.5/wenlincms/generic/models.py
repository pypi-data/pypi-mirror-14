...
from wenlincms.generic.managers import CommentManager, KeywordManager
from wenlincms.core.models import SiteRelated, Orderable
...
class ThreadedComment(Comment):
    ...
    so we can't inherit from ``SiteRelated`` in ``wenlincms.core``, and
    therefore need to set the site on ``save``. ``CommentManager``
    inherits from wenlincms's ``CurrentSiteManager``, so everything else
    ...
    def avatar_link(self):
        from wenlincms.core.templatetags.wenlincms_tags import gravatar_url
...
class Keyword(SiteRelated):
    ...
    parent = models.ForeignKey('self', verbose_name='上级', null=True, blank=True,
                               ...
                               help_text="<br>列表菜单中可重新计算所有标签的频率")
