...
from wenlincms.utils.urls import home_slug
...
class PublishedManager(Manager):
    ...
    def published(self):
        return self.filter(Q(status=2) & Q(audit_status=1))
    ...
    def url_map(self, **kwargs):
        ...
        for model in get_models():
            if issubclass(model, self.model):
                for item in (model.objects.published().filter(**kwargs).order_by('-id')[:limit_for_each_model]):
