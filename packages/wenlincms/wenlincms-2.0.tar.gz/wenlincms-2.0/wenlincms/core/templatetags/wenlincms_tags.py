...
if "compressor" in settings.INSTALLED_APPS:
    ...
    def compress(parser, token):
        ...
        loaded from ``wenlincms_tags``, allowing us to provide
...
@register.inclusion_tag(get_theme() + "/includes/form_fields.html", takes_context=True)
...
@register.inclusion_tag(get_theme() + "/includes/form_errors.html", takes_context=True)
...
@register.inclusion_tag(get_theme() + "/includes/pagination.html", takes_context=True)
...
@register.inclusion_tag(get_theme() + "/includes/search_form.html", takes_context=True)
...
def thumbnail(image_url, width, height, quality=95, left=.5, top=.5,
    ...
    # image, which is something we do in wlfilebrowser when a new image
...
def admin_dropdown_menu(context):
    ...
    context["dropdown_menu_selected_site_id"] = "1"
    return context
