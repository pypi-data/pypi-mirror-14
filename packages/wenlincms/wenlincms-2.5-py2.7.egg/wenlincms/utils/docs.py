...
def build_deploy_docs(docs_path):
    try:
        ...
        return
    project_template_path = path_for_import("wenlincms.project_template")
...
def build_changelog(docs_path, package_name="wenlincms"):
    """
...
def build_modelgraph(docs_path, package_name="wenlincms"):
    """
    ...
    apps = [a.rsplit(".")[1] for a in settings.INSTALLED_APPS
            if a.startswith("wenlincms.") or a.startswith(package_name + ".")]
    try:
...
def build_requirements(docs_path, package_name="wenlincms"):
    """
    Updates the requirements file with wenlincms's version number.
    """
    mezz_string = "wenlincms=="
    project_path = os.path.join(docs_path, "..")
