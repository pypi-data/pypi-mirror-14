...
def set_dynamic_settings(s):
    ...
    add_to_builtins("wenlincms.template.loader_tags")
    ...
    # From wenlincms 3.1.2 and onward we added the context processor
    ...
    cp = "wenlincms.pages.context_processors.page"
    if ("wenlincms.pages" in s["INSTALLED_APPS"] and
    ...
    if s["TESTING"]:
        ...
        # catch-all urlpattern such as wenlincms's pages app.
    ...
    # wenlincms's settings available to its offline context,
    ...
    if "compressor" in s["INSTALLED_APPS"]:
        ...
        def wenlincms_settings():
            ...
            return settings
        s["COMPRESS_OFFLINE_CONTEXT"]["settings"] = wenlincms_settings

    # Ensure the wenlincms auth backend is enabled if
    # wlapps.accounts is being used.
    if "wlapps.accounts" in s["INSTALLED_APPS"]:
        auth_backend = "wenlincms.core.auth_backends.wenlincmsBackend"
        s.setdefault("AUTHENTICATION_BACKENDS", [])
        prepend("AUTHENTICATION_BACKENDS", auth_backend)

    # Ensure WlAdmin is after wenlincms in app order so that
    # admin templates are loaded in the correct order.
    wladmin_name = s.get("PACKAGE_NAME_ADMIN")
    if s["TESTING"]:
    ...
    if "wenlincms.generic" in s["INSTALLED_APPS"]:
        s.setdefault("COMMENTS_APP", "wenlincms.generic")
        append("INSTALLED_APPS", "django.contrib.comments")

    # Ensure wenlincms.boot is first.
    try:
        move("INSTALLED_APPS", "wenlincms.boot", 0)
    except ValueError:
        pass
