...
from wenlincms.conf import settings, registry
...
class SettingsForm(forms.Form):
    ...
    ``wenlincms.conf`` that is marked as editable.
    """
    ...
    def __iter__(self):
        ...
        # 下面对菜单重新排序
        try:
            from wlapps.wlcore.defaults import menu_order
            result = []
            # 先筛选order中指定的
            for g in menu_order:
                if len(menu_order[g]):
                    for i in range(len(menu_order[g])):
                        for f in fields:
                            if f.name == menu_order[g][i]:
                                f.group = g
                                result.append(f)
                else:
                    for f in fields:
            ...
            # order中未指定的和不是杂项的
            for f in fields:
                if f not in result and f.group != misc:
                    result.append(f)
            # 添加杂项
            for f in fields:
                if f not in result and f.group == misc:
