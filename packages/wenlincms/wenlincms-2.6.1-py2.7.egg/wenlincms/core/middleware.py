...
from wenlincms.utils.cache import (cache_key_prefix, nevercache_token, _hashed_key,
...
class _Deprecated(object):
    def __init__(self, *args, **kwargs):
        ...
        msg = "wenlincms.core.middleware.%s is deprecated." % self.old
        if self.new:
            msg += (" Please change the MIDDLEWARE_CLASSES setting to use "
                    "wenlincms.core.middleware.%s" % self.new)
        warn(msg)
...
class AdminLoginInterfaceSelectorMiddleware(object):
    ...
    def process_view(self, request, view_func, view_args, view_kwargs):
        login_type = request.POST.get("wenlincms_login_interface")
...
class UpdateCacheMiddleware(object):
    ...
    def process_response(self, request, response):
        ...
        # ``FetchFromCacheMiddleware`` having a cache get miss, the HTTP status must be OK
        # and the response mustn't include an expiry age, indicating it shouldn't be cached.
        marked_for_update = getattr(request, "_update_cache", False)
        anon = hasattr(request, "user")
    ...
    def process_request(self, request):
        ...
        if response and request.method == "POST":
            if resolve(request.get_full_path()).url_name == "fb_do_upload":
                # The handler for the flash file uploader in wlfilebrowser
                ...
                # when /admin is restricted to SSL using wenlincms's SSL
