...
def is_full_install():
    for app in ["forms", "galleries",]:
        if "wenlincms.%s" % app not in settings.INSTALLED_APPS:
            return False
    return True
...
def create_pages(app, created_models, verbosity, interactive, **kwargs):
    ...
    from wenlincms.forms.models import Form
...
if not settings.TESTING:
    ...
    if "wenlincms.pages" in settings.INSTALLED_APPS:
        from wenlincms.pages import models as pages_app
