...
Glover & MacLow (2007, ApJS, 169, 239), and using the updated rates
and expanded reaction set of Gong & Ostriker (2015, in prep).
"""
...
########################################################################
specList = ['H+', 'H2', 'H2+', 'H3+', 'He+', 'OHx', 'CHx', 'CO', 'C', 
            'C+', 'HCO+', 'O', 'M+']
...
_cr_react = [
    ...
    { 'spec' : ['H2', 'H2+', 'e-'], 'stoich' : [-1, 1, 1], 'rate': 2.0 },
    { 'spec' : ['C',  'C+',  'e-'], 'stoich' : [-1, 1, 1], 'rate': 3.85 },
    { 'spec' : ['CO', 'H',   'HCO+', 'e-'], 
      'stoich' : [-1, -1, 1, 1], 'rate': 6.52 },
    { 'spec' : ['CO', 'H2',  'HCO+', 'e-', 'H'], 
      'stoich' : [-1, -1, 1, 1, 1], 'rate': 2*6.52 },
    { 'spec' : ['C',  'C+',  'e-'], 'stoich' : [-1, 1, 1], 'rate': 1020 },
    { 'spec' : ['CO', 'C',   'O' ], 'stoich' : [-1, 1, 1], 'rate': 10 }
]
...
# (21) He+  + H2  -> H+   + He  + H
# (22) CHx  + H   -> H2   + C
# (23) H    + H   -> H2              (grain catalyzed)
# (24) H+   + e-  -> H               (grain catalyzed)
# (25) M+   + e-  -> M
# (26) H3+  + M   -> M+   + H   + H2
########################################################################
_twobody_react = [
...
def _twobody_ratecoef(T, nH, nH2, ne, chi):
    """
    ...
    -------
    ratecoef : array(26)
       Rate coefficients for the 26 reactions in the network
    """
    ...
    ratecoef[0] = 2.0e-9
    ratecoef[1] = 1.99e-9 * T**-0.190
    ratecoef[2] = 1.7e-9
    ratecoef[3] = 3.7e-14*np.exp(-35./T)
    ratecoef[4] = 1.6e-9
    ratecoef[5] = 3.3e-13 * T**-1.3 * np.exp(-23./T)
    ratecoef[6] = 9.15e-10 * (0.62 + 45.41/np.sqrt(T))
    ratecoef[7] = 7.7e-11
    ratecoef[8] = 7.95e-10 * T**-0.339 * np.exp(0.108/T)
    ratecoef[9] = 1.0e-11/np.sqrt(T) * \
                  (11.19 - 1.676*logT - 0.2852*logT**2 +
                   0.04433*logT**3)
    ratecoef[10] = 4.54e-7 * T**-0.52
    ratecoef[11] = 8.46e-7 * T**-0.52
    alpha = np.sqrt(T/6.670e-3)
    ...
    ratecoef[12] = krr + kdr
    ratecoef[13] = 1.15e-5 * T**-0.64
    ratecoef[14] = 2.84e-9 * T**0.42 * np.exp(-T/46600.)
    ratecoef[15] = 2.753e-14 * (315614./T)**1.5 * \
    ...
    ratecoef[19] = 7.2e-15
    ratecoef[20] = 3.7e-14*np.exp(-35./T)
    ratecoef[21] = 2.81e-11*T**0.26
    ratecoef[22] = 3.0e-18*np.sqrt(T)
    psi = chi*np.sqrt(T)/ne
    ratecoef[23] = 12.25e-14 / \
                   (1.0 + 8.074e-6*psi**1.378 *
                    (1.0 + 508.7*T**0.01586*psi**(-0.4723-1.102e-5*lnT)))
    ratecoef[24] = 3.8e-10*T**-0.65
    ratecoef[25] = 2.0e-9
...
class NL99_GO(chemNetwork):
    ...
    def __init__(self, cloud=None, info=None):
        ...
        if cloud is None:
            ...
            if info is not None:
                ...
                if 'Zd' in info.keys():
                    self._Zd = info['Zd']
                if 'temp' in info.keys():
                    self._temp = info['temp']
        ...
        abd1 = abundanceDict(specListExtended, self.extendAbundances())
        cfac = self.cfac
        n = self.nH * cfac
        rcoef = _twobody_ratecoef(
            self.temp, n, abd1['H2']*n, abd1['e-']*n, 
        ...
        abd['He+'] \
            ...
            (n * (abd1['H2'] * (rcoef[3]+rcoef[19]+rcoef[20]) +
                  abd1['CO'] * rcoef[4] +
                  abd1['e-'] * rcoef[9]))
    ...
    def extendAbundances(self, xin=None):
        ...
        xgrow = np.zeros(len(specListExtended))

        # Copy abundances if passed in; otherwise user stored ones
        if xin is None:
            xgrow[:len(specList)] = self.x
        else:
            xgrow[:len(specList)] = xin

        # Derive HI abundance from other types of H
        xgrow[13] = 1.0 - xgrow[0] - 2*(xgrow[1]+xgrow[2]) - \
                    3*xgrow[3]

        # He abundance = total He abundance - He+ abundance
        xgrow[14] = self.xHe - xgrow[4]

        # Neutral metal abundance = total metal abundance - ionized
        # metal abundance
        xgrow[15] = self.xM - xgrow[12]

        # e abundance = H+ + H2+ + H3+ + He+ + C+ + HCO+ + M+
        xgrow[16] = self.x[0] + self.x[2] + self.x[3] + self.x[4] + \
                    self.x[9] + self.x[10] + self.x[12]
    ...
    def dxdt(self, xin, time):
        ...
        mu = (xgrow[0]+xgrow[13] + 2*(xgrow[1]+xgrow[2]) +
            ...
            (xgrow[0] + xgrow[1] + xgrow[2] + xgrow[3] + xgrow[4] +
             xgrow[13] + xgrow[14] + xgrow[16])
        sigma_tot = np.sqrt(self.sigmaNT**2 + kB*self.temp / (mu*mH))
        ...
        xdot += _ph.dxdt(xgrow, n, self.chi, self.AV, 
                         [[self.NH*xgrow[1], sigma_tot],
                          [self.NH*xgrow[7], self.NH*xgrow[1]]])

        # Get rates for two-body reactions and add their contribution
        # to xdot
        ratecoef = _twobody_ratecoef(
            self.temp, n, xgrow[1]*n, xgrow[16]*n, 
            np.exp(-self.AV)*self.chi)
        xdot += _twobody.dxdt(xgrow, n, ratecoef, self.Zd)
