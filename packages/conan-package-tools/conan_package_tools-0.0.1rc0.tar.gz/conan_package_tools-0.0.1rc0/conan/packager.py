...
class ConanMultiPackager(object):
    ...
    default_gcc_versions = ["4.6", "4.8", "4.9", "5.1", "5.2", "5.3"]
    ...
    def __init__(self, args=None, username=None, channel=None, runner=None,
        ...
        self.channel = channel or self._get_channel(stable_branch_pattern)
    ...
    def add_common_builds(self, shared_option_name=None, pure_c=True, visual_versions=None):
        ...
        if visual_versions:
            self.logger.warn("Parameter 'visual_versions' for 'add_common_builds' method are"
                             " deprecated and will be removed soon. Please use the specified in the project docss.")
    ...
    def _docker_pack(self, curpage=1, total_pages=1):
        ...
        for gcc_version in self.gcc_versions:
            ...
            command = "sudo docker run --rm -v %s:/home/conan/project -v " \
                      ...
                      "cd project && sudo pip install conan_package_tools==0.0.1rc0 --upgrade && sudo pip install conan==0.0.1rc7 --upgrade && " \
    ...
    def upload_packages(self, reference, password, remote=None):
        self.logger.warn("Method upload_packages is deprecated and will be removed soon.\n Please"
                         " instance ConanMultiPackager with 'reference', 'password', 'remote' and 'upload' parameters"
                         " or use the environment variables and call run(). Read the project docs for more information")
    ...
    def _upload_packages(self):
        ...
        if not self.reference or not self.password or not self.channel or not self.username:
            return
        command = "conan upload %s/%s/%s --all --force" % (self.reference, self.username, self.channel) 
    ...
    def _get_channel(self, stable_branch_pattern):

        pattern = stable_branch_pattern or "master"
        ...
        channel = "stable" if appveyor and prog.match(appveyor_branch) and not os.getenv("APPVEYOR_PULL_REQUEST_NUMBER") else channel

        return channel or os.getenv("CONAN_CHANNEL", "testing")
