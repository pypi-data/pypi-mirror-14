...
#   12) Huge: add state statutes. ALREADY WORKS FOR WESTLAW
...
#           if it's a statute, go straight to westlaw PDF download rather than original image
#   18) See if you can use the element class function is_enabled more! Much more useful than
#           wait and click.
#   19) Possible solution for law review article ToCs: 
#           a) before renaming article, ask if user wants to manually download Toc
#           b) if so, allow to download and prompt user to merge two most recently
#               downloaded files (i.e., or rather ideally, the article and the ToC)
#           c) if not, call renaming function on downloaded PDF file
#           NOTE: which will come first? Newest file comes first, I believe. Can always
#                 change.
#   20) Add check to see if getcite has been updated. If so, notify user.
#   21) Make sure rename also moves to correct directory--or maybe not--would we need
#       to specify whether history or source? (another problem solved by using a class
#       object)
#   22) Things that could be done with classes:
#           Could contain a dictionary with previous entries, so if a "history" is sought,
#               it could automatically pull up the preferred name and the footnote number
#           The dictionary could also have an element that indicated whether the entry was 
#               a history or a source
#   24) If a case has no history, create a PDF file that says, NO HISTORY.
#           additionally: if no neg. history, create that on PDF, then add history, or vice versa
#       
#            
#
#   BUGS
#       (1) If you get the history for a U.S.C. statute, sometimes there are multiple
#       options in Westlaw. getcite can't handle this, it expects to go straight
#       to the statute. Add a try, and if there are two, click one of them (which one?)
#       (2) If you use the ' character, it doesn't allow you to go forward
#
#
#
...
def merge_most_recent_files():
    # Merges recently downloaded file
    # in the Source folder (article) with the most recently
    # downloaded file in the downloads folder (toc)
    ...
    # this is our second file, that will go first.
    ...
    if ".DS_Store" in files[-1]:
        if len(files) > 1:
            ...
            print("No files to merge in download directory! Please make sure the directory is not empty.")
            return
    else:
        newest_files.append(files[-1])
    
    print(newest_files[0])

    os.chdir(DOWNLOAD_DIR+SOURCE_DIR)

    # create a list from all the files (excluding directories) in the SOURCE direcory
    # this is our first file
    files = filter(os.path.isfile, os.listdir(DOWNLOAD_DIR+SOURCE_DIR))

    # add directory path to the file name
    files = [os.path.join(DOWNLOAD_DIR+SOURCE_DIR, f) for f in files]
    ...
    if ".DS_Store" in files[-1]:
        if len(files) > 1:
            ...
            print("No files to merge in source directory! Please make sure the directory is not empty.")
            return
    else:
        newest_files.append(files[-1])
    
    print(newest_files[1])

    answer = None

    # Make sure user wants to merge the files!
    answer = input("CAUTION: getcite is about to merge the files above. Are you sure you want to continue? (y/n): ")

    while True:

        if not re.match('^[ynYn]+$', answer):
            answer = input("Invalid answer. (y/n): ")
        # if not, just go back
        if answer.lower() == 'n':
            return
        if answer.lower() == 'y':
    ...
    # now ask what to call output DON't Need this if just using second file name
    # saved_name = input("Please type the citation or name you would like to use for the\n\
# merged document, or type \"stop\": ")
    # if len(saved_name) > 0:
    #    citation = saved_name
    # if "stop" in saved_name:
    #    return

    # now ask for footnote (how was this not here before??)
    ...
    ## merger.write(author_abbreviation+" "+footnote_number+" "+citation+" "+editor_abbreviation+".pdf")
    ## replacing this with just using newest_files[1]
    merger.write(newest_files[1])
    
    #print("File saved as: %s" % author_abbreviation+" "+footnote_number+" "+citation+" "+editor_abbreviation+".pdf")
    print("File saved as: %s" % newest_files[1])
    return

"""


    # remove the old files (needs to be done first on windows)
    for filename in newest_files:
        os.remove(filename)
    ...
    if ".DS_Store" in files[-1]:
        if len(files) > 1:
            ...
            print("No files exist to move. Please try again.")
            return
    else:
        newest_file_for_moving = files[-1]

    # move it to the source directory
    os.rename(newest_file_for_moving, DOWNLOAD_DIR+SOURCE_DIR+author_abbreviation+" "+footnote_number+" "+citation+" "+editor_abbreviation+".pdf")
"""
...
or type \"stop\" to go back: ")
        ...
        if ".DS_Store" in files[-1]:
            if len(files) > 1:
                ...
                print("No files! Please make sure the directory is not empty.")
                return
        else:
            newest_file = files[-1]
        print(newest_file)
...
def get_westlaw_history(citation, driver, with_args):
    ...
    if "Signon" in driver.title:
        ...
        try:
            ...
            elem = wait.until(EC.presence_of_element_located((By.ID, "co_clientIDContinueButton")))
            elem.click()
    ...
    try:
        ...
        print("Element not visible. Please try again.")
        return
...
def get_westlaw_citation(citation, driver):
    ...
    if "Signon" in driver.title:
        ...
        try:
            ...
            print("Timed out while logging in. Please try again.")
            return
    ...
    try:
        ...
        print("Timed out finding search box...")
        return
    except ElementNotVisibleException:
        print("Search box not visible... waiting.")
        time.sleep(2)
        try:
            wait = WebDriverWait(driver, 6)
            print("Locating...")
    ...
    try:
        ...
        print("Timed out finding search button...")
        return
    except ElementNotVisibleException:
        print("Search button not visible... try again.")
        return
...
def main():
    ...
    citation = None
    prev_citation = None
    commands = ["article", "help", "history", "rename", "merge"]
    ...
    while True:
        ...
        elif "merge" in citation.lower():
            merge_most_recent_files()
