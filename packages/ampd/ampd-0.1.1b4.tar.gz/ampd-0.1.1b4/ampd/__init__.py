...
_loglevel = os.environ.get('LOGLEVEL_AMPD', os.environ.get('LOGLEVEL', 'INFO'))
_logger = logging.getLogger('ampd')
_logger.setLevel(_loglevel)
_formatter = logging.Formatter(fmt='%(levelname)s: %(name)s: %(message)s (%(pathname)s %(lineno)d)')
...
from ._client import Client, ServerProperties
