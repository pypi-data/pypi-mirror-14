...
class ClientGLib(_client.Client, GObject.GObject):
    ...
    __gsignals__ = {
        ...
        'client-disconnected': (GObject.SIGNAL_RUN_FIRST, None, (int,)),
    }
    ...
    def __init__(self, coding=_client.Client._DEFAULT_CODING, coding_server='utf-8', excepthook=None):
        ...
        self.connect_loop(self._connected_cb, self._disconnected_cb)

    def _connected_cb(self):
        self.emit('client-connected')

    def _disconnected_cb(self, reason):
        self.emit('client-disconnected', reason)
...
class ServerPropertiesGLib(_client.ServerProperties, GObject.GObject):
    ...
    def _update_status(self, status):
        super(ServerPropertiesGLib, self)._update_status(status)
