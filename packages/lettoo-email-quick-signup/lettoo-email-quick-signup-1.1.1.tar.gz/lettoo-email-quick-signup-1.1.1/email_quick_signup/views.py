...
from .status import ApiStatus, EMAIL_AND_KEY_REQUIRED, CONFIRMATION_DOES_NOT_EXIST, VERIFICATION_KEY_INVALID, \
...
class VerifyEmailView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if not self.kwargs['email'] or not self.kwargs['key']:
            return Response(ApiStatus(EMAIL_AND_KEY_REQUIRED).get_json(), status=status.HTTP_400_BAD_REQUEST)
        ...
        if result == 'invalid':
            ...
            return Response({'key': token.key, 'user': email_address.user.dict()}, status=status.HTTP_200_OK)
