...
class ChangeEmailView(APIView, FormView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if self.form.is_valid():
            ...
            if QuickEmailAddress.objects.filter(user=request.user, email=email, verified=True).exists():
                return Response(ApiStatus(CHANGE_EMAIL_ADDRESS_EXIST).get_json(), status=status.HTTP_400_BAD_REQUEST)

            email_address, created = self.get_or_create_object(request.user, email)
            email_address.change(request, email, confirm=False)
            send_email = send_email_confirmation(request, email_address.user, signup=False)
    ...
    def get_or_create_object(self, user, email):
        queryset = QuickEmailAddress.objects.filter(user=user, primary=True)

        if queryset and len(queryset) > 0:
            return queryset[0], False
        else:
            obj = QuickEmailAddress.objects.create(user=user, email=email)
            return obj, True
