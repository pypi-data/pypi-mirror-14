...
class Downloader(object):
    ...
    def retrieve(self):
        ...
        while retry():
            ...
            try:
                ...
                logger.warn('Connection error: {}!'.format(e))
                logger.info(action)
                if retry():
                    time.sleep(5)
    ...
    def _transfer(self):
        ...
        with self._open_file as out_file:
            with Progress(self.url, rel, self.content_length,
                          self._resume_size) as progress:
    ...
    def _open_file(self):
        ...
        return open(self.file_path, flags)
    ...
    def _check_size(self):
        ...
        if self.content_length > available:
            raise NotEnoughDiskSpace(self._download_dir, self.content_length,
                                     available)
    ...
    def outfile_size(self):
        return (os.path.getsize(self.file_path) if self._file_exists else 0)
