...
class Graph:
    ...
    def link_nodes(self, source, destination, weight=1, weights=[1, 1], bidirectional=False):
        ...
        if bidirectional:
            ...
            s_d_link = source.link_to(destination, weights[0])
            d_s_link = destination.link_to(source, weights[1])
    ...
    def get_link(self, source, destination):
        if not self.nodes_are_linked(source, destination):
            return None
        n0 = self.find_node_by_label(source)
        return n0.links()[destination]
