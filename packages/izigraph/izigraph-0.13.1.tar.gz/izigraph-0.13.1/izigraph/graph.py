...
class Graph:
    ...
    def nodes(self):
        ...
        """
        return OrderedDict(sorted(self.__nodes.items(), key=lambda t: int(t[0])))
    ...
    def links_count(self):
        if self.__is_directed:
            return len(self.__links_dict)
        return len(self.__links_dict)/2
    ...
    def link_nodes(self, source, destination, weight=1, weights=None, bidirectional=False, weight_by_distance=False):
        """
        :param weight_by_distance:
        ...
        if source.label() not in self.__node_neighbors:
            self.__node_neighbors[source.label()] = []
        if not self.__is_directed and destination.label() not in self.__node_neighbors:
            self.__node_neighbors[destination.label()] = []
        if bidirectional:
            if weights is None:
                weights = [1, 1]
            if not isinstance(weights, list) and len(weights) != 2:
                raise ValueError('weights must be a list of two elements')
            if destination.label() not in self.__node_neighbors:
                self.__node_neighbors[destination.label()] = []
            s_d_link = source.link_to(destination, weight=weights[0])
            d_s_link = destination.link_to(source, weight=weights[1])
            self.__links_dict[(source.label(), destination.label())] = s_d_link
            self.__links_dict[(destination.label(), source.label())] = d_s_link
            self.__node_neighbors[source.label()].append(destination)
            self.__node_neighbors[destination.label()].append(source)
            return s_d_link, d_s_link
        if weight_by_distance:
            weight = self.get_distance_between_nodes(source, destination)
        s_d_link = source.link_to(destination, weight=weight)
        self.__links_dict[(source.label(), destination.label())] = s_d_link
        self.__node_neighbors[source.label()].append(destination)
        if not self.__is_directed:
            self.__links_dict[(destination.label(), source.label())] = destination.link_to(source, weight=weight)
            self.__node_neighbors[destination.label()].append(source)
        return s_d_link
    ...
    def links_dict(self):
        return OrderedDict(sorted(self.__links_dict.items(), key=lambda link: link[0]))

    def get_distance_between_nodes(self, source, destination):
        from math import sqrt, pow
        if isinstance(source, str):
            source = self.find_node_by_label(source)
        if isinstance(destination, str):
            destination = self.find_node_by_label(destination)
        x_distance = destination.position()[0] - source.position()[0]
        y_distance = destination.position()[1] - source.position()[1]
        distance_sum = pow(x_distance, 2) + pow(y_distance, 2)
        return sqrt(distance_sum)

    def unlink_nodes(self, source, destination):
        if isinstance(source, str):
            source = self.find_node_by_label(source)
        if isinstance(destination, str):
            destination = self.find_node_by_label(destination)
        if self.nodes_are_linked(source, destination):
            del(self.__links_dict[source.label(), destination.label()])
            source.unlink_from(destination)
            if not self.__is_directed:
                del (self.__links_dict[destination.label(), source.label()])
                destination.unlink_from(source)
            return True
        return False

    def node_neighbors(self):
        return OrderedDict(sorted(self.__node_neighbors.items(), key=lambda t: t[0]))

    def get_neighbors_from_node(self, node):
        if isinstance(node, str):
            return self.__node_neighbors[node]
        return self.__node_neighbors[node.label()]
