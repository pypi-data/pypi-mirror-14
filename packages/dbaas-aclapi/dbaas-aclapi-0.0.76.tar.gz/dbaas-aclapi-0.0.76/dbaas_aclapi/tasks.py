...
def unbind_address_on_database(self, database_bind, user=None):
    ...
    AuditRequest.new_request("destroy_databasebind", "Admin", "localhost")
...
def monitor_aclapi_jobs(self,):
    ...
    try:
        ...
        for job in AclApiJob.objects.filter(
                job_status__in=[PENDING, RUNNING, ERROR], database__isnull=False):
            acl_client = get_acl_client(job.database.database.environment)
