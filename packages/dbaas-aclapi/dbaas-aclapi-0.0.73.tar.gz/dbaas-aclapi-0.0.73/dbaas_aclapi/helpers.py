...
def bind_address(database_bind, acl_client):
    from dbaas_cloudstack.models import DatabaseInfraAttr
    from dbaas_aclapi.models import (CREATED, DatabaseBind,
...
def unbind_address(database_bind, acl_client, infra_instances_binds):
    action = "permit"
    database = database_bind.database
...
def monitor_acl_job(job_id, acl_client, retries=600, interval=200):
    LOG.debug("Monitoring job_id: {}".format(job_id))
