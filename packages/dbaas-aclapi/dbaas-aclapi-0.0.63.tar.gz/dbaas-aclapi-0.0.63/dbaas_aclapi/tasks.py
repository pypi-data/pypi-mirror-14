...
def unbind_address_on_database(self, database_bind, user=None):
    ...
    try:
        ...
        for infra_instance_bind in infra_instances_binds:
            ...
            try:
                query_results = acl_client.query_acls(custom_options)
                for vlan in query_results['envs']:
                    for environment in vlan['vlans']:
                        environment_id = environment['environment']
                        for rule in environment['rules']:
                            rule_id = rule['id']
                            response = acl_client.delete_acl_by_id(environment_id, rule_id)
                            if 'jobs' in response:
                                job_list.append(response['jobs'])
                infra_instance_bind.delete()
            except Exception as e:
                raise Exception("Access {} could not be deleted! Error: {}".format(
                    infra_instance_bind, e))

        for job in [item for sublist in job_list for item in sublist]:
            if monitor_acl_job(job_id=job, database_bind=database_bind):
                continue
            else:
...
def monitor_acl_job(job_id, database_bind, retries=600, interval=200):
    database = database_bind.database
    ...
    for attempt in range(retries):
        ...
        sleep(interval)
        LOG.info("Monitoring Job for {} minutes".format(interval / 60 / attempt))
