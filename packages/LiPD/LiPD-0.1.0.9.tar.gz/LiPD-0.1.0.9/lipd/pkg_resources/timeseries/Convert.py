...
class Convert(object):
    ...
    def lipd_extract_main(self, lipd_tsos):
        ...
        for tso in self.lipd_tsos:
            ...
            self.lipd_extract_paleo()

        return self.lipd_master

    def lipd_set_current(self):
        """
    ...
    def verify_tsnames(self):
        ...
        for name, tso in self.ts_tsos.items():
            ...
            for incorrect in bad_keys:
                ...
                self.recent[incorrect] = self.get_valid_tsname(incorrect)
    ...
    def get_valid_tsname(self, invalid):
        """
        ...
        try:
            ...
            if re_pub_invalid.match(invalid):
                ...
                # Case 4: pubYear (camelcase)
                    ...
                    valid = self.iter_ts('pub', invalid)
            ...
            # CLIMATE INTERPRETATION
                ...
                valid = self.iter_ts('climateInterpretation', g1)

            else:
                # ROOT
                valid = self.iter_ts('root', invalid)

            # LAST CHANCE:
            # Specific case that isn't a typical format, or no match. Go through all possible entries.
            if not valid:
                valid = self.iter_ts(None, invalid)
    ...
    def iter_ts(self, category, invalid):
        """
