...
class PostInstall(install):
    ...
    def run(self):
        ...
        # Moving code to the notebooks folder
        # dir_pkg = os.path.join(dir_nb, "pkg_resources")
        # if os.path.exists(dir_pkg):
        #     shutil.rmtree(dir_pkg)
        # shutil.copytree(os.path.join(here, 'lipd/pkg_resources'), dir_pkg)

        if not os.path.isdir(dir_nb):
            os.mkdir(dir_nb)

        # Copy quickstart notebook and resources to LiPD Notebooks folder
        shutil.copy(os.path.join(here, 'lipd/examples/Welcome LiPD - Quickstart.ipynb'), dir_nb)
