...
class Base:
    ...
    #----------------------------------------------------------------------
    def __custom_request__(self, url, call, method, **kwargs):
        """"""
        kwargs = self.__fix_lists__(kwargs)
        if method == "get":
            response = requests.get(url+call+"/", params=kwargs)
        elif method == "post":
            ...
            if csrftoken:
                kwargs.update({"csrfmiddlewaretoken": csrftoken})
            response = client.post(url+call+"/", data=kwargs)
