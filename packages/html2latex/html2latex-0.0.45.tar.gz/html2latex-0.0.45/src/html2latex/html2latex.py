...
class Table(HTMLElement):
    ...
    def __init__(self, element, *args, **kwargs):
        ...
        table_width_scaleup_factor = 0.8

        for col_width in col_widths:
            colspecifier = r">{\raggedright\arraybackslash}p{%1.4f\linewidth}" % (
                float(table_width_scaleup_factor * col_width / total_width))

            colspecifiers.append(colspecifier)

        first_row = self.element.findall('.//tr')[0]

        for td, colspecifier in zip(first_row.findall('.//td'), colspecifiers):
            if td is None:
                continue
        ...
        for (col_index, td) in enumerate(children):
            ...
            if colspan:
                ...
                for i in range(col_index + 1, col_index + colspan):
                    elements_to_be_deleted.append(children[i])
    ...
    def render(self, *args, **kwargs):
        ...
        if is_first_row:
            if not ('multicol' in latex_code or 'multirow' in latex_code):
                ...
                latex_code = r'\multicolumn{1}{|' + colspecifier + r'|}{\cellcolor{black!20}\fontseries{b}\selectfont ' + latex_code + r'}'
