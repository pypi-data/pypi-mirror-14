    ...
    class HttpRequestHandler(BaseHTTPServer.BaseHTTPRequestHandler):
        ...
        def do_GET(self):
            ...
            if session_cookie.getvalue() != '':
                logger.debug('sending session cookie {}'.format(
            ...
            if self.path == '/auth.do':
                ...
                for i in self.headers:
                    if i == 'content-length':
                        ...
                        logger.debug('session cookie value: {}'.format(
                                     session_cookie.getvalue()))
            ...
            while counter <= 4:
                if session_cookie.getvalue() is not '':
                    ...
                    if 'Location' in ret.headers.keys():
                        ...
                        poker.truncate(0)
                        poker.write('valid')
                        self.logger.info('session cookie expire at {}, '
    ...
    * Raise exceptions when failing to start or stop as needed.
    ...
    def daemon_status():
        ...
        * raise an exception if pid file is missing whilst the sub_daemon
        ...
        """
        pid_value = find_pid()
        logger.debug('pid_value: {}'.format(pid_value))
...
if __name__ == '__main__':
    ...
    if sys.argv[1] in action_item.keys():
        ...
        sys.exit('argument {} is not supported.\n'
