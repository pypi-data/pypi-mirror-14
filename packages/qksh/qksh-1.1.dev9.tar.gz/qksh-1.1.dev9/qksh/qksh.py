...
# TODO: need to put below code into some sort of initial setup script
...
def _parse_arg(arg_list=None, ):
    ...
    # and do regex check
    logger.info('Checking arguments, argument list: {}'.format(
    ...
    if len(arg_list) > 1:
        ...
        if arg_list[1] == 'status':
            if dctl.daemon_status()[0] == 1:
                ...
                sys.exit('sub_daemon is running, PID: {}'.format(
        ...
        if arg_list[1] == 'start':
            ...
            if g[0] == 1:
                ...
                sys.exit('sub_daemon already running, PID: {}'.format(g[1]))
            # toggle.start_daemon()
            # sys.exit('sub_daemon started.')
        ...
        if arg_list[1] == 'restart':
            dctl.daemon_restart()
            sys.exit('sub_daemon {}'.format(arg_list[1]))
...
def _auth_with_f5site(obj):
    ...
    * Raise an error if found response code is 401. see:
    ...
    try:
        ...
        logger.debug('returning value of get_cookie(): {}'.format(auth_cookie))
        if auth_cookie == 401:
            raise AuthenticateError
    except AuthenticateError as e:
        dctl.daemon_stop()
        logger.warning(
            'authentication fail, stop sub_daemon, {}'.format(e.dump_error())
        )
        raise
...
def pid_check(session_obj):
    ...
    logger.debug('print ret value: {}'.format(ret))
    if ret[0] == 1:
...
def main():
    ...
    logger.debug('case match? {}'.format(ret))
    if ret == 1:
