...
# TODO: need to move to logging system, logging appears not working...
def logging_config():
...
class RetrieveCookie:
    ...
    logger = logging.getLogger("sub_daemon.ihealth_api")
    ...
    def retrieve_cookie(self):
        ...
        self.logger.debug('value: %s', pickle.loads(pickled_cookie.content))
        return requests.utils.cookiejar_from_dict(
            pickle.loads(pickled_cookie.content)[0])
    ...
    def get_cookie(username, password):
        ...
        if ret.status_code != 200:
            print "Server Status Code: " + str(ret.status_code)
    ...
    def get_file(self, file_link, file_name=None):
        ...
        ret = requests.get(file_link, verify=False, cookies=cookie)
    ...
    def uploadqkview(self, case_num, qkview_path):
        ...
        if ret.status_code == 303 and "Location" in ret.headers:
            ...
            print "qkview uploaded, Link: " \
                  "https://ihealth.f5.com/qkview-analyzer/qv/"+hash_id
        else:
            print "qkview uploading failed. Server status code:", \
                ret.status_code
        return ret.content
