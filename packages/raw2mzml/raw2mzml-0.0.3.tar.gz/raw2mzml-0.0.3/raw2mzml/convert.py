...
def convert_single_file(input_file, output_file, extract_levels, pick_levels, intensity_threshold,
    ...
    this function returns an integers, where 0 indicates successfull conversion, other
    codes > 0 indicate error conditions. These values come from msconvert.exe.
    """
    ...
    proc.wait()
    print("conversion %-6s: %s" % ("ok" if proc.returncode == 0 else "failed", input_file))
...
def convert_batch(input_files, target_folder, extract_levels, pick_levels, format_,
    ...
    this function returns a list of integers, where values 0 indicate successfull conversion, other
    codes > 0 indicate error conditions. These values come from msconvert.exe.
    """
    ...
    if len(input_files) == 1:
        return_code =  convert_single_file(input_files[0], output_files[0], extract_levels, pick_levels,
        ...
        return_codes = pool.map(target_function, args)
