...
#TODO: Remove this class if this is not getting used.
...
#TODO: Use setup method to initialize the client and the use that.
class UnitTest(TestCase):
    ...
    def test_dispatch_api(self):
        ...
        response = c.get('/api/charts/timesheet4.json/?query=q1&start=1&end=12', {})
        ...
        response = c.get('/api/charts/timesheet.json/?query=q1', {})
        self.assertEqual(response.status_code, 200)
        received_data = json.loads(response.content)
        self.assertGreaterEqual(len(received_data), 1)
        self.assertGreaterEqual(len(received_data[0]['rows']), 1)
    ...
    def test_dispatch_xls(self):
        c = Client()
        response = c.get('/api/charts/timsheet.xls/?query=q1', {})
        self.assertEqual(response.status_code, 200)
    ...
    def test_dispatch_pdf(self):
        c = Client()
        response = c.get('/api/reports/timesheet.pdf/?query=q1', {})
        self.assertEqual(response.status_code, 200)
