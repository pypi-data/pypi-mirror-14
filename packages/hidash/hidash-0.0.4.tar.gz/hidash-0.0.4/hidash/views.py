...
def dispatch_chart(request, chart_id):
    ...
    for key, value in charts.iteritems():
        if key == chart_id:
            if check_permissions(charts.get(key), request):
...
def _parse_charts(charts_xml):
    ...
    for chart in charts:
        ...
        if chart.find("type") is not None:
            ...
            chart_type = "Table"
        current_chart = ChartData(chart_id, db, group, permissions_list, chart_handler, current_queries, current_columns, chart_type)
