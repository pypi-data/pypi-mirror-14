...
def wctot_dict(wc_obj, sector, scale, par):
    ...
    if sector[:2] == 'bs':
        ...
        c7 = wcsm_120[6] - np.dot(yi, wcsm_120[:6]) # c7 (not effective!)
        c8 = wcsm_120[6] - np.dot(zi, wcsm_120[:6]) # c8 (not effective!)
        eps_s = running.get_ms(par, scale)/running.get_mb(par, scale)
        c7p = eps_s * c7
        c8p = eps_s * c8
        wcsm_120[21] = c7p + np.dot(yi, wcsm_120[15:21]) # c7p_eff
        wcsm_120[22] = c7p + np.dot(zi, wcsm_120[15:21]) # c8p_eff
