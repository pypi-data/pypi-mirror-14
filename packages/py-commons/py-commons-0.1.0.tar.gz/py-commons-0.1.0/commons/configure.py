...
class Factory(Directive):
    ...
    def process_element(self, arg, ctx):
        """
        recursively simplify and resolve directives for arg using ctx as a context
        """
        if isinstance(arg, Directive):
            return self.process_element(arg(ctx), ctx)
        elif hasattr(arg, 'items'):
            res = {}
            for k, v in arg.items():
                res[k] = self.process_element(v, ctx)
            return res
        elif isinstance(arg, list):
            return map(partial(self.process_element, ctx=ctx), arg)
        else:
            return arg
    ...
    def __call__(self, ctx):
        ...
        for a in argspec.args[:pos_cut]:
            ...
            arg = config.pop(a)
            arg = self.process_element(arg, ctx)
            args.append(arg)

        for a in argspec.args[pos_cut:]:
            if a in config:
                arg = config.pop(a)
                kwargs[a] = self.process_element(arg, ctx)

        if argspec.keywords:
            for k in list(config.keys()):
                arg = config.pop(k)
                kwargs[k] = self.process_element(arg, ctx)
...
class FileContent(Directive):
    def __init__(self, filename):
        self.filename = filename

    def __call__(self, ctx):
        import os
        item = path.join(ctx._pwd, self.filename)
        if not os.path.exists(item):
            raise ConfigurationError('No such file: {}'.format(item))

        with open(item, 'r') as f:
            return ''.join(f.readlines())

@Configuration.add_multi_constructor('file')
def _read_file_constructor(loader, tag, node):
...
def import_string(import_name, silent=False):
    ...
    if isinstance(import_name, string_types):
