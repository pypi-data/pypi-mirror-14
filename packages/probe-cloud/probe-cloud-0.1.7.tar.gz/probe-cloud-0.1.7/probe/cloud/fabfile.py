...
def _get_params(sim, probe=False, destdir='.', tmp=False):
    ...
    if probe:
        user = 'probe'
        host = 'argo.physics.muni.cz'
        params_from = os.path.join(sim['simdir_archive'], '{}.params'.format(sim['name']))
    else:
        user = sim['user']
        host = sim['host']
        params_from = os.path.join(sim['simdir'], 'input.params')
    ...
    print '[_get_params] user: {}, host: {}, params_from: {}, params_to: {}'.format(user, host, params_from, params_to)

    local('scp {}@{}:{} {}'.format(user, host, params_from, params_to))
...
def get_remote_info():
    ...
    is_probe_cloud = run('ps aux | grep probe_cloud.py | grep -v grep', quiet=True).stdout
    if is_probe_cloud == '':
        is_running = False
        whois = None
    else:
