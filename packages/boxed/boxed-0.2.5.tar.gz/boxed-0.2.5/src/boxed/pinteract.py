...
class Pinteract:
    """
    Conveniently interact with a child process.
    """
    ...
    def status(self):
        ...
        try:
            status = self._psdata.status()
        except psutil.NoSuchProcess:
            return 'dead'
        return self._status_reduction.get(status, status)
    
    def is_running(self):
        """Return True if child process is still running."""
        
        return self.status() in ('running', 'disk-sleep')
