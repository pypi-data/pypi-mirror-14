...
if not COVE_API_ID or not COVE_API_SECRET:
    raise ImproperlyConfigured(
        "In order to use the PBS COVE service with you must "
...
COVE_EMBED_CODE_TEMPLATE = (
    ...
    "src='http://player.pbs.org/partnerplayer/__EMBEDKEY__/"
    "?autoplay=true&start=0&end=0&chapterbar=true&topbar="
    "false&endscreen=true' allowfullscreen></iframe>"
)


def process_cove_video(instance):
    """
    Process a StacksEmbed instance associated with the COVE service
    ...
    if cove_video.get('count') == 1:
        ...
        if not instance.poster_image:
            ...
            if image_url is not None:
                img = create_wagtail_image_from_url(image_url, instance.name)
