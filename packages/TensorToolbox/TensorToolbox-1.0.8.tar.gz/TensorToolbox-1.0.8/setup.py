...
def deps_install(package):
    print("[DEPENDENCY] Installing %s" % package)
    try:
        pip.main(['install', '--no-use-wheel', '--upgrade', package])
...
class TensorToolbox_install(install):
    def run(self):
        deps_install('SpectralToolbox')
        import numpy as np
        include_dirs.append(np.get_include())
        deps_install('Cython')
        deps_install('prettytable')
        deps_install('h5py')
        deps_install('matplotlib')
        deps_install('UQToolbox')
        deps_install('mpi4py')
        deps_install('mpi_map')
        install.run(self)
