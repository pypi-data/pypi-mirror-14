...
DEFAULT_FILE_NAME = {"nginx": "nginx.conf"}
DEFAULT_FILE_TEMPLATE = {"nginx": "nginx.conf.jinja2"}
DEFAULT_DIR_NAME = {"nginx": ["glb"]}
...
def deal_default_conf_file(service_type, conf_dist,
                           old_conf_dist, cf_names):
    if service_type == 'nginx':
        flag = False
        f_name = DEFAULT_FILE_NAME.get(service_type)
        template_name = DEFAULT_FILE_TEMPLATE.get(service_type)
        conf_file_path = "%s%s" % (conf_dist, f_name)
        for dir_name in DEFAULT_DIR_NAME.get(service_type, []):
        ...
        # TODO deal default conf with haproxy
        pass
...
class Process:
    ...
    def run_data(self, lock, t):
        while t.isAlive():
            ...
            if config_files:
                cf_names = config_files.get('cf_names', {})
                deal_default_conf_file(self.s_type,
                                       self.conf_dist,
