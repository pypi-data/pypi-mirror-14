...
Image.DEBUG = False
ImageFont.DEBUG = False
ImageDraw.DEBUG = False
import argparse
import os
...
def make_movie_from_pngs(image_path, **kwargs):
  ...
  if add_time is not None:
    ...
    if "RTplot.ppm" in path:
      rtp = lcse.rtplot_reader(path)
      time_map = dict((dump, val['T']) for dump, val in rtp.dump_map.items())
    elif "AProfile-01" in path:
      ap_set = lcse.aprofile_set(path, lazy=True)
      time_map = dict((rp.dump, rp.time / 60.0) for rp in ap_set)
    elif "RProfile-01" in path:
      rp_set = lcse.rprofile_set(path, lazy=True)
      time_map = dict((rp.dump, rp.get('timerescaled')) for rp in rp_set)
    else:
      time_map = {}

    image_path = add_time_to_pngs(image_path, prefix, kwargs.get('font'),
                                  kwargs.get('tmp_dir'), time_map,
...
def build_argparse():
  ...
  parser.add_argument('--add-time', nargs='*',
        help='Add dump labels and optionally time if given an RTplot.ppm file or RProfile-01 / AProfile-1 directories')
