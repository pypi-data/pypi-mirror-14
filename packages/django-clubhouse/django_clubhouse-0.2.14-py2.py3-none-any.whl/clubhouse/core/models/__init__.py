...
def ensure_model(model_or_string):
    if isinstance(model_or_string, basestring):
        try:
            model_or_string = apps.get_model(*model_or_string.split('.'))
        except TypeError:
            raise ValueError('Invalid block context: %s' % model_or_string)

    if not isinstance(model_or_string, models.base.ModelBase):
        raise ValueError('Block context is not a model: %s' % model_or_string)
