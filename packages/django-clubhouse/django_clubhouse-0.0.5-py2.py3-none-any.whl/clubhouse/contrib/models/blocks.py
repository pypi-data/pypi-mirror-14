...
from clubhouse.core.models import BlockBase,ReusableBlock
from clubhouse.contrib.models.types import ContentBlock, AsideBlock
from clubhouse.contrib.models.pages import PageBlock,SizeMixin

__all__ = ['GroupingBlock','GroupContentBlock','WysiwygBlock','GalleryBlock',
    'GalleryBlockImage','VerticalSpacingBlock','TwitterFeedBlock',
    'RowSeparatorBlock']


class GroupingBlock(ReusableBlock):
    block_types = (ContentBlock,)
    class Meta:
...
class GroupContentBlock(PageBlock,SizeMixin):
    group = models.ForeignKey(GroupingBlock, related_name='content_blocks', on_delete=models.CASCADE)
    class Meta:
        ordering = ('order',)
    ...
    def template(self):
        try:
            return super(GroupContentBlock,self).template
...
class GalleryBlock(ReusableBlock):
    block_types = (ContentBlock, AsideBlock, BaseGallery)
    class Meta:
...
class VerticalSpacingBlock(BlockBase):
    ...
    block_types = (ContentBlock, AsideBlock)
