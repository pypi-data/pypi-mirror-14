...
from clubhouse.core.models import BlockType, ReusableBlock, BlockBase
...
class BlockTypeAdmin(six.with_metaclass(BlockTypeAdminBase,BaseModelAdmin)):
    ...
    def __init__(self, block_type, admin_site):
        if not issubclass(block_type, BlockType):
            raise ImproperlyConfigured('Block type %s does not extend '
                    'clubhouse_core.BlockType' % block_type.__class__\
                            .__name__)
