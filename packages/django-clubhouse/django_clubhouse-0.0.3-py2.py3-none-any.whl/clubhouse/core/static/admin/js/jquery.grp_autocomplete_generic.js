...
(function($){
    ...
    var lookup_link = function(id, val) {
        ...
        var goto_edit = (obj_val || ! MODEL_URL_ARRAY[val].selectable);
        var admin_suffix = obj_val ? obj_val : (goto_edit ? 'add' : '');
        var lookuplink_id = (goto_edit ? 'edit_' : 'lookup_') + id;
        var lookupCB = goto_edit ? 'showRelatedObjectPopup' : 'showRelatedObjectLookupPopup';

        lookuplink.attr('id', lookuplink_id);
        lookuplink.attr('href', window.ADMIN_URL + MODEL_URL_ARRAY[val].app + "/" + MODEL_URL_ARRAY[val].model + '/' + admin_suffix + '?');
        lookuplink.attr('onClick', 'return ' + lookupCB + '(this);');
        return lookuplink;
    };
