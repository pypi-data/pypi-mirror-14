...
from clubhouse.core.models import BlockBase, BlockTypeBase
...
class ClubHouse(LazyAdminSite):
    ...
    def register(self, model_or_iterable, admin_class=None, **options):
        if not admin_class:
            ...
            for model in model_or_iterable:
                ...
                admin_class = BlockAdmin if issubclass(model,BlockBase) else None
                super(ClubHouse,self).register(model, admin_class, **options)
            return
        super(ClubHouse, self).register(model_or_iterable, admin_class, **options)
    ...
    def register_block_type(self,type_or_iterable,admin_class=None,**options):
        ...
        """
        if not admin_class:
            from clubhouse.core.options import BlockTypeAdmin
            admin_class = BlockTypeAdmin
        if isinstance(type_or_iterable,BlockTypeBase):
