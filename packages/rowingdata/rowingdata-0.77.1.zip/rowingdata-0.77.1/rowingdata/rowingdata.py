...
try:
    ...
    print "We don't know rowing physics"
    weknowphysics = 0
...
def phys_getpower(velo,rower,rigging):
    power = 0
    if (weknowphysics==1):
	res = rowingphysics.constantvelo(velo,rower,rigging,Fmax=600)
	force = res[0]
	power = res[3]
	ratio = res[2]
	result = [power,ratio,force]
    else:
	result = [0,0,0]
...
def setpower(df,nr_of_rows,powerstring,
	     favgstring,drivetimestring,
	     drivelengthstring = 'DriveLength (meters)',
	     pacestring=' Stroke500mPace (sec/500m)'):


    # creating a rower and rigging for now
    # in future this must come from rowingdata.rower and rowingdata.rigging
    r = rowingphysics.crew()
	...
	if (p != 0) & (spm != 0):
	    ...
	    print i,nr_of_rows,res[0],res[1],res[2],velo,spm
...
# need to select only trackpoints with Cadence, Distance, Time & HR data 
	...
	self.lat_values = self.root.xpath(self.selectionstring
					  +'/ns:Position/ns:LatitudeDegrees',
					  namespaces={'ns':namespace})

	self.long_values = self.root.xpath(self.selectionstring
...
#	ax6.set_yticks(np.arange(1.,2.,0.05))
	...
	yrange = y_axis_range(s,ultimate=[0,10])
...
#	ax6.set_yticks(np.arange(1.35,1.6,0.05))
	...
	s = np.concatenate((df.ix[:,' DriveTime (ms)'].values/1000.,
			   df.ix[:,' StrokeRecoveryTime (ms)'].values/1000.))
	yrange = y_axis_range(s,ultimate=[0,12])
