...
def interval_string(nr,totaldist,totaltime,avgpace,avgspm,
    ...
    stri = "{nr:0>2}{sep}{td:0>5.0f}{sep}{inttime:0>5}{sep}".format(
	nr = nr,
	sep = separator,
	td = totaldist,
    ...
    stri += "{tpace:0>7}{sep}{tspm:2.1f}{sep}{thr:3.1f}".format(
...
class summarydata:
	...
	for i in range(nr_rows):
	    ...
	    stri2 += "{tpace:0>7}{sep}{tspm:2.1f}{sep}{thr:3.1f}".format(
		tpace=tpace,
		sep=separator,
		tspm=tspm,
		thr = thr
...
class rower:
    def __init__(self,hrut2=142,hrut1=146,hrat=160,hrtr=167,hran=180,hrmax=192):
	self.ut2=hrut2
	self.ut1=hrut1
	self.at=hrat
	self.tr=hrtr
