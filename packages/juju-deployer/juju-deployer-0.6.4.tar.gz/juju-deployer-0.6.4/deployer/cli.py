...
def setup_parser():
    ...
    parser.add_argument(
        ...
        help=('Terminate all machines but the bootstrap node. '
              ...
              'Use -TT to forcefully terminate.'),
        dest="terminate_machines", action="count", default=0)
    parser.add_argument(
        '-t', '--timeout',
    ...
    parser.add_argument(
        '-n', '--no-relations',
        default=False, dest='no_relations', action='store_true',
        help=('Do not add relations to environment, just services/units '
              '(default: False)'))
...
def run():
    ...
    if options.destroy_services or options.terminate_machines:
        ...
        env.connect()
        env.reset(terminate_machines=bool(options.terminate_machines),
