...
@autobahn_sync.register('calendar.create')
...
@autobahn_sync.register('calendar.modify')
...
@autobahn_sync.register('calendar.meeting.list')
def meeting_list(calendar_id):
    return list_meetings(calendar_id)


@autobahn_sync.register('calendar.meeting.get')
def meeting_get(meeting_id):
    meeting = get_meeting(ObjectId(meeting_id))
    return treat_data(meeting)


@autobahn_sync.register('calendar.meeting.modify')
...
@autobahn_sync.register('calendar.meeting.delete')
...
def on_challenge(challenge):
    import json
    return json.dumps({'application_name': 'calendar', 'version': 0.1})
