...
class SensorServer(Sensor, SensorServerInterface):
    ...
    def __init__(self, settings=None):
        ...
        :type _mullticastBindPort: int """
        self._clients = {}
        """ Map of registered clients """
        self._key2deviceId = {}
        """ key (ip:port) -> deviceId lookup
        :type _key2deviceId: dict """
    ...
    def _do_packet(self, packet, ip, port):
        ...
        :type packet: T >= audience.si.app.message.APPMessage
    ...
    def _do_join_packet(self, packet, ip, port):
        ...
        :type packet: audience.si.app.message.APPJoinMessage
        ...
        try:
            ...
            self.person_new(people)
        except:
            self.exception("Failed to update people (person_new())")
            return
    ...
    def _do_unjoin_packet(self, packet, ip, port):
        ...
        try:
            self.person_leave(client['people'])
        except:
            self.exception("Failed to remove people (person_leave())")
            return
    ...
    def _do_update_packet(self, packet, ip, port):
        ...
        :type packet: audience.si.app.message.APPUpdateMessage
        :param ip: Client ip adress
        :type ip: str
        :param port: Client port
        :type port: int
        ...
        """
        self.debug("()")
        deviceId = packet.header.device_id
        if deviceId <= Id.SERVER:
            self.error("ProtocolViolation: Invalid device id")
            return
