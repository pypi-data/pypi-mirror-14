...
class Sensor(Loadable, StartStopable):
    ...
    def __init__(self, settings=None):
        ...
        """ Adress of multicast group """
        self._multicast_ip = settings.get(
            "multicast_bind_ip",
            self._get_local_ip()
        )
        """ Ip to sendmulticast messages """
    ...
    def _unpack(self, data):
        ...
        :raises audience.si.app.message.ProtocolViolation: Failed to decode
        """
        header, _ = APPHeader.unpack(data)
        cls = guess_class(header.message_type)
        if cls is None:
            self.warning(u"Unkown type {}\nHeader: {}".format(
