...
def init_logging(stdout_enabled=True):
    ...
    audit_rotating_file = ConcurrentRotatingFileHandler(filename=audit_log_file_name,
                                                        mode='a',
                                                        maxBytes=404857600,
                                                        backupCount=10)
    ...
    data_rotating_file = ConcurrentRotatingFileHandler(filename=data_log_file_name,
                                                       mode='a',
                                                       maxBytes=404857600,
                                                       backupCount=10)
    ...
    connections_rotating_file = ConcurrentRotatingFileHandler(filename=connections_log_file_name,
                                                              mode='a',
                                                              maxBytes=404857600,
                                                              backupCount=10)
    ...
    credentials_rotating_file = ConcurrentRotatingFileHandler(filename=credential_log_file_name,
                                                              mode='a',
                                                              maxBytes=404857600,
                                                              backupCount=10)
        ...
        try:
            ...
            while keep_going:
                ...
                try:
                    ...
                    q = UsergridQueryIterator(source_collection_url, page_delay=config.get('page_sleep_time'),
                                              sleep_time=30)
...
def migrate_out_graph_edge_type(app, collection_name, source_entity, edge_name):
    ...
    connection_query = UsergridQueryIterator(connection_query_url, sleep_time=30)
...
def migrate_in_graph_edge_type(app, collection_name, source_entity, edge_name):
    ...
    if collection_name in config.get('exclude_collection', []):
        connection_logger.info('Excluding (Collection) entity [%s / %s / %s]' % (app, collection_name, source_uuid))
        return True
...
def migrate_graph(app, collection_name, source_entity):
    ...
    key = 'visited:{uuid}'.format(uuid=source_uuid)
    ...
    if USE_CACHE:
        ...
        if date_visited is not None:
            ...
            connection_logger.info(
                    'VISITING [%s / %s / %s] at %s' % (
                        app, collection_name, source_uuid, str(datetime.datetime.utcnow())))

            cache.set(name=key, value=str(datetime.datetime.utcnow()), ex=config.get('visit_cache_ttl', 3600))
...
def get_best_source_entity(app, collection_name, source_entity):
    ...
    if r_get_source_entity.status_code == 200:
        ...
        q = UsergridQueryIterator(source_entity_query_url, sleep_time=30)
