...
#        "threadsavailable",
    ...
    def visit_FromCImportStatNode(self, node):
        if not node.relative_level and (
                node.module_name == "cython" or node.module_name.startswith("cython.")):
    ...
    def try_to_parse_directive(self, optname, args, kwds, pos):
        ...
        elif isinstance(directivetype, collections.Callable):
            if kwds is not None or len(args) != 1 or not isinstance(
                    args[0], (ExprNodes.StringNode, ExprNodes.UnicodeNode)):
                raise PostParseError(pos,
                    'The %s directive takes one compile-time string argument' % optname)
...
class TransformBuiltinMethods(EnvTransform):
    ...
    def _inject_locals(self, node, func_name):
        ...
        if func_name in ('locals', 'vars'):
            ...
            local_names = sorted(var.name for var in list(lenv.entries.values()) if var.name)
