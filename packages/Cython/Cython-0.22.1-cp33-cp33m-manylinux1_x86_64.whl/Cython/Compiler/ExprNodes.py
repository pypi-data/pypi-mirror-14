...
def check_negative_indices(*nodes):
    ...
    for node in nodes:
        if (node is None
                or not isinstance(node.constant_result, (int, float))):
            continue
...
class NewExprNode(AtomicExprNode):
    ...
    def infer_type(self, env):
        ...
        if constructor is None:
            ...
            type.scope.declare_cfunction('<init>', func_type, self.pos)
            constructor = type.scope.lookup('<init>')
...
class IndexNode(ExprNode):
            ...
            if base_type.is_pyobject:
                ...
                if base_type.is_ptr or base_type.is_array:
                    ...
                    if base_type.is_fused:
                        ...
                        self.type = base_type.specialize(dict(list(zip(base_type.templates, self.type_indices))))
    ...
    def parse_indexed_fused_cdef(self, env):
        ...
        fused_to_specific = dict(list(zip(fused_types, specific_types)))
    ...
    def generate_sequence_packing_code(self, code, target=None, plain=False):
        ...
        if self.mult_factor and not plain:
            ...
            if mult_factor.type.is_int:
                ...
                if isinstance(mult_factor.constant_result, int) \
...
class FuncLocalsExprNode(DictNode):
    def __init__(self, pos, env):
        local_vars = sorted([
            entry.name for entry in list(env.entries.values()) if entry.name])
...
class CmpNode(object):
    ...
    def calculate_cascaded_constant_result(self, operand1_result):
        ...
        if (isinstance(operand1_result, (bytes, str)) and
                isinstance(operand2_result, (bytes, str)) and
