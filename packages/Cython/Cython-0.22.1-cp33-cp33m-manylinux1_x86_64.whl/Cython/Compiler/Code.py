...
class UtilityCodeBase(object):
    ...
    def _add_utility(cls, utility, type, lines, begin_lineno, tags=None):
        ...
        if tags:
            all_tags = utility[2]
            for name, values in list(tags.items()):
                if KEYWORDS_MUST_BE_BYTES:
...
class GlobalState(object):
    ...
    def generate_string_constants(self):
        ...
        for c, cname in list(self.pyunicode_ptr_const_index.items()):
    ...
    def generate_num_constants(self):
        consts = [(c.py_type, c.value[0] == '-', len(c.value), c.value, c.value_code, c)
                  for c in list(self.num_const_index.values())]
        consts.sort()
        decls_writer = self.parts['decls']
        init_globals = self.parts['init_globals']
