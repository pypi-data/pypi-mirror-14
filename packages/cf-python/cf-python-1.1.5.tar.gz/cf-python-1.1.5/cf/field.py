...
#    def regridc(self, dst, src_axes=None, dst_axes=None,
#                method='conservative', use_dst_mask=False,
#                compute_field_mass=None, i=False):
#        '''
#
...
#
#By default the the regridding is a first-order conservative
#interpolation, but bilinear interpolation is available.
#
#**Metadata**
#
...
#input data array is masked. By default the mask of the destination
#grid is also taken into account. If the destination field data has
#more dimensions than the number of axes specified then the mask is
#taken from the 1-3 dimensionsal section of the data where the indices
#of all axes other than X and Y are zero.
#
#**Method**
#
#The interpolation is carried by out using the `ESMF` package - a
...
#        The field containing the new grid. If dst is a field list the
#        first field in the list is used. Alternatively a dictionary
#        of 1D dimension coordinates can be passed containing keywords
#        matching the names of the axes to be regridded.
#
#    src_axes : *optional*
#        Select dimension coordinates from the source field for
#        regridding. See cf.Field.axes for options for selecting
#        specific axes. This keyword may be None if dst is a
#        dictionary with keys corresponding to the axes. In this
#        case the keys will be sorted alphanumerically.
#
#    dst_axes : *optional*
#        Select dimension coordinates from the destination field for
#        regridding if a field is passed in. If this is none then the
#        value for src_axes is used. See cf.Field.axes for options
#        for selecting specific axes.
#
#    method : string, optional
...
#            dst_dict = True
#        
#        # Check src, dst, src_axes and dst_axes for consistency.
#        if src_axes is None:
#            if not dst_dict:
#                raise ValueError('If dst is a field src_axes must be specified')
#            else:
#                src_axes = sorted(dst.keys())
#        else:
#            set_src_axes = set(src_axes)
#            if len(set_src_axes) < len(src_axes):
#                raise ValueError('Duplicate axes in src_axes.')
#            if dst_dict and set_src_axes != set(dst.keys()):
#                raise ValueError('The keys of dst must correspond to src_axes.')
#        #--- End: if
#        if dst_axes is None:
#            dst_axes = src_axes
#        else:
#            if len(set(dst_axes)) < len(dst_axes):
#                raise ValueError('Duplicate axes in dst_axes.')
#            if len(dst_axes) != len(src_axes):
#                raise ValueError('There must be the same number of ' +
#                                 'destination axes as source axes.')
#        #--- End: if
#        n_regrid_dims = len(src_axes)
#        if n_regrid_dims > src.ndim:
#            raise ValueError('More axes than are present in src specified.')
#        if not dst_dict and n_regrid_dims > dst_dims:
#            raise ValueError('More axes than are present in dst specified.')
#        #--- End: if
#        if 1 < n_regrid_dims > 3:
#            raise ValueError('Between 1 and 3 axes may be regridded.')
#        #--- End: if
#        
#        # Retrieve the source field's dimension coordinates
#        src_dims = []
#        for axis in src_axes:
#            src_dims.append(self.dim(axes))
#        #--- End: for
#                
#        # Retrieve the destination field's dimension coordinates
#        dst_dims = []
#        for axis in dst_axes:
#            if dst_dict:
#                dst_dims.append(dst[axis])
#            else:
#                dst_dims.append(dst.dim(axes))
#        #--- End: for
#        
#        # Check src_dims and dst_dims for consistency of length
#        for i, dim in enumerate(src_dims):
#            if len(dim) != len(dst_dims[i]):
#                raise ValueError('Source and destination coordinates do not ' +
#                                 'have the same shape.')
#        #--- End: for
#        
#        # Slice the source data into sections of as many as possible
#        # dimensions for regridding. If the sections will not fit into
#        # memory well cut each section into further segments along the
#        # axes keeping the dimensions along which regridding will be
#        # performed intact.
#        # TO DO
#        
#        # Regrid each segment.
