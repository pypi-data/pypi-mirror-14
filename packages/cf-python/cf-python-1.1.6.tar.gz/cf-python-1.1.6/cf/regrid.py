...
:Parameters:
    ...
    method: string, optional
        ...
        is used. If it is set to 'nearest_stod' then nearest neighbor
...
:Returns:
    ...
    out: bool
        ...
        hasbounds = lon.hasbounds and lat.hasbounds and use_bounds
        if hasbounds:
            x_bounds = lon.bounds
        ...
        if use_bounds:
            ...
            for d in xrange(0, ndim):
                if coords[d].hasbounds and not coords[d].contiguous(overlap=False):
                    raise ValueError('Coordinate ' + str(d) +
                                     ' does not have contiguous bounds.')
                #--- End: if
