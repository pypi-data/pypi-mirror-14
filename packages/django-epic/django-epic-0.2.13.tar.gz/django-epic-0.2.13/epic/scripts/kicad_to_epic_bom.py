...
for comp in xml.find('components'):
    ...
    if do_not_place:
        print('%s: info: skipping refdes %s marked as do-not-place (DNP)' %
    ...
    if cmp_out and c.footprint is not None:
        print('\nBeginCmp\nReference = %s;\nValeurCmp = %s;\n'
              'IdModule  = %s;\nEndCmp' %
              (utf8(c.refdes), utf8(c.value), utf8(c.footprint)), file=cmp_out)
...
for components in bom_list:
    ...
    print('%s,%s,%s,%s,%s,%s,%s,%s' \
          % (utf8(qty), utf8(c.value), utf8(c.footprint),
             utf8(format_part_number(c.part_id)),
             utf8(c.mfg), utf8(c.mfg_pn),
             utf8(quote(substitutes)),
             utf8(quote(refdes))), file=out)
