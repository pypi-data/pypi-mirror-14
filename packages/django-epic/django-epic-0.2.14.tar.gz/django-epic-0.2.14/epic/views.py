...
class Part_Autocomplete(al.AutocompleteModelBase):
    ...
    def choice_value(self, choice):
        return '%d' % choice.id
al.register(Part, Part_Autocomplete)

class Order_Autocomplete(al.AutocompleteModelBase):
    order_by = '-id'
...
al.register(Order, Order_Autocomplete)

class Manufacturer_Autocomplete(al.AutocompleteListBase):
    attrs = {
...
al.register(Manufacturer_Autocomplete)

class Footprint_Autocomplete(al.AutocompleteListBase):
    attrs = {
...
al.register(Footprint_Autocomplete)
...
class SubstituteWidget(al.TextWidget):
...
class SubstituteField(CharField):
    ...
    def validate(self, value):
        ...
        if (instance is None or instance.id is None) and \
           not (len(slist) == 1 and slist[0] == ''):
            raise ValidationError('Substitutes may not be specified on new '
                                  'part.')

        equiv = None if instance is None else instance.equivalent_parts()
        for part_str in slist:
            if len(part_str) == 0:
                continue
            pn = int(part_str)
...
class Part_Edit_Form(al.ModelForm):
    ...
    def clean_substitute(self):
        ...
        if self.instance is not None and self.instance.id is not None:
            ...
            old_set = set(self.instance.equivalent_parts())
    ...
    class Meta:
        ...
        fields = [ 'mfg', 'mfg_pn', 'descr', 'val', 'footprint', 'mounting',
                   ...
                   'status', 'substitute', 'datasheet' ]
        autocomplete_names = { 'footprint': 'Part_Autocomplete' }
        widgets = {
            'mfg': al.TextWidget('Manufacturer_Autocomplete'),
            'footprint': al.TextWidget('Footprint_Autocomplete')
        }
...
def part_bom_edit(request, pk):
    ...
    widgets = {
        'comp': al.TextWidget('Part_Autocomplete'),
    }
...
class Inventory_Edit_Form(al.ModelForm):
...
def inventory_edit(request, pk, warehouse):
    ...
    widgets = {
        'part': al.TextWidget('Part_Autocomplete'),
    }
...
class Order_Edit_Form(al.ModelForm):
    def __init__(self, *args, **kwargs):
...
class Ship_Edit_Form(al.ModelForm):
    def __init__(self, fixed, *args, **kwargs):
    ...
    class Meta:
        ...
        widgets = {
            'ts': DateTimePicker(options={'format': 'YYYY-MM-DD'}),
            'ordr': al.TextWidget('Order_Autocomplete')
        }
...
class Part_Datasheet_Edit_Formset(BaseInlineFormSet):
    ...
    def initialize_virtual_fields(self, datasheet_id):
        # setup initial data for calculated (virtual) fields:
        for form in self:
            init = form.initial
            if 'id' in init:
                p = get_object_or_404(Part, pk=init['id'])
                init['mfg_pn'] = p.mfg + ' ' + p.mfg_pn
...
def datasheet_edit(request, pk, parts_list=None):
    ...
    widgets = {
        'id': al.TextWidget('Part_Autocomplete')
    }
    ...
    if pk is None:
        ...
        Part_Inline_Form_Set = inlineformset_factory(
            Datasheet, Part,
            fields=('id',), widgets=widgets,
            formset=Part_Datasheet_Edit_Formset, extra=3, can_delete=False)
    else:
        ...
        Part_Inline_Form_Set = inlineformset_factory(
            Datasheet, Part,
            fields=('id',), widgets=widgets,
            formset=Part_Datasheet_Edit_Formset, extra=3, can_delete=True)
    ...
    part_formset.initialize_virtual_fields(None)
