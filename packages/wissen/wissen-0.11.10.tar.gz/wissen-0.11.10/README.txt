...
- add NAIVEBAYES fast abort
- add learner.untrain ()
- remove learner.rbuild (), add learner.optimize ()
- add learner.rbuild ()
...
- F: selected features by GSS MAX
- NB: NAIVEBAYES, RO: ROCCHINO
...
Which do you choice? In my case, I choose F 5,000 with ROCCHINO cause of low unclassified ratio. But if speed was more important I might choice F 3,000 with NAIVEBAYES.
...
If you don't do optimize it make SIMILARITY and ROCCHINO classifiers inefficient (also it will be NOT influence to NAIVEBAYES, TFDIF, FEATUREVOTE classifiers). But you think it's more important retraining regulary rather than speed performance, you should not optimize.
...
.. code:: python  
  ...
  classifier.quess (
    qs, 
    lang = "un", 
    cl = [ NAIVEBAYES (Default) | FEATUREVOTE | ROCCHINO | TFIDF | SIMILARITY | META ],
    top = 0,
    cond = ""
  )
  classifier.close ()
