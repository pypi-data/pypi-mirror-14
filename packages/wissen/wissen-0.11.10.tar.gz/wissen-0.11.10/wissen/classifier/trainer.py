...
class Trainer (indexer.Indexer):
	...
	def build (self, dfmin = 0, dfmax = 0, commit = True, rebuild_index = False):
		...
		if hasattr (self.si.segments, "parameters"):
			train_temp = self.si.segments.parameters ["train"]
		else:
			train_temp = {}	
		self.writer.commit (self.corpus)		
		# sould be exactly here
		self.si.segments.parameters ["build"] = (dfmin, dfmax)		
		self.si.segments.parameters ["train"] = train_temp
		if rebuild_index:
	...
	def optimize (self):
		#print ("===============", self.si.segments.parameters ["build"])
