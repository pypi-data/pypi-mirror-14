...
static CHAR_T * nestedconsume (FastParserObject* self, CHAR_T* p, CHAR_T* end, CHAR_T* current_tag);
...
static CHAR_T *
nestedconsume (FastParserObject* self, CHAR_T* p, CHAR_T* end, CHAR_T* current_tag)
{
	PyObject* res;
	CHAR_T* t; 
	unsigned char ch = '\0';
		...
		if (token & TAG) {
			if (token == TAG_END) {
				...
				if (self->finish_starttag) {
					...
					if (strncmp ("script", b, len) == 0) {
						p = nestedconsume (self, b, end, "/script");
						if (!p) return -1;
					} else if (strncmp ("style", b, len) == 0) {
						p = nestedconsume (self, b, end, "/style");
						if (!p) return -1;
					}
