...
from . import metaclassifier, naivebayes
	...
	def choice_max (self, mis):
		...
		for f2 in f2s:
			...
			for f1 in self.terms:
				...
				t.append (R)			
			summi = sum (t)
			if self.choiceby == wissen.AVG:
				score = summi / len (t)
			elif self.choiceby == wissen.MIN:
				score = min (t)				
			else:
...
class Classifier (metaclassifier.Classifier, naivebayes.Classifier):
	def __init__ (self, reader, analyzer, featset, use_top_features, logger = None):
		self.clfs = {}
		self.subcl = wissen.FEATUREVOTE
		self.threshold = 1.0
		self.choiceby = wissen.AVG
		self.scoreby = wissen.IG
		self.scorer = None
		self.mis_cache = {}
		self.cached = 0
	...
	def setopt (self, **opts):	
		...
		self.scorer = getattr (_wissen, "%smn" % self.scoreby.upper ())
