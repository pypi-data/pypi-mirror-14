...
class Classifier (searcher.Searcher):
	...
	def do_refresh (self, *arg, **karg):
		...
		self.addClassifier (wissen.ROCCHINO)
		self.addClassifier (wissen.TFIDF)
		self.addClassifier (wissen.SIMILARITY)		
		self.addClassifier (wissen.MULTIPATH)
		self.addClassifier (wissen.META)		
		
		subclfs = dict ([(cl, obj) for cl, obj in self.clfs.items () if cl in (wissen.NAIVEBAYES, wissen.FEATUREVOTE, wissen.ROCCHIO)])
		self.clfs [wissen.MULTIPATH].setopt (clfs = subclfs)
		self.clfs [wissen.META].setopt (clfs = subclfs)
	...
	def do_quess (self, qs, lang = 'un', cl = "naivebayes", top = 0, cond = "", *arg, **karg):
		...
		if not classifier:
			return {"code": 504, "err": "No Classifier", "total": 0, "result": []}
			#return [501, 0, 0, 0]
