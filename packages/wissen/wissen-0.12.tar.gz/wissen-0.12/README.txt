...
.. code:: python
  ...
  def start_wissen (numthreads, logger):    
    ...
    searcher = col.get_searcher ()  
    wissen.assign ("data1", searcher)
    
    analyzer = wissen.standard_analyzer (max_term = 1000, stem = 2)
    col = wissen.model ("./data2", wissen.READ, analyzer)
    classifier = col.get_classifier ()  
    wissen.assign ("data2", classifier)
...
Mounting Wissen On Skitai-Saddle
---------------------------------

And here's an example for `Skitai-Saddle`_ app.
...
.. code:: python
  ...
  @app.startup	
  def startup (wasc):
    config.start_wissen (wasc.numthreads, wasc.logger.get ("app"))
    wasc.register ("wissen", wissen)
  
  @app.route('/query')
  def query (was, alias, qs, offset = 0, fetch = 10, sort = "", summary = 30, lang = "en"):
    return was.wissen.query (alias, qs, offset, fetch, sort, summary, lang)
  
  @app.route('/guess')
  def quess (was, alias, qs, lang = "en", top = 0, cond = ""):
    return was.wissen.quess (alias, qs, lang, wissen.NAIVEBAYES, top, cond)
  
  @app.route('/delete')
  def delete (was, alias, qs):
    return was.wissen.delete (alias, qs)
      
  @app.route('/refresh')
  def refresh (was, alias):
    return was.wissen.refresh (alias)
  
  @app.route('/close')
  def close (was, alias):
    return was.wissen.close (alias)


Also note you need not handle wissen shutdown, because Skitai will automatically call wissen.cleanup () if you registered wissen by wasc.register ().
...
============
  ...
  0.12 - add wissen.assign(alias, searcher/classifier) and query(alias), guess(alias)
