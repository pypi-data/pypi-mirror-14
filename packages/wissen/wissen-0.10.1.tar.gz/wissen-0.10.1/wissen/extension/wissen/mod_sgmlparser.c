...
static CHAR_T * skipltag (FastParserObject* self, CHAR_T* p, CHAR_T* end, CHAR_T* ttag);
...
static CHAR_T *
skipltag (FastParserObject* self, CHAR_T* p, CHAR_T* end, CHAR_T* ttag)
{
	unsigned char ch = '\0';
	char tag [30];
	int tagindex=0;
	...
	while (p < end) {
		...
		if (ch == '/') { /* maybe comments */
			...
			tag [tagindex] = '\0';
			if (strcmp ((const char*)tag, (const char*)ttag) == 0) {
	...
	while (p < end) {
	  ...
	  eot: /* end of token */
		...
		if (q != s && self->handle_data) {
			...
			if (len) {
//#if PY_MAJOR_VERSION >= 3
//				res = PyObject_CallFunction(self->handle_data, "O", PyUnicode_DecodeUTF8 (self->databuffer, len, "Can't decode data"));				
//#else
		...
		if (token & TAG) {
			if (token == TAG_END) {
				...
				if (self->finish_starttag) {
					...
					tag [ ++len] = '\0';
					if (strncmp ("/script", tag, len) == 0 || strncmp ("/style", tag, len) == 0 || strncmp ("/object", tag, len) == 0) {
						//printf ("%s\n%s\n----------------------", b, end) ;
						p = skipltag(self, b, end, (CHAR_T*)tag);						
					}

					if (strncmp ("/p", tag, len) == 0 ||
					     strncmp ("/br", tag, len) == 0 ||
					     strncmp ("/td", tag, len) == 0 ||
					     strncmp ("/table", tag, len) == 0 ||
					     strncmp ("/div", tag, len) == 0
					) {
						self->addspace = 1;
					}
