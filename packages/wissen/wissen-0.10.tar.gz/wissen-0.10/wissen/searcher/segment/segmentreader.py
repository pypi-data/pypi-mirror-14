...
class SegmentReader (segment.Segment):
	def __init__ (self, si, seg):
		self.si = si
		segment.Segment.__init__ (self, si.fs.get (seg), seg, 'r', self.si.plock)
	...
	def getDocument (self, mem, docId, summary = 0, terms = None):
		if terms:
			return self.fd.read (mem, docId, summary, terms)
		else:
			return self.fd.read (mem, docId, summary)	
		
	def getTermInfo (self, mem, term, fdno):
		return self.ti.get (mem, term [:100], fdno)
	
	def getTermInfos (self, mem, term, fdno):
		return self.ti.getwildcard (mem, term [:100], fdno)		
...
class SegmentBulkReader (SegmentReader):
	def __init__ (self, si, seg):
		self.si = si
		segment.Segment.__init__ (self, si.fs.get (seg), seg, 'm')
