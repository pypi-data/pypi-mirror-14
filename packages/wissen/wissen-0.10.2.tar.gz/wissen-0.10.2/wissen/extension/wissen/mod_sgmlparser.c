...
static CHAR_T * skipltag (FastParserObject* self, CHAR_T* p, CHAR_T* end);
...
generalize (FastParserObject* self, char* temp, int limit)
{
	...
	for (i = 0; i < limit; i++) {
		...
		else if (i==0 && self->addspace && !isspace (ch) &&  !isspace (self->lch)) {
			self->databuffer [self->databufferpos++] = ' ';
		}
...
static CHAR_T *
skipltag (FastParserObject* self, CHAR_T* p, CHAR_T* end)
{
	unsigned char ch = '\0';
	...
	while (p < end) {
		...
		if (!end_starttag) {
			if (ch == '>') end_starttag = 1; // find end of start tag
			continue;
		}
		...
		if (ch == '/') { /* maybe comments */
			...
			tag [tagindex] = '\0';
			if (strcmp ((const char*)tag, (const char*)self->current_tag) == 0) {
...
_sgmlop_new(int xml)
{
	...
	self->resizefactor = 1;
	self->current_taglen = 1024;
	self->current_tag = malloc (self->current_taglen);
	self->current_tag [0] = '/';
	self->lch = ' ';
	self->addspace = 0;
		...
		if (token & TAG) {
			if (token == TAG_END) {
				...
				if (self->finish_starttag) {
					...
					if (self->current_taglen < (len + 2)) {
						self->current_taglen = len + 2;
						self->current_tag = realloc (self->current_tag, self->current_taglen);
						self->current_tag [0] = '/';
					}
					
					for (i=1; i<len + 1; i++) {
						self->current_tag [i] = tolower (*temp++);
					}										
					self->current_tag [ ++len] = '\0';
					if (strncmp ("/script", self->current_tag, len) == 0 || strncmp ("/style", self->current_tag, len) == 0 || strncmp ("/object", self->current_tag, len) == 0) {
						//printf ("%s\n%s\n----------------------", b, end) ;
						p = skipltag(self, b, end);
					}

					if (strncmp ("/p", self->current_tag, len) == 0 ||
					     strncmp ("/br", self->current_tag, len) == 0 ||
					     strncmp ("/td", self->current_tag, len) == 0 ||
					     strncmp ("/table", self->current_tag, len) == 0 ||
					     strncmp ("/div", self->current_tag, len) == 0
					) {
						self->addspace = 1;
					}
