...
class QueryStringError (Exception): pass
...
class Query:
	...
	def __init__ (self, segments, qs, lang, offset, fetch, sort, analyzer, estTotal = False):
		...
		self.sort = sort
		self.analyzer = analyzer
		self.terminfos = TermInfos (self.segments, self.analyzer, self.sort == "tfidf")
		...
		self.hits = _wissen.Compute (memory.get (), self.sort == "tfidf" and self.segments.si.getN () or 0)		
		self.totalcount = 0
		self.duration = 0
		self.esegments = []
		self.bucket = []
	...
	def setSort (self):
		...
		if self.sort == "tfidf" and self.terminfos.isScorable ():
			...
			if typeinfo.typemap.isint (type) and len (self.segments):
				sp = self.segments.readers [0].getSortMapPointer (num) # checking sortmap
				if sp:
	...
	def query (self):
		...
		try:
			...
			raise
			raise QueryStringError("Error in %s" % self.qs)
