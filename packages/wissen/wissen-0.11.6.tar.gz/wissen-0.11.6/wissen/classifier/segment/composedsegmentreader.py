...
from wissen.searcher import searcher
...
class ComposedSegmentReader:
	...
	def load (self):
		...
		self.reader = segment.Segment (self.si.fs.get (seg), seg, "r")
		self.reader.open ()
		...
		col = wissen.collection (os.path.join (self.si.indexdir, "%s.ts" % self.si.alias), wissen.READ, self.si.analyzer, self.si.logger)		
	...
	def getFeatures (self, mem, terms, numvoca, top = 0):
		termsfv = []
		if top:
			classifier = _wissen.Classifier (mem, self.corpus, numvoca, self.getN())
