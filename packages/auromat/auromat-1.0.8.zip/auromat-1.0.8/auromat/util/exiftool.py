...
# NOTE:
# Maik Riechert: added 'nums' argument to use/not use -n flag
# if nums=False, then each tag can be suffixed with '#' to apply -n individually
# (2014-01-21)
# Maik Riechert: added 'copy_tags(from,to)' to copy EXIF tags from one file to another
...
class ExifTool(object):
    ...
    def __init__(self, nums=True, executable_=None):
        if executable_ is None:
            self.executable = executable
        else:
            self.executable = executable_
        self.running = False
        self.nums = ['-n'] if nums else []
