...
class ApiApi(object):
    ...
    def nodes_post_obm_id_by_id(self, identifier, content, **kwargs):
        """
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.nodes_post_obm_id_by_id(identifier, content, callback=callback_function)
        ...
        # verify the required parameter 'content' is set
        if ('content' not in params) or (params['content'] is None):
            raise ValueError("Missing the required parameter `content` when calling `nodes_post_obm_id_by_id`")
