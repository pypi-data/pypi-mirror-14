...
class VirtualService(HttpClient, KempBaseObjectModel):
    ...
    def save(self, update=False):
        if not update:
            if self._is_sub_vs:
                ...
                #data = get_data(response)
                #subvs = VirtualService(self.access_info, created_subvs_id[0])
                #subvs.populate_default_attributes(data)
                #self.subvs_entries.append(subvs)
            else:
    ...
    def create_sub_virtual_service(self):
        """Create a virtual service marked as subvs
