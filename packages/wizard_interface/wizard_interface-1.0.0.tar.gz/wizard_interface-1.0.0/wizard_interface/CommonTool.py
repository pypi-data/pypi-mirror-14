...
class LogTool(logging.Logger):
    ...
    def set_logger_formatter(self, format_str=None):
        fmt_str = "%(asctime)s [%(filename)s line %(lineno)d] [%(name)s %(levelname)s] %(message)s"
...
class MD5Tool():
    ...
    def __init__(self, logger: logging.Logger):
        ...
        self.logger.info("create md5 tool")

    def MD5_str(self, src_str: str, encoding="utf-8"):
        try:
            self.logger.info("md5 str start: %s" % src_str)
            m5 = hashlib.md5()
            m5.update(src_str.encode(encoding))
            res = m5.hexdigest()
            self.logger.info("md5 str success %s" % res)
            return res
    ...
    def MD5_dict(self, src_dict: dict, encoding="utf-8", safe_key: str=None):
        try:
            self.logger.info("md5 dict start: %s" % src_dict)
            ...
            self.logger.info("md5 dict success: %s" % res)
            return res
        except Exception as error:
            self.logger.error(error)
...
class XMLDictTool():
    ...
    def __init__(self, logger: logging.Logger):
        self.logger = logger
        self.logger.info("create XMLDictTool")

    def dict_to_xml(self, src_dict: dict):
        try:
            self.logger.info("convert dict to xml start: %s" % src_dict)
    ...
    def xml_to_dict(self, src_xml: str):
        try:
            self.logger.info("convert xml to dict start: %s" % src_xml)
            res = xmltodict.parse(src_xml)
            self.logger.info("convert xml to dict success: %s" % res)
            return res
        except Exception as error:
            self.logger.error(error)
...
class GzipperTool():
    ...
    def __init__(self, logger: logging.Logger):
        self.logger = logger
        self.logger.info("create Gzipper")

    def unzip(self, src_data: str):
        try:
            self.logger.info("unzip start: %s" % src_data)
            res = gzip.decompress(src_data)
            self.logger.info("unzip success: %s" % res)
            return res
        except Exception as error:
            self.logger.error(error)
    ...
    def zip(self, src_data: str, encoding="utf-8", zip_level: int=9):
        try:
            self.logger.info("zip start: %s" % src_data)
            src = src_data.encode(encoding)
            res = gzip.compress(src, zip_level)
            self.logger.info("zip success: %s" % res)
            return res
        except Exception as error:
            self.logger.error(error)
