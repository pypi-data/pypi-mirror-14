...
def get_venv_full_package_path(pkg_path):
    # if we pass the optional path argument here we can potentially have a scheme where
    # envy can be globally installed.
...
def sync(args):
    ...
    try:
        # need to build up path here so packages can be sync'd from anywhere (if $DEV_SRC is provided)
