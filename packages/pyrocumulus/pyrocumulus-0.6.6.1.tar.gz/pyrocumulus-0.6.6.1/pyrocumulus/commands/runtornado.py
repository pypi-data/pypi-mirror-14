...
class RunTornadoCommand(BaseCommand):
    ...
    def run(self):
        ...
        # here things get tricky. if application is an instance of
        # tornado.web.Application it can't be yielded. It it's an instance
        # of pyrocumulus.web.applications.Application it can be.

        @gen.coroutine
        def listen():  # pragma: no coverage
            try:
                yield self.application.listen(self.port)
            except gen.BadYieldError:
                pass

        ioloop_inst.run_sync(listen)
        ioloop_inst.start()
