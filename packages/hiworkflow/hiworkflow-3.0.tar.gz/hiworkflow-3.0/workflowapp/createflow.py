...
class BuildWorkflow(object):
    ...
    def get_open_task_list(self):
        """
        Return a list of all open tasks (is_active = True) for a workflow.
        """
        workflow_id = Workflow.objects.filter(id = self.workflow_id)
    ...
    def get_closed_task_list(self):
        """
        Return a list of all closed tasks (is_active = False) for a workflow.
        """
        workflow_id = Workflow.objects.filter(id = self.workflow_id)
    ...
    def visualize(self):
        """
        Return states and transition of a workflow to display it graphically.
        """
...
def visualize_workflow(approval_workflow):
    ...
    Using Joint JS library to represent a workflow.
    "visualize.html" file is used to display the workflow.
    Joint JS library files are included in static folder.
    "result" contains all the states and transitions in a workflow
    """
