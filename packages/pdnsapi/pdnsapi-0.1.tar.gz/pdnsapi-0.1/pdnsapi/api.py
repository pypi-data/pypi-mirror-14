...
class Record(object):
    ...
    def __init__(self, name, type_, content, ttl=default_ttl, disabled=False):
        self.name = name
        self.type = type_
...
class Zone(Resource):
    ...
    def get_records(self, name=None, type_=None, content=None):
        pass
    ...
    def update_record(self, current_name, current_type, new_record):
        ...
        updated_zone = Resource.perform_request(patch, self.url,
                                                data=json.dumps(data))

        self.__update_records(updated_zone['records'])
    ...
    def __format_record_data(change_type,
                             ...
                             current_name=None,
                             current_type=None):
        if current_name is None and current_type is None:
            current_name = record.name
            current_type = record.type
        records = None

        if change_type is not "DELETE":
            records = [{
    ...
    def __update_records(self, records):
        self.__record_list = [Record(record['name'], record['type'],
...
class RootResource(Resource):
    ...
    def create_zone(self, name, name_servers):
        """
        ...
        return Zone(zone_data['name'], zone_data['records'], zone_data['url'])
    ...
    def delete_zone(self, zone_name):
        ...
        :return:
        """
        self.perform_request(delete, '{0}/{1}'.format(self.zones_url,
                                                      zone_name))
...
def init_api(url, server_id, key):
    ...
    return RootResource("{0}/servers/{1}/zones".format(url, server_id))
