...
def Prokka_Commandline( Contig,Genius,Spieces,Strain,Center,Prefix,OutPut,Plasmid,Evalue ):
	raw_commandline = "nohup "  +config_hash["Tools"]["prokka"]+ ' --rfam  --prefix %(Prefix)s --outdir %(output)s --evalue %(e_value)s  --genus %(Genius)s --strain %(Strain)s  --cpus 64 --compliant --force --quiet --centre %(Centre)s   --locustag %(Prefix)s'%(
	    {
	        "Centre":Center,
	        "Prefix":Prefix,
	        "output":OutPut,
