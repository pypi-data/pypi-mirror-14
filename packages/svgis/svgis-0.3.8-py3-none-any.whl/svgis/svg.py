...
def _element(tag, **kwargs):
    return '<{}{}/>'.format(tag, toattribs(**kwargs))
...
def circle(point, precision=None, **kwargs):
    ...
    Returns:
        str
    '''
    return _element('circle', cx=_round(point[0], precision), cy=_round(point[1], precision), **kwargs)
...
def text(string, start, precision=None, **kwargs):
    ...
    return _wrap('text', string, x=start[0], y=start[1], **kwargs)
...
def polyline(coordinates, precision=None, **kwargs):
    ...
    '''
    fmt = _fmtcoord(precision)
    points = ' '.join(fmt.format(c) for c in coordinates)
    return _element('polyline', points=points, **kwargs)
...
def polygon(coordinates, precision=None, **kwargs):
    ...
    fmt = _fmtcoord(precision)
    points = ' '.join(fmt.format(c) for c in coordinates)
    return _element('polygon', points=points, **kwargs)


def toattribs(**kwargs):
    attribs = ' '.join('{}="{}"'.format(k, v) for k, v in list(kwargs.items()) if v is not None)
...
def group(members=None, **kwargs):
    ...
    '''
    members = members or ''
    return _wrap('g', ''.join(members), **kwargs)
...
def drawing(size, members, viewbox=None, style=None):
    ...
    return _wrap('svg', contents, **kwargs)
