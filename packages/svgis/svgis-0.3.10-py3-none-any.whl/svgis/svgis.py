...
class SVGIS(object):
    ...
    def _prepare_layer(self, layer, filename, bounds, scalar, **kwargs):
        ...
        result['id_field'] = id_field if id_field in list(layer.schema['properties'].keys()) else None
    ...
    def _compose_file(self, filename, unprojected_bounds=None, **kwargs):
        ...
        return {
            'members': group,
            'id': kwargs['layer'],
            'class': ' '.join(_style.sanitize(c) for c in list(layer.schema['properties'].keys()))
        }
