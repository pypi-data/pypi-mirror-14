...
class DirProc(FsTaskProcessor):
    ...
    defargs={'path': '', 'md5': '', 'sha1': '', 'sha256': '', 'mindepth': '0', 'maxdepth': '10'}
...
class DFProc(FsTaskProcessor):
    ...
    def run(self,ts,data,args):
        ...
        if args['path']:
            ...
            dflist=[]            
            points = [p.mountpoint for p in psutil.disk_partitions()]
    ...
    def run(self,ts,data,args):
        ...
        if not '*' in self.tpconf['read']:
            if not path in self.tpconf['read']:
                ...
                ts.oc.log.error("File {} is not allowed to read. Whitelist read files: {}. Maybe try: {} ?".format(path,self.tpconf['read'], tpconfline))
                ts.stop()
                return None
        ...
        try:                    
            ...
            ts.oc.log.error('Cannot read file \'{}\': {}'.format(path, str(e)))
            ts.stop()
            return None
