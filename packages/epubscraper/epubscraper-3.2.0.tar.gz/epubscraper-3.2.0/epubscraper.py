...
def printer(dfile, path, blogdir, postdir, link, imagedir, overwrite):
    """
...
def postgen(path, outdir, overwrite=False, ps=8):
    ...
    partial_printer = partial(printer, path=path, blogdir=outdir, postdir=postdir, link=link, imagedir=imagedir, overwrite=overwrite)
    ...
    for p, subdirs, files in os.walk(postdir):
        for f in [f for f in files if f.endswith(".markdown")]:
            posts.append(os.path.join(p, f))
    keyf = lambda text: text.split("/")[-1][11:]
    posts = [list(items) for gr, items in groupby(posts, key=keyf)]
    for listi in posts:
        if len(listi) > 1:
            for i in listi[-1]:
                os.remove(i)
