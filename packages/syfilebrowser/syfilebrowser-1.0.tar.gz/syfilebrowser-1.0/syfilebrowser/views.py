...
from syfilebrowser.functions import (get_path, get_breadcrumbs,
...
mixin_class_name = "syfilebrowser.storage.%sMixin" % storage_class_name

# Workaround for django-s3-folder-storage
if django_settings.DEFAULT_FILE_STORAGE == 's3_folder_storage.s3.DefaultStorage':
    mixin_class_name = 'syfilebrowser.storage.S3BotoStorageMixin'
...
def browse(request):
    ...
    return render_to_response('syfilebrowser/index.html', {
        'dir': path,
        'p': p,
        'page': page,
...
# mkdir signals
syfilebrowser_pre_createdir = Signal(providing_args=["path", "dirname"])
syfilebrowser_post_createdir = Signal(providing_args=["path", "dirname"])
...
def mkdir(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            try:
                # PRE CREATE SIGNAL
                syfilebrowser_pre_createdir.send(sender=request, path=path, dirname=form.cleaned_data['dir_name'])
                # CREATE FOLDER
                default_storage.makedirs(server_path)
                # POST CREATE SIGNAL
                syfilebrowser_post_createdir.send(sender=request, path=path, dirname=form.cleaned_data['dir_name'])
                # MESSAGE & REDIRECT
    ...
    return render_to_response('syfilebrowser/makedir.html', {
        'form': form,
        'query': query,
...
def upload(request):
    ...
    return render_to_response('syfilebrowser/upload.html', {
        'query': query,
...
# upload signals
syfilebrowser_pre_upload = Signal(providing_args=["path", "file"])
syfilebrowser_post_upload = Signal(providing_args=["path", "file"])
...
def _upload_file(request):
    ...
    if request.method == 'POST':
        ...
        if request.FILES:
            ...
            # PRE UPLOAD SIGNAL
            syfilebrowser_pre_upload.send(sender=request, path=request.POST.get('folder'), file=filedata)
            ...
            # POST UPLOAD SIGNAL
            syfilebrowser_post_upload.send(sender=request, path=request.POST.get('folder'), file=FileObject(smart_text(file_path)))
...
# delete signals
syfilebrowser_pre_delete = Signal(providing_args=["path", "filename"])
syfilebrowser_post_delete = Signal(providing_args=["path", "filename"])
...
# rename signals
syfilebrowser_pre_rename = Signal(providing_args=["path", "filename", "new_filename"])
syfilebrowser_post_rename = Signal(providing_args=["path", "filename", "new_filename"])
...
def rename(request):
    ...
    return render_to_response('syfilebrowser/rename.html', {
        'form': form,
        'query': query,
