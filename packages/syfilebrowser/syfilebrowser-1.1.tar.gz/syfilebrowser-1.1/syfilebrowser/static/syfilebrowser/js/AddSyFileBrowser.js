var SyFileBrowser = {
    ...
    // change this
    thumb_prefix: 'thumb_',
    no_thumb: 'syfilebrowser/img/no_thumb.gif',
    ...
    init: function() {
        ...
        for (var i=0; i<scripts.length; i++) {
            if (scripts[i].src.match(/AddSyFileBrowser/)) {
                var idx = scripts[i].src.indexOf('syfilebrowser/js/AddSyFileBrowser');
                SyFileBrowser.admin_media_prefix = scripts[i].src.substring(0, idx);
                break;
            }
        }
    },
...
addEvent(window, 'load', SyFileBrowser.init);
