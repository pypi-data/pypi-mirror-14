...
class UsergridQueryIterator(object):
    ...
    def _get_next_response(self, attempts=0):
        ...
        try:
            ...
            if r.status_code == 200:
                ...
                if attempts < 10:
                    self.logger.error('URL=[%s] code=[%s], response: %s' % (target_url, r.status_code, r.text))
                    self.logger.error('Sleeping %s after HTTP [%s] for retry attempt=[%s]' % (
    ...
    def next(self):
        ...
        if self.last_response is None:
            ...
            self._process_next_page()
            self.logger.info('Sleeping [%s]s between pages' % self.page_delay)
