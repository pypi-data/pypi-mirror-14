...
class Request(object):
    ...
    def __new__(cls,
                environ,
                parse_content_type=re.compile(
                    r'\s*[^\s;]*;\s*charset=([\w\d\-]+)').match):
        if 'fresco.request' in environ:
            return environ['fresco.request']

        request = object.__new__(cls)
        request.environ = environ
        environ['fresco.request'] = request
        if 'CONTENT_TYPE' in environ:
            match = parse_content_type(environ['CONTENT_TYPE'])
            if match:
