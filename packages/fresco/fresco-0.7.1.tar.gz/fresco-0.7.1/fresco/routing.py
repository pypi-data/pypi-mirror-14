...
class RouteCollection(MutableSequence):
    ...
    def __init__(self, routes=None, route_class=None, cache=True):
        ...
        if cache:
            self._route_cache = LRUCache(self.min_route_cache_size)
    ...
    def resize_route_cache(self):
        self._route_cache.max_size = max(
    ...
    def get_routes(self, path, method, request=None):
        ...
        """
        use_cache = self._route_cache is not None
        matching_routes = None
        if use_cache:
            try:
                matching_routes = self._route_cache[(method, path)]
            except KeyError:
                pass

        if matching_routes is None:
            matching_routes = ((r, r.match(path, method))
                               for r in self.__routes__)
            matching_routes = ((r, t) for (r, t) in matching_routes
                               if t is not None)
            if use_cache:
                matching_routes = \
                        self._route_cache[method, path] = \
                        list(matching_routes)

        for route, result in matching_routes:
    ...
    def route_wsgi(self, path, wsgiapp, rewrite_script_name=True,
        ...
        :param preserve_script_name: if ``True`` (the default),
