...
class Match:
	...
	# def compute_sim_and_dif(self):
	# 	best_1 = []
	# 	best_2 = []

	# 	for n1 in self.from_nodes:
	# 		nodes = [self.edges[(n1,n2)] for n2 in self.to_nodes]
	# 		nodes.sort(reverse=True)
	# 		best_1.append(nodes[0])
	# 		best_2.append(nodes[1]) # note that this will only work for K>=2 

	# 	best_1 = np.array(best_1)
	# 	best_2 = np.array(best_2)

	# 	self.sim = np.mean(best_1)
	# 	self.dif = np.mean(best_1-best_2)
