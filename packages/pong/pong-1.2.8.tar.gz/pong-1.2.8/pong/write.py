		...
		for run1 in cluster_matches:
			...
			for run2 in cluster_matches[run1]:
				...
				for i in match.from_nodes:
					...
					# Maybe: we could allow the user to choose the number of best matches to output, otherwise print all
	...
	for kgroup in all_kgroups:
		...
		for run in kgroup.rep_runs:
			...
			if len(runs[run].sim_runs)>0:
				...
				#___________sim_within uses the similarity within the "best" perm (even if it's not valid) for each cluster
...
def write_json(pong, as_file=False):
	...
	for kgroup in all_kgroups:
		...
		for run in kgroup.rep_runs:
			...
			sim_within = [] #starts with 1 because if this rep run is the only one in the mode then similarity is 1 -> is this a correct idea?
			if len(runs[run].sim_runs)>0:
				sim_runs_id = sorted([run] + runs[run].sim_runs)
				for x in sim_runs_id:
					for y in sim_runs_id[sim_runs_id.index(x)+1:]:
