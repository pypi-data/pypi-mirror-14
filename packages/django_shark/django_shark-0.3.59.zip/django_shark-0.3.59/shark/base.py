...
class Renderer:
    def __init__(self, handler=None, indent=0, amp=False, inline_style_class_base='style_'):
        self._html = []
        self._css = []
        self._css_classes = {}
        self._js = []
    ...
    def append_resource(self, url, type, module, name=''):
        self._resources.append(Resource(url, type, module, name))
    ...
    def css(self):
        for style, class_name in self._css_classes.items():
            self.append_css('.' + class_name + '{' + style + '}')
...
class Collection(list):
    ...
    def render(self, handler=None, indent=0):
        renderer = Renderer(handler=handler, indent=indent)
