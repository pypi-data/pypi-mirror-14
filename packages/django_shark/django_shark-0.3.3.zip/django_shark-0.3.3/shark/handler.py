            ...
            if isinstance(pattern, RegexURLResolver):
                ...
                if 'handler' in pattern.default_args and issubclass(pattern.default_args['handler'], BaseHandler):
                    ...
                    if handler not in handlers:
                        ...
                        for args in handler.get_sitemap():
...
def shark_urls(modules, page_handler=None, use_static_pages=True, static_amp=False):
    handlers = []
    def add_handler(obj):
        if inspect.isclass(obj) and issubclass(obj, BaseHandler) and 'route' in dir(obj):
            if obj.enable_amp:
                handlers.append(url(obj.make_amp_route(obj.route), shark_django_amp_handler, {'handler':obj}, name=obj.get_unique_name() + '.amp'))
            if obj.route:
                handlers.append(url(obj.route, shark_django_handler, {'handler':obj}, name=obj.get_unique_name()))
    ...
    if page_handler and use_static_pages:
        StaticPage.enable_amp = static_amp
