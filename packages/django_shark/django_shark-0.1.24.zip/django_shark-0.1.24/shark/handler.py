...
class StaticPageHandler(BasePageHandler):
    def render_page(self, url_name):
        self.append('Hello')


def shark_urls(modules, page_handler=None, use_static_pages=True):
    handlers = []
    ...
    if page_handler and use_static_pages:
        static_page_handler = new_class('_StaticPageHandler', (StaticPageHandler, page_handler))
        handlers.append(url('^page/(.*)$', shark_django_handler, {'handler': static_page_handler}, name=static_page_handler.get_unique_name()))
