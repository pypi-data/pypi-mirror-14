...
class GatewayCostElectricalConsumption(GatewayBase):
    ...
    def __init__(self, endpoint, token, http_management=None):
        super(GatewayCostElectricalConsumption, self).__init__(endpoint, token, http_management=http_management)

    def get(self, id, call="cost/electrical/consumption"):
        raise Exception("Method get not allowed by readings, just go for fetch")
    def _get_nice_timestamps(self, date):
        return date.strftime("%Y-%m-%dT%H:%M:%S")

    def _basic_fetch(self, parameters, call, list_consumption):
        params = deepcopy(parameters)
        params["from"] = self._get_nice_timestamps(params["from"])
        params["to"] = self._get_nice_timestamps(params["to"])
        new_consumption = super(GatewayCostElectricalConsumption, self).fetch(params, call)
    ...
    def _loop_fetch(self, parameters, call, interval):
        ...
        while parameters["from"] + interval < parameters["to"]:
            list_consumption = self._basic_fetch(parameters, call, list_consumption)
            parameters["from"] += interval
        return self._basic_fetch(parameters, call, list_consumption)
    ...
    def fetch(self, parameters=None, call="cost/electrical/consumption"):
        ...
        if parameters["resolution"] in ["QH", "HH", "H"]: return self._fetch_year(parameters, call)
