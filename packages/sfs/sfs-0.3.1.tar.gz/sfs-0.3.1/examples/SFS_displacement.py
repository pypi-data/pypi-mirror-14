...
#grid = sfs.util.xyz_grid([-2, 2], [-2, 2], 0, spacing=0.04)
grid = [np.random.uniform(-2, 2, 10000), np.random.uniform(-2, 2, 10000), 0]
...
v = sfs.mono.synthesized.generic(omega, x0, n0, d * twin * a0 , grid,
                                 source=sfs.mono.source.point_velocity)
...
plt.cla()
sfs.plot.particles(X, facecolor='black', s=2, trim=[-3, 3, -3, 3])
plt.axis('off')
#plt.title(title)
plt.grid()
