...
deltan = 0.99  # absorption factor of walls
n0 = [1, 0, 0]  # normal vector of source (only for compatibilty)
N = 20  # maximum order of modes
#N = [1, 1, 1]  # room mode to compute

fresponse = False  # freqeuency response or sound field?
...
if not fresponse:
    ...
    #grid = sfs.util.xyz_grid([0, L[0]], 0, 0, spacing=.1)

    p = sfs.mono.source.point_modal(omega, x0, n0, grid, L, N=N, deltan=deltan)
    vx, vy, vz = sfs.mono.source.point_modal_velocity(omega, x0, n0, grid, L, N=N, deltan=deltan)
