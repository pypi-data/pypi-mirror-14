                ...
                for url_postfix, http_methods, name in meth.url_rule_cache:
                    ...
                    def wrapper(f):
                        ...
                        def proxy_meth(*meth_args, **meth_kwargs):
                            response = f(view_cls, *meth_args, **meth_kwargs)
                            return response
...
class SleepyView(BaseSleepyView):
    ...
    def __init__(self, model, session, schema, fields, **kwargs):
    ...
    def as_view(cls, name, *class_args, **class_kwargs):
        fields = class_kwargs.pop("fields", cls.fields)
        schema = class_kwargs.pop("schema", cls.schema)
        model = class_kwargs.pop("model", cls.model)
        return super().as_view(name, *class_args, model=model, schema=schema, fields=fields, **class_kwargs)
