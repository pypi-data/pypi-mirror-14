...
class ScoredSent2Vec(utils.SaveLoad):
    def __init__(self, scoredsentences, model_file=None, alpha=0.025, window=5, sample=0, seed=1,
        workers=1, min_alpha=0.0001, sg=1, hs=1, negative=0, cbow_mean=0, iteration=1):
        self.sg = int(sg)
        self.table = None # for negative sampling --> this needs a lot of RAM! consider setting back to None before saving
        self.alpha = float(alpha)
    ...
    def reset_sent_vec(self, scoredsentences):
        ...
        for sent in scoredsentences:
            self.sents_len += 1
            self.scores_size=len(sent[1])         
        self.sents         = empty((self.sents_len, self.layer1_size), dtype=REAL)
        self.scores_weight = empty((self.scores_size,self.layer1_size), dtype=REAL)
        # randomize weights vector by vector, rather than materializing a huge random matrix in RAM at once
        for i in xrange(self.sents_len):
            self.sents[i]         = (random.rand(self.layer1_size) - 0.5) / self.layer1_size
        #self.scores_weight = (random.rand(self.layer1_size,self.scores_size) - 0.5) / self.layer1_size
    ...
    def train_sent(self, scoredsentences, total_words=None, word_count=0, sent_count=0, chunksize=100,train_mode=0):
        ...
        def prepare_sentences():
            for sent_no, s_sentence in enumerate(scoredsentences):
                sentence=s_sentence[0]
                # avoid calling random_sample() where prob >= 1, to speed things up a little:
                # sampled = [self.vocab[word] for word in sentence
                #            if word in self.vocab and (self.vocab[word].sample_probability >= 1.0 or self.vocab[word].sample_probability >= random.random_sample())]
                sampled = [self.vocab.get(word, None) for word in sentence]
                yield (sent_no, sampled, s_sentence[1])
...
class LineScoredSentence(object):
    """Simple format: one sentence = one line; words already preprocessed and separated by whitespace."""
    def __init__(self, source,scorefn):
        """
        `source` can be either a string or a file object.
    ...
    def __iter__(self):
        ...
        try:
            ...
            self.source.seek(0)
            for line in self.source:
                sent =utils.to_unicode(line).split()
                yield [sent,self.scorefn(sent)]
        except AttributeError:
            # If it didn't work like a file, use it as a string filename
            with utils.smart_open(self.source) as fin:
                for line in fin:
