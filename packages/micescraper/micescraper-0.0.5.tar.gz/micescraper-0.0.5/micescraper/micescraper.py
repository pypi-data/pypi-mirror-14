...
def rate_applicant(cid, rating, comment, username, password):
  """
  """
  try:
    r = requests.post(
        "https://mice.cs.columbia.edu/recruit/showApplicant.php", 
        data={
          'username':username, 
          'password': password,
...
def scrape_applicant(url, username, password, outdir):
  ...
  for r in dl_doc(url, username, password ,False):
    ...
    # if we hit this block, then there doesn't exist a consolidated pdf for the applicant
