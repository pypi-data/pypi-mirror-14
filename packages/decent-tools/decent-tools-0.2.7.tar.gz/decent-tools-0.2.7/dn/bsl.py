...
class DnBsl(bsl.SerialBSLTarget, object):
            ...
            while size > 0:
                ...
                if self._timeout > 0:
                    if time.time() - start > self._timeout:
                        break
            return data
        self.serial.read = functools.partial(monkey_read, self.serial)
    ...
    def BSL_SETMEMOFFSET(self, address_hi_bits):
        ...
        answer = self.bsl(bsl.bsl.BSL_SETMEMOFFSET, packet, expect=0)
...
def main(argv=None):
    ...
    dnbsl.verbose = (logging.ERROR - logging.WARNING) / 10
