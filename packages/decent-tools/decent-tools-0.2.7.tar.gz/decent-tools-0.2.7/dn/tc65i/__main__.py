...
def cb_put_file(modem, path, args):
    ...
    logger.info('transferred %s to %s', path, destpath)
...
def cb_atcmd(modem, path, args):
    ...
    logger.info('running at command %s ...', path)
    res = modem.send(path)
    logger.info(res)
...
def main():
    ...
    ser = serial.serial_for_url(args.port,
                                baudrate=BAUDRATE, timeout=TIMEOUT)
    ftdi_dev = ser.udev
    if not args.no_cbus:
        ftdi_dev.set_bitmode(bsl.TC65I_MODE, ftdi_dev.BITMODE_CBUS)
    try:
        ...
        ser.close()
        if not args.no_cbus:
            # ftdi_dev.set_bitmode(bsl.MSP430_UART_MODE,
