...
with file(os.devnull, "w") as devnull:
    version = subprocess.call([command, " --version"], stdout=devnull, stderr=devnull)
if not version == 0 and os.name == "nt":
    latest_r = []
    for basedir in ("C:\\Program Files\\R\\", "C:\\Program Files (x86)\\R\\"):
        if os.path.isdir(basedir):
            for r_directory in os.listdir(basedir):
                version_re = re.search(r"[0-9\.]+", r_directory)
                if version_re is not None:
                    r_version = [int(v) for v in version_re.group(0).split(".")]
                    if r_version > latest_r:
                        latest_r = r_version
                        command = os.path.join(os.path.join(basedir, r_directory), "bin\\R.exe").replace("/", "\\")
    if latest_r != [] and os.path.isfile(command):
        with file(os.devnull, "w") as devnull:
            version = subprocess.call([command, " --version"], stdout=devnull, stderr=devnull)
r_installed = version == 0
if r_installed:
    try:
        import pyper
    except ImportError:
...
def plot(coverage, panel, outputstem):
    ...
    r_process.run(rcode)
#    with file(os.devnull, "w") as devnull:
#        r_process = subprocess.Popen([command, "--vanilla"], stdin=subprocess.PIPE, stdout=devnull, stderr=devnull, universal_newlines=True)
#        r_process.communicate(rcode+"\nq()\n")
