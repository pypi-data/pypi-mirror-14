...
def JdkVersion(version, high=99, low=0):
    ...
    if resp.status_code == 200:
        return JdkVersion(high, guess)
    else:
        return JdkVersion(guess, low)

def JdkDownload(version):
    url = JdkVersion(version)
    print('URL:', url)
    resp = get(url)
    open('/usr/portage/distfiles/jdk-{}-linux-{}.tar.gz', 'wb').write(resp.content)
...
def Main():
    ...
    class Program(object):
        ...
        def __init__(self):
            ...
            argp.add_argument('--quick', '-q', action='store', help='quick mode')
            argp.add_argument(
        ...
        def run(self):
            ...
            jdkFetchRestriction = list(
                chain.from_iterable(
                    filter(
                        None,
                        (JDK_FETCH_RESTRICTION_RE.findall(line) for line in emerge)
                    )
                )
            )
            if jdkFetchRestriction:
                JdkVersion(jdkFetchRestriction[0])
                retryEmerge = True
            if retryEmerge:
            ...
            if tamperedPackages or packagesInstalled and not self.args.quick:
