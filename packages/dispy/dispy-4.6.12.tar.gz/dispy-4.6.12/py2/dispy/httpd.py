    ...
    class _HTTPRequestHandler(BaseHTTPRequestHandler):
        ...
        def do_GET(self):
            if self.path == '/cluster_updates':
                ...
                self.end_headers()
                self.wfile.write(json.dumps(updates, cls=DispyHTTPServer.ObjectEncoder).encode())
                return
                ...
                self.send_header('Content-Type', 'application/json; charset=utf-8')
                self.end_headers()
                self.wfile.write(json.dumps(status, cls=DispyHTTPServer.ObjectEncoder).encode())
                return
                ...
                self.send_header('Content-Type', 'application/json; charset=utf-8')
                self.end_headers()
                self.wfile.write(json.dumps(clusters, cls=DispyHTTPServer.ObjectEncoder).encode())
                return
            else:
