...
class ehcOpenApiHandler:
    ...
    def sqlQuery(self):
        ...
        jsonObj = {'lang':lang,'db':db,'query':query,'host':host,'ehcname':ehcname,'ehcid':ehcid,'type':type}
        data = json.dumps(jsonObj)
        return data
...
class jobOpenApiHandler:
    ...
    def createJobFromOutside(self):
        ...
        if options.Notification == None:
            ...
            notifition = options.Notification
        ...
        jsonObj = {'projectId':projectid,'name':name,'description':description,'userId':userid,'variable':variable,'projectVersionId':projectversionid,'notifition':notifition,'cc':cc}
        data = json.dumps(jsonObj)
        return data
    ...
    def createJob(self):
        ...
        jsonObj = {'name':name,'description':description,'variable':variable,'projectVersionId':projectversionid,'notifition':notifition,'cc':cc}
        data = json.dumps(jsonObj)
        a = [id,data]
        return a
...
class moduleOpenApiHandler:
    ...
    def createModule(self):
        ...
        if options.Udfs == None:
            print 'please input Module udfs file!'
            sys.exit(-1)
        else:
            udfs = options.Udfs

        if options.File == None:
            print 'please input Module files!'
            sys.exit(-1)
        else:
            file = options.File

        if options.Params == None:
            print 'please input Module parans!'
            sys.exit(-1)
        else:
            params = options.Params

        if options.Inputs == None:
            print 'please input Module inputs!'
            sys.exit(-1)
        else:
            inputs = options.Inputs

        if options.Outputs == None:
            print 'please input Module outputs!'
            sys.exit(-1)
        else:
            outputs = options.Outputs

        if options.Code == None:
            print 'please input Module logic code!'
            sys.exit(-1)
        else:
            code = options.Code


        jsonObj = {'name':name,'description':description,'type':type,'tags':tags,'udfs':udfs,'file':file,'params':params,'inputs':inputs,'outputs':outputs,'code':code}
        data = json.dumps(jsonObj)
        return data
    ...
    def moduleList(self):
        ...
        jsonObj = {'page':page,'num':num,'tags':tags,'keyword':keyword,'order_key':order_key,'order_by':order_by}
        data =json.dumps(jsonObj)
        return data
...
class projectOpenApiHandler:
    ...
    def setupWorkspace(self):
        ...
        (options,args) = parser.parse_args()
        jsonObj = {'projectId':options.Projectid,'locations':options.Locations,'connections':options.Connections,'variable':options.Variable,'envariables':options.Envariables}
        data = json.dumps(jsonObj)
        return data
