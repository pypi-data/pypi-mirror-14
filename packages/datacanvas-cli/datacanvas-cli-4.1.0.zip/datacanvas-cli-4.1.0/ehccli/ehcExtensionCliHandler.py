...
class ehcExtensionCliHandler():
    ...
    def handlerExtensionCmd(self,cmd):
        if cmd in ['help', '-h', '--help']:
            ...
            print 'ip:http://%s:%s'%(self.config.getAddress(),self.config.getPort())
            a = raw_input('whether to change?(y/n)')
            if a.lower() == 'y':
                host = raw_input('Input your ehc ip[%s]:'%self.config.getAddress())
                self.config.ehcHost(host)
                port = raw_input('Input your ehc port[%s]:'%self.config.getPort())
                self.config.ehcPort(port)
                sys.exit(1)
            elif a.lower() == 'n':
                sys.exit(1)
            else:
