...
def test10_positive_exact():
    """Positive for RBL"""
    imap = mock.Mock()
    tfilter = filter.RblFilter(imap, {'filter': 'rbl', 'org': 'spamcheck.swqzx', 'action': 'delete'}, {})
    message = email.message_from_file(open('test/rbl/spam.msg'))
    # Pretend gethostbyname HAS an entry about the IP in the mail
    with mock.patch('mailfilter.filter.gethostbyname', return_value='127.0.0.10') as f:
...
def test20_negative_exact():
    """Negative for RBL"""
    imap = mock.Mock()
    tfilter = filter.RblFilter(imap, {'filter': 'rbl', 'org': 'spamcheck.plhb', 'action': 'delete'}, {})
    message = email.message_from_file(open('test/rbl/spam.msg'))
    # Pretend gethostbyname has no entry about the IP in the mail
    with mock.patch('mailfilter.filter.gethostbyname', side_effect=socket.gaierror) as f:
