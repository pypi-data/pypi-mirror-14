...
Log = logging.getLogger('{}.{}'.format(app.Name, __name__))
...
class Filter(object):
    ...
    def extract(message):
        ...
        if message.is_multipart():
            return ''.join(BodyFilter.extract(m) for m in message.get_payload(decode=True))
        else:
            return message.get_payload(decode=True) or message.get_payload(decode=True)

class All(Filter):
    def __init__(self, imap, filterDef, globalConfig):
        self.log = logging.getLogger('{}.{}.{}'.format(app.Name, str(self.__module__), __name__))
        super(All, self).__init__(imap, filterDef, globalConfig)
...
class RblFilter(Filter):
    ...
    def __init__(self, imap, filterDef, globalConfig):
        ...
        self.log = logging.getLogger('{}.{}.{}'.format(app.Name, str(self.__module__), __name__))
        super(RblFilter, self).__init__(imap, filterDef, globalConfig)
...
class ContentFilter(Filter):
    ...
    def __init__(self, imap, filterDef, globalConfig):
        ...
        self.log = logging.getLogger('{}.{}'.format(app.Name, str(self.__module__), __name__))
        # Setup check
        lexer = shlex.shlex(filterDef['check'], posix=True)
        lexer.whitespace_split = True
        args = tuple(lexer)
        ...
        super(ContentFilter, self).__init__(imap, filterDef, globalConfig)
...
class BodyFilter(ContentFilter):
    ...
    def __init__(self, imap, filterDef, globalConfig):
        ...
        self.log = logging.getLogger('{}.{}.{}'.format(app.Name, str(self.__module__), __name__))
        super(BodyFilter, self).__init__(imap, filterDef, globalConfig)
...
class HeaderFilter(ContentFilter):
    ...
    def __init__(self, imap, filterDef, globalConfig):
        ...
        self.log = logging.getLogger('{}.{}.{}'.format(app.Name, str(self.__module__), __name__))
        super(HeaderFilter, self).__init__(imap, filterDef, globalConfig)
    ...
    def check(self, msgid, folder, message):
        ...
        :rtype: bool
        """
        result = self.filterFunction.check(msgid, message[self.header])
...
class PyzorFilter(Filter):
    ...
    def __init__(self, imap, filterDef, globalConfig):
        ...
        super(PyzorFilter, self).__init__(imap, filterDef, globalConfig)
...
class BayesFilter(ContentFilter):
    ...
    def __init__(self, imap, filterDef, globalConfig):
        ...
        """
        self.log = logging.getLogger('{}.{}.{}'.format(app.Name, str(self.__module__), __name__))
        super(BayesFilter, self).__init__(imap, filterDef, globalConfig)
...
class UrlFilter(Filter):
    ...
    def __init__(self, imap, filterDef, globalConfig):
        ...
        """
        self.log = logging.getLogger('{}.{}.{}'.format(app.Name, str(self.__module__), __name__))
        super(UrlFilter, self).__init__(imap, filterDef, globalConfig)
    ...
    def checkPart(self, part):
        ...
        if part.get_content_type() == 'text/html':
            ...
            return not urls or any(CheckSpamUrl(u) for u in urls)
