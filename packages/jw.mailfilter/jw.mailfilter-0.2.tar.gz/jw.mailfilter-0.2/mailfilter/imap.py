...
class IMAP(IMAPClient):
    ...
    def __init__(
        ...
        self.log = logging.getLogger('{}.{}'.format(app.Name, self.__class__.__name__))
        super(IMAP, self).__init__(host, *args, **kwargs)
        ...
        for i in range(10):
            try:
                ...
                try:
                    ...
                    self.log.critical('From self._create_IMAP4() in %s: %s', self.host, e2)
                    continue
                try:
                    self.login(self.username, self.password)
                except Exception as e2:
                    self.log.critical('From login("%s", "***") in %s: %s', self.username, self.host, e2)
                    continue
        else:
            self.log.critical(
