...
class Folder(object):
    ...
    def run(self):
        ...
        while True:
            if idleUsed:
                ...
                try:
                    idleResult = self.imap.noop()
                except Exception as e:
                    self.log.critical('%s: From imap.noop: %s', self.name, e)
                    if not self._startImap():
                        break
                    continue
    ...
    def _check(self, search='UNSEEN'):
        ...
        for folder, filters in self.folders:
            self.log.info('Checking folder %s in %s', folder, self.name)
    ...
    def _filterFromString(self, filterDef):
        ...
        """
        try:
            filterClass = LoadClass('jw.mailfilter.filter', filterDef['filter'], type_=filter.Filter)
        except KeyError:
            raise UndefinedFilter(str(filterDef))
        except Exception as e:
            raise filter.FilterError('Account definition error %s: %s' % (repr(e), repr(filterDef)))
