...
def bwa_mem(uuid, fastq1_path, fastq2_path, reference_fasta_path, readgroup_json_path, engine, logger):
    step_dir = os.getcwd()
    ...
    bwa_cmd = ['bwa', 'mem', '-t ' + thread_count, '-T', '0', '-R ' + '"' + rg_str + '"', reference_fasta_path,
               '"' + fastq1_path + '"' , '"' + fastq2_path + '"']
    samtools_cmd = ['samtools', 'view', '-Shb', '-o', '"' + outbam + '"', '-']
