...
def __default_name(name, with_domain=True):
    names = name.split("_")
    if with_domain:
...
def __parse_server(d_name, name, upstream_name, backends, entrypoints):
    listens = []
    server_names = [name]
...
def __parse_balancer(balancer):
    name = __default_name(balancer.name, False)
    ...
    server, ssl_file = __parse_server(name, default_name, upstream_name, backends, entrypoints)
