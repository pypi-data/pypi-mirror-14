...
from plytix.retailers.models.product import Product, ProductPictures
...
class ProductsEndpoint(BaseEndpoint):
    ...
    def search(self, folder_list=None, group_by=None, name=None, operator=None, product_list=None, sku_list=None,
    ...
    def pictures(self, product_list, origin):
        """
        :param product_list: Product identifier's list.
        :param origin: Address of the site where the product is going to be served.
        ...
        endpoint = 'pictures'
        try:
            response = super(ProductsEndpoint, self)._search(endpoint=endpoint, data=data, page_length=len(product_list))
            data = response.json()
            page = data[RESPONSE_METADATA][RESPONSE_METADATA_PAGE]
            total_pages = data[RESPONSE_METADATA][RESPONSE_METADATA_TOTAL_PAGES]
            total = data[RESPONSE_METADATA][RESPONSE_METADATA_TOTAL]
            items = [ProductPictures.parse(product) for product in data[MODEL_PRODUCTS]]
...
class BankEndpoint(BaseEndpoint):
    ...
    def search(self, brand_id=None, folder_id=None, name=None, name_list=None, product_id=None, operator=None,
        ...
        :param identifier: Search by SKU.
