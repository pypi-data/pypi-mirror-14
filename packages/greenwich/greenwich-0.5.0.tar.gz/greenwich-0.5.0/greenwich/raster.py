...
# FIXME: WTF, order matters here!?
#from greenwich.io import MemFileIO, VISFile, vsiprefix
#from greenwich.io import VSIFile, MemFileIO, vsiprefix
...
# TODO: Make ImageDriver @classmethod?
...
# TODO: support the geo_interface (e.g. Fiona, Shapely, ArcPy, PyShp, GeoDjango)
# FIXME: Get rid of PIL and use gdal.RasterizeLayer()
...
# TODO: move to geometry module? Geometry class or static method?
...
#class AvailableDrivers(dict):
class DriverDict(dict):
    _writekey = 'DCAP_CREATE'

    def __init__(self, drivers=None):
        #dict.__init__(self, defaults or {})
        #dict.__init__(self, drivers or available_drivers())
        if drivers is None:
            drivers = {}
            for i in range(gdal.GetDriverCount()):
                d = gdal.GetDriver(i)
    ...
    def filter_writable(self):
        """Return a dict of drivers supporting raster writes."""
        return {k: v for k, v in self.items() if self._writekey in v}
...
class ImageDriver(object):
    ...
    def raster(self, path, size, bandtype=gdal.GDT_Byte):
        ...
        # TODO: do not babysit users with this check.
...
class Raster(Comparable):
    ...
    def __init__(self, path, mode=gdalconst.GA_ReadOnly):
        ...
        if path and not isinstance(path, gdal.Dataset):
            ...
            #vpath = vsipath(getattr(path, 'name', path))
            #dataset = gdal.Open(vpath, mode)
        else:
            dataset = path
        if not dataset:
        ...
        # GDAL band indices are one-based.
    ...
    def __array_interface__(self):
        return {'shape': self.shape,
                # FIXME: Get this from gdal.array_modes dict
                #'typestr': '<f8',
                'typestr': gdal.array_modes[self[0].DataType],
                #'data': self.tobytes(),
                'version': 3}

    def __getattr__(self, attr):
        """Delegate calls to the gdal.Dataset."""
        # FIXME: Deal with potentially closed gdal.Dataset!
        try:
    ...
    def get_offset(self, envelope):
        ...
        #TODO: Use 1 pixel as the bare minimum? Causes test_clip() failure,
        # right need to use max not min, duh.
        #nx = min(lr_px[0] - ul_px[0], self.ds.RasterXSize - ul_px[0], 1)
        #ny = min(lr_px[1] - ul_px[1], self.ds.RasterYSize - ul_px[1], 1)
    ...
    def save(self, to, driver=None):
        ...
        # TODO: save to BytesIO, StringIO
        #with VSIFile(self.name) as f:
            #bio.write(f.read())
            #f.readinto(bio)
...
MODES = {'r': gdalconst.GA_ReadOnly,
         'r+': gdalconst.GA_Update}
#MODES['rb'] = MODES['r']

# TODO: open() should also return a raster for bytes of a full image like
# im = PIL.open(BytesIO(pngbytes))
# Alias the raster constructor as open().
#open = Raster
#def open(path, mode=gdalconst.GA_ReadOnly):
    ##path = getattr(path, 'name', path)
    ## TODO: Should we handle File objects with file.read()?
    ## Path could be entire image as BytesIO
    ##if isinstance(path, str):
    ## Initialize from StringIO and BytesIO instances.
    ##if hasattr(path, 'getvalue'):
    ##if hasattr(path, 'read'):
        ##imgio = MemFileIO(delete=True)
        ##gdal.FileFromMemBuffer(imgio.name, path.read())
...
def open(path, mode=gdalconst.GA_ReadOnly):
    ...
    mode -- gdal constant representing access mode
    """
    # TODO: Duplicated here and Raster()
    path = getattr(path, 'name', path)
    try:
        return Raster(vsiprefix(path), mode)
    except AttributeError:
...
#?def frombuffer(imgdata):

# def fromarray(arrayproto):
def fromarray(obj):
    # arr = obj.__array_interface__
    # bandtype = arr['typestr']
    bandtype = gdal_array.flip_code(obj.dtype.type)
    # GDAL Driver.Create() expects x,y,z shape.
    return frombytes(obj.tobytes(), obj.shape, bandtype)

# TODO: Match the Pillow signature, already uses same func name.
#def frombytes(mode=gdal.GDT_Byte, size, data):
