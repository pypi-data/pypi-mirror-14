...
def transform_tile(xtile, ytile, zoom):
    ...
    # 2^2n
    #maxnumtiles = 2 ** (2 * zoom)
    #raise ValueError
    n = 2.0 ** zoom
    # zoom level 3 = 64 max tiles, width = 2^3 = 8, height = 8
    # TODO: wrap xtile, but not ytile?
    # FIXME: Isn't it invalid to have >180 longitude, how should we handle
    # that.
    #if xtile > n or ytile > n:
        #raise ValueError('Invalid tile coordinate for zoom level %d' % zoom)
    #try:
        #lat_rad = math.atan(math.sinh(math.pi * (1 - 2 * ytile / n)))
        #y = math.degrees(lat_rad)
    #except OverflowError:
        #y = -90.0 if ytile >= 0 else 90.0
