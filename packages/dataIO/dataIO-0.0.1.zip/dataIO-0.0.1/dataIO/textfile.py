...
if __name__ == "__main__":
    ...
    class Unittest(unittest.TestCase):
        def test_readlines(self):
            result = list(readlines(path, strip=None))
            self.assertListEqual(result, f_lines)
             
            result = list(readlines(path, skiplines=3, strip=None))
            self.assertListEqual(result, f_lines[3:])
             
            result = list(readlines(path, nlines=3, strip=None))
            self.assertListEqual(result, f_lines[:3])
            
            result = list(readlines(path, skiplines=3, nlines=3, strip=None))
            self.assertListEqual(result, f_lines[3:3+3])
            
        def test_readchunks(self):
            result = list(readchunks(path, strip=None))
            self.assertEqual(result[0], f_lines[0:1])
            
            result = list(readchunks(path, skiplines=3, strip=None))
            self.assertEqual(result[0], f_lines[3:4])
            
            result = list(readchunks(path, chunksize=3, strip=None))
            self.assertEqual(result[0], f_lines[0:3])
            
            result = list(readchunks(path, skiplines=3, chunksize=3, strip=None))
            self.assertEqual(result[0], f_lines[3:6])
