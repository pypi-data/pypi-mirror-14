...
class StarBlock(object):
        ...
        try:
           ...
           if self.dictionary:
               ...
               try:
                   ...
                   except StarDerivationFailure,s:
                       print "In StarBlock.__getitem__, " + `s`
                ...
                if do_calculate:
                   ...
                   try:
                       ...
                       try:
                           ...
                           print "Could not turn %s into a value:" + `s`
                           return rawitem
                   else:
                       print 'Set %s to derived value %s' % (key, `new_value`)
                       return new_value
    ...
    def format_value(self,itemvalue,stringsink,compound=False,hints={}):
        ...
        if isinstance(itemvalue,basestring):  #need to sanitize
           ...
           stringsink.write('}',unindent=True)
        elif isinstance(itemvalue,(float,int,long)):  #TODO - handle uncertainties
           stringsink.write(str(itemvalue),canbreak=True,startcol=startcol)   #numbers
        else:
           raise ValueError, 'Value in unexpected format for output: %s' % `itemvalue`
...
class StarDerivationFailure(Exception):
    def __init__(self,fail_name):
        self.fail_name = fail_name
    def __str__(self):
