...
def make_python_function(in_ast,func_name,targetname, special_ids=[{}],
    ...
    if len(withtable) > 0 or cur_row:  # a loop method
            ...
            # preamble +="        print 'Globals inside looped drel_func:' + `globals()`\n"
            ...
            if cur_row and len(actual_names) > 1:    #i.e. have real names from category
                ...
                end_body += "       else:\n"
                end_body += "           return []\n"
            end_body+=      "       return map(lambda %s:drel_func(%s),%s)\n" % (dummy_vars,dummy_vars,map_names)
            end_body+=     "    else:\n"
...
# everything up to the final number
        ...
        outstring += "\nprint 'names in cat = %s' % `__pyallitems`"
        outstring += "\n" + "__pycitems = filter(lambda a:%s,__pyallitems)" % (api_table["has_name"] % "a")
        ...
        outstring += "    __pycitems = filter(lambda a:%s,__pyallitems)\n" % (api_table["has_name"] % "a")
        outstring += "    print 'After category creation, names in cat ->' + `__pycitems`\n"
