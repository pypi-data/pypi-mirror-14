...
class aprofile_set(object):
  ...
  def __init__(self, source, lazy=False, verbose=False, **kwargs):
  ...
  def get_dump(self, dump=None):
    ...
    if dump in self.profiles:
      ...
      self.log.error("dump %i not found" % dump)
      return None
...
class aprofile(object):
  ...
  header_var_list = [
      ...
      dict(name='source_dump', pos=8, type='i'),
  ]

  def __init__(self, filename, verbose=False, lazy=False, names=None, **kwargs):
    """ Load an AProfile
  ...
  def get_state(self):
    ...
    if self.lazy:
      with open(self._filename, 'r') as f:
        f.seek(self._header_size + 108 * self.buckets)
  ...
  def get_table(self, index):
    ...
    if self.lazy:
      with open(self._filename, 'r') as f:
        f.seek(self._header_size + 108 * self.buckets + data_offset)
        data = np.fromfile(f, dtype=np.float32, count=count).reshape(shape, order='F')
    else:
...
def main():
  ...
  if os.path.isdir(path):
    ...
    for k in header_keys:
      print("%s: %s" % (k, rp.header[k]))

    print "Normals", rp.normals
    print "Centers", rp.centers
    print "Edges", rp.corners
    print "Neighbors", rp.neighbors
    print "State", rp.get_state()
    print "r", rp.r
