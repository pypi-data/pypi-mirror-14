...
from oic.extension.message import TokenIntrospectionResponse, TokenIntrospectionRequest
...
class OAuth2Authorization(object):
    def __init__(self, introspection_endpoint, introspection_authn=None):
        # type: (str, Optional[requests.auth.AuthBase]) -> None
        self.introspection_endpoint = introspection_endpoint
        self.introspection_authn = introspection_authn

        self.callback = None

        self.client = oic.extension.Client()

        # TODO create OAuth client instance?

    def _authorize(self):
        authz_header = flask.request.headers.get("Authorization")
        if not authz_header.startswith("Bearer"):
            raise ValueError("Wrong token type.")

        access_token = authz_header[len("Bearer"):]
        request = TokenIntrospectionRequest(token=access_token)
        introspection_response = requests.post(self.introspection_endpoint,
                                               data=request.to_urlencoded().encode("utf-8"),
                                               auth=self.introspection_authn)

        if introspection_response.status_code == 200:
            flask.g.authorization = TokenIntrospectionResponse(**introspection_response.json())

        return self.callback()

    def oauth2_protected(self, f):
        self.callback = f
