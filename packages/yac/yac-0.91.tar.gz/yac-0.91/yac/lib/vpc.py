...
def get_vpc_prefs_from_local_file(vpc_prefs_file):
    ...
    vpc_inputs_script_path = get_variable(vpc_prefs,'vpc-inputs',"")
    ...
    if vpc_inputs_script_path:
        ...
        set_variable(vpc_prefs,'vpc-inputs',inputs_file_yac_path)
    ...
    # If the vpc prefs references service templates, then we need to save them
    # under yac customization so they can be accessed locally.

    if 'service-templates' in vpc_prefs:

        for key in vpc_prefs['service-templates'].keys():

            template_file_name = vpc_prefs['service-templates'][key]

            # use the preferences name for the inputs file namespace
            yac_file_key = get_file_reg_key(template_file_name,vpc_prefs_name)

            template_file_contents = get_file_contents(os.path.join(prefs_path,template_file_name))

            # save the file under yac/lib/customizations
            inputs_file_yac_path = create_customization_file(yac_file_key,template_file_contents)

            # save the file as a yac-local customization
            vpc_prefs['service-templates'][key] = inputs_file_yac_path
