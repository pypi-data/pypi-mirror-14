...
def validate_services_consumed(services_consumed):

    valid_services_consumed = get_all_services_consumed()

    validation_errors = {}

    unsupported_resources = list(Set(services_consumed) - Set(valid_services_consumed))
...
def get_all_services_consumed():
    # currently supported build-in services
...
def get_stack_templates(service_descriptor, vpc_prefs, service_parmeters):
    ...
    load_balancer_refs = []
    if ('i-elb' in services_consumed):
        load_balancer_refs.append({"Ref": "InternalElb"})
    if ('e-elb' in services_consumed):
...
def _get_stock_template_files(services_consumed, vpc_prefs):
    ...
    if 'services-consumed' in vpc_prefs:
        vpc_prefs_template_map = vpc_prefs['services-consumed']
...
def deploy_stack_files(service_descriptor, service_parmeters): 
    ...
    _load_files(files_for_boot, service_parmeters)
...
# get the ip address of an ec2 instance in a stack identified by stack_name
# with a name containing name_search_string.
# If no name_search_string provided, returns IP address of all stack EC2 instances
def get_ec2_ip( stack_name , name_search_string=""):
    ...
    # if there are Reservations pull instances out of response, else set
...
def get_stack_s3_bucket(params):

    return get_stack_param_value( params , "S3Bucket" )

def get_stack_snapshot( params ):

    return get_stack_param_value( params , "DBSnapshotId" )

def get_stack_param_value( service_params , stack_param_name ):
    ...
    if len(param_tuple)==2:
        ...
        if param_value:
            ...
            if yac_param_key in decoded_dict:
                yac_param_value = decoded_dict[yac_param_key]
