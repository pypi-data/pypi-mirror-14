...
def main():
    ...
    service_parmeters = get_service_parmeters("service-alias", "dev", "", 
    ...
    if args.dryrun:
        ...
        print "env vars: %s"%str(env_variables)
        print "volumes map: %s"%str(volumes_map)
        print "volumes bindings: %s"%str(volumes_bindings)
        print "port bindings: %s"%str(port_bindings)
        print "special command: %s"%args.cmd
        print user_msg
