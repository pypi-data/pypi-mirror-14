...
def create_customization_file(key,file_rel_path,file_contents):

    script_file_rel_path = os.path.join(key,file_rel_path)

    # write the script contents to a local file under lib
    # include the app name in the path to prevent collisions with other custom scripts
    ...
    return os.path.join( 'yac','lib','customizations', script_file_rel_path)
