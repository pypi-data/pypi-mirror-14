...
from yac.lib.stack import validate_stock_resources
...
def validate_service(service_descriptor):
    ...
    # get the stock resources specified in the server parameters
    stock_resources = get_variable(service_descriptor,'stock-resources',[])

    # check that all are valid
    val_resource_errors = validate_stock_resources(stock_resources)
...
def get_service_parmeters(service_alias, env_arg, params_file, 
    ...
    # add stock resources from the service descriptor to the params
    stock_resources = get_variable(service_descriptor,'stock-resources',[])
    set_variable(service_parmeters,'stock-resources',stock_resources)
    ...
    # add service-inputs (used to drive user-prompts for param that are not otherwise
    # specified)
