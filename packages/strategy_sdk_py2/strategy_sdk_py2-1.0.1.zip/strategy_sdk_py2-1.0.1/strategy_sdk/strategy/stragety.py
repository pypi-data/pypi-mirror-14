...
from strategy.bean.bean import StrategyOrderDetail, StrategyOrderError, StrategyQuoteDataResult, \
    StrategyError, StrategyAccountDetailResultBean, StrategyPositionStaticsResultBean, \
    StrategyPositionDetailResultBean, StrategyRequestOrder, \
    StrategyConnectionInfo, StrategyConfiguration, ArrayStrategyFinanceAccountDetails, ArrayStrategyPositionDetails, \
    TickFiveLevelData
...
class BaseStrategy(AbstractStrategy, AbstractNotifyMessageHandler, AbstractAsyncMessageHandler):
    ...
    def req_handshake(self, version):
        # XXX Java代码为空，注释掉了
        return None
    ...
    def req_account_detail(self, strategy_id):
        result = StrategyAccountDetailResultBean()
        ...
        try:
            ...
            if response_info.is_success():
                data = ArrayStrategyFinanceAccountDetails().from_json(data)
                result.data = data
                result.error = self._new_succeed_error()
            else:
                data = StrategyError().from_json(data)
                result.error = data
            return self._convert_2_strategy_error(response_info.protocolMessage)
        except Exception, e:
            logging.exception(u"%s", e)
    ...
    def req_position_statics(self, strategy_id):
        result = StrategyPositionStaticsResultBean()
    ...
    def req_quota_data(self, quote_data_category):
        result = StrategyQuoteDataResult()
        # XXX java代码空
        return result

    def req_position_detail(self, strategy_id):
        result = StrategyPositionDetailResultBean()
        ...
        try:
            ...
            if response_info.is_success():
                ...
                err = data.get(u"error")
                data = data.get(u"data")
                err = StrategyError().from_dict(err)
                data = ArrayStrategyPositionDetails().from_dict(data)
                result.data = data
                result.error = err
        except Exception, e:
            logging.exception(u"%s", e)
    ...
    def req_order(self, order_info):
        ...
        try:
            ...
            if response_info.is_success():
                data = response_info.data
                logging.info(u"response order: ", data)
                return data
        except Exception, e:
            logging.exception(u"%s", e)
            self._handle_request_exception(e)
...
class SimpleStrategy(AbstractStrategy, AbstractNotifyMessageHandler):
    ...
    def req_login(self, conn_info):
        ...
        logging.info(u"req_login, err is ", err.to_json())
        return err
    ...
    def on_notify_order_error(self, order_error):
        logging.exception(u"order_error: ", order_error.to_json())

    def on_notify_trade_detail(self, trade_detail):
        pass

    def on_notify_order_detail(self, order_detail):
        logging.info(u"order detail: %s", order_detail)
...
class TestStrategy(unittest.TestCase):
    ...
    def test_simple_strategy(self):
        ...
        try:
            account_detail = simple_strategy.req_account_detail(u"1")
        ...
        try:
            position_detail = simple_strategy.req_position_detail(u"1")
        ...
        try:
            position_statics = simple_strategy.req_position_statics(u"1")
