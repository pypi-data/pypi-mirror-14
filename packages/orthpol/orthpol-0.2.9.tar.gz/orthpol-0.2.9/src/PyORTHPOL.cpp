...
Py_polyeval(PyObject *self, PyObject *args)
{
  ...
  if (n >= 0) {
    if (normalized == 0){
      ...
      for (int i = 0; i < dims[0]; i++) outinner[i] = 1./sqrt(beta[0]);
    }
  }
  if (n >= 1) {
    ...
    if (normalized == 0){
      for (int i = 0; i < dims[0]; i++) 
        outinner[i] = rs[i] - alpha[0];
    } else {
      for (int i = 0; i < dims[0]; i++) 
        outinner[i] = (rs[i] - alpha[0]) * old1[i] / sqrt(beta[1]);
    }
  }
  if (n >= 2) {
  ...
  for (int j=2; j <= n; j++){
    ...
    if (normalized == 0){
      ...
      for (int i = 0; i < dims[0]; i++)
        outinner[i] = (rs[i] - alpha[j-1]) * old1[i] / sqrt(beta[j]) - 
          sqrt(beta[j-1]/beta[j]) * old2[i];
    }
  }
...
Py_numeric_gamma(PyObject *self, PyObject *args)
{
  ...
  double sqnrm;
  // INTERNAL
  double *tmp, *nrmpol, *nrmold1, *nrmold2;
  ...
  // Free space and prepare output
  if (n>=1) { delete [] nrmold1; }
  if (n>=2) { delete [] nrmold2; }
  delete [] nrmpol;
  delete [] zeros;
  delete [] weights;
