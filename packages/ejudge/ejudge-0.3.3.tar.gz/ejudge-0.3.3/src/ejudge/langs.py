...
class LanguageManager(metaclass=LanguageMeta):
    ...
    def run(self, inputs, *, timeout=None, context=None):
        ...
        try:
            ...
            return iotypes.ErrorTestCase.timeout(
                data=self.flush_io(),
                error_message=msg)

        except Exception as ex:
            return remove_trailing_newline(iotypes.ErrorTestCase.runtime(
                data=self.flush_io(),
                error_message=self.prepare_error(ex),
            ))
    ...
    def exec_pinteract(self, inputs, context):
        ...
        for inpt in inputs:
            try:
                ...
                #
                # The default behavior is just to send a truncated IoTestCase
                if process.is_dead():
                    if self._send_early_termination_error:
                        missing = [inpt]
                        missing.extend(inputs)
                        missing = '\n'.join('    ' + x for x in missing)
                        msg = ('process closed with consuming all inputs. List'
                               ' of unused inputs:\n')
                ...
                # This clause is just a safeguard. We don't expect to go here
                return iotypes.ErrorTestCase.runtime(
                    list(result),
                    error_message='An internal error occurred while trying to '
                                  'interact with the script: %s.' % ex
                )
            append_non_empty_out()
