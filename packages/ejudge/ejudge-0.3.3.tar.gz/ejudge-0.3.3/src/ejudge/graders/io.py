...
def _run(inputs, manager=None, build_error=None):
    ...
    if build_error:
        return build_error_tree(*build_error)
...
def build_error_tree(ex, tb):
    out = io.StringIO()
    traceback.print_tb(tb, file=out)

    return iotypes.IoSpec(
        [iotypes.ErrorTestCase.build(error_message=out.getvalue())]
    )
