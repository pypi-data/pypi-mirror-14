...
from .middleware import Middleware as NoopMiddleware
...
class Engine(object):
    ...
    def __init__(self, transport, rpc_handler, rpc_middleware=None):
        ...
        # Middleware for inbound requests and notifications
        self.rpc_middleware = rpc_middleware or NoopMiddleware()
    ...
    def handle_message(self, message):
        ...
        if isinstance(message, Notification):
            ...
            def worker():
                if self.rpc_middleware.process_message(message) is not None:
                    return
                try:
            ...
            def worker():
                try:
                    response = self.rpc_middleware.process_message(message)
                    if response is None:
                        response = self.rpc_handler(message)
                    mw_response = self.rpc_middleware.process_response(message, response)
