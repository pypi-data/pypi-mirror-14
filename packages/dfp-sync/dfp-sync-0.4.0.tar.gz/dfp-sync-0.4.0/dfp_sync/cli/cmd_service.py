...
def setup(parser):
  ...
  srv_parser.add_argument('--no-pagination', action="store_true",
...
def gen_export_commands(opts):
  ...
  service = DFPService(opts.service_name, 
                       config=opts.config, 
                       page= False if opts.no_pagination else True)
