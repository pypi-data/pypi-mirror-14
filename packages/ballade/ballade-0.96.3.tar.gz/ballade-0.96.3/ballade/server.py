...
class ProxyHandler:
    ...
    def on_request_line(self, request_line_buffer):
        try:
            ...
            logging.error("This request is not compatible with HTTP protocol.")
            self.inbound.close()
            return
        self.inbound.read_until(b'\r\n\r\n', self.on_header)
    ...
    def on_header(self, header_buffer):
        self.header_dict = dict(header_parser(header_buffer))
        ...
        if self.method == b'CONNECT':
            ...
            if b'Proxy-Connection' in self.header_dict.keys():
                self.header_dict[b'Connection'] = self.header_dict[b'Proxy-Connection']
            for header in [b'Proxy-Connection', b'Proxy-Authenticate', b'Transfer-Encoding', b'Upgrade']:
            ...
            if b'Host' in self.header_dict:
                ...
                if (self.version == b'HTTP/1.0' and self.header_dict.get(b'Connection').decode().lower() != 'Keep-Alive'.lower()) or\
                        (self.version == b'HTTP/1.1' and self.header_dict.get(b'Connection').decode().lower() == 'Close'.lower()):
                    # Outbound is closed by remote server, only need to close the inbound
                    self.inbound.close()
            else:
                self.outbound.close()
                self.inbound.close()
        logging.debug(self.header_dict)


class ProxyServer(tornado.tcpserver.TCPServer):

    def __init__(self, connector=None, auth=None):
