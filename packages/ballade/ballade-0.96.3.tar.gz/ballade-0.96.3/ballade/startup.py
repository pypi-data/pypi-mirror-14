...
def main():
    ...
    if config.get('auth') and config.get('auth').get('username') and config.get('auth').get('password'):
        auth = base64.encodebytes((config['auth']['username'] + ':' + config['auth']['password']).encode()).strip()
    else:
        auth = None

    connector = RulesConnector(path=regex_path, config=config)
    server = ProxyServer(connector, auth)
