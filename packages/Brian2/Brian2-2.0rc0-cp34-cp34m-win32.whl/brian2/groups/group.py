...
class Indexing(object):
    ...
    def __call__(self, item=None, index_var=None):
        ...
        if isinstance(item, tuple):
            ...
            if item is None or (isinstance(item, str) and item == 'True'):
                item = slice(None)
...
class VariableOwner(Nameable):
    ...
    def get_states(self, vars=None, units=True, format='dict',
        ...
        if vars is None:
            vars = [name for name, var in self.variables.items()
...
class Group(VariableOwner, BrianObject):
    ...
    def _resolve_external(self, identifier, run_namespace, user_identifier=True,
        ...
        if isinstance(resolved, collections.Callable) and not isinstance(resolved, Function):
