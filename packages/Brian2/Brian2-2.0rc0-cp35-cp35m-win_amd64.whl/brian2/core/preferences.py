...
class BrianGlobalPreferences(MutableMapping):
    ...
    def get_documentation(self, basename=None, link_targets=True):
        ...
        s = ''
        if basename is None:
            basenames = [tuple(basename.split('.')) for basename in list(self.pref_register.keys())]
            basenames.sort()
    ...
    def _as_pref_file(self, valuefunc):
        ...
        '''
        s = ''
        for basename, (prefdefs, basedoc) in list(self.pref_register.items()):
            s += '#' + '-' * 79 + '\n'
    ...
    def do_validation(self):
        ...
        '''
        for name, value in list(self.prefs_unvalidated.items()):
            self[name] = value
    ...
    def check_all_validated(self):
        ...
        if len(self.prefs_unvalidated):
            ...
            logger.warn("The following preferences values have been set but "
                        ...
                        "import." % ', '.join(list(self.prefs_unvalidated.keys())),
                        once=True)
...
class BrianGlobalPreferencesView(MutableMapping):
    ...
    def __init__(self, basename, all_prefs):
        ...
        self._subcategories = [key for key in all_prefs.pref_register.keys()
