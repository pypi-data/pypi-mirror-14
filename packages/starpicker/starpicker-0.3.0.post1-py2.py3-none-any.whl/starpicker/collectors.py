...
class RedditMentionCollector(BaseCollector):

    url = 'http://metareddit.com/feeds/monitor/{config.METAREDDIT_MONITOR_ID}/skypicker/rss/'.format(config=config)
    review_class = reviews.RedditMentionReview
    enabled = bool(config.METAREDDIT_MONITOR_ID)
