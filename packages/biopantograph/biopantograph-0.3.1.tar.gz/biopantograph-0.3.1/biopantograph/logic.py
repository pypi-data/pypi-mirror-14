...
## NL: formula simplification and absorption law

def is_atom(s):
    return not s.args or is_symbol(s.op)

def clean_formula(s):
    """Clean redundant items (A|A), (A&A) to A"""

    if is_atom(s): return s     ## (Atoms are unchanged.)
    args=map(clean_formula, s.args)

    if s.op in ("|", "&"):
        if len(args)==2 and is_atom(args[0]) and is_atom(args[1]):
        # if both args are the same, keep only one
            if args[0]==args[1]:
                return args[0]
        else:
            # try absorption
            args=absorption_law(s, args)
            ...
            for a in args:
                if a in present: toDelete.append(a)
                else: present.add(a)
...
def raise_same_op(s):
    ...
    # descend recursively
    args=map(raise_same_op, s.args)
    
    to_expand=[]
    for a in args:
        if not is_atom(a) and a.op==s.op:
            to_expand.append(a)
...
def absorption_law(s, args):
    if s.op=="|": otherop="&"
    elif s.op=="&": otherop="|"
    else: assert False, "absorption law cannot handle this op: %s" % (s.op)
    ...
    atoms = [x for x in args if is_atom(x)]
    for atom in atoms:
        for elem in args:
            if not is_atom(elem) and elem.op==otherop and atom in elem.args:
                toDelete.add(elem)
