...
class sigterm_protection(object):

    """Context manager which blocks SIGTERM until the context is left.
    A caught sigterm will be recorded and handled with sys.exit() as soon as no other
    sigterm_protection context manager is active.
    """

    opened_files = AtomicCounter()

    """
    we need the counter because we may only restore the signal handler if no other thread
    redirected this using this context manager !
    """
    ...
    def __enter__(self, *a, **kw):
        self._term_triggered = False

        def record_sigterm(sig, frame):
            get_logger().warn("caught SIGTERM during file I/O")
            get_logger().warn("args where %s and %s" % (self.a, self.kw))
            self._term_triggered = True

        self._orig_sigterm_handler = signal.signal(signal.SIGTERM, record_sigterm)
        self.opened_files.inc()

    def __exit__(self, *a, **kw):
        signal.signal(signal.SIGTERM, self._orig_sigterm_handler)
        self.opened_files.dec()
        if self._term_triggered:
            while not self.opened_files.is_zero():
                time.sleep(0.001)
            get_logger().warn("exit because of caught SIGTERM during file I/O")
            sys.exit(128 + signal.SIGTERM)  # convention


@contextlib.contextmanager
def sigterm_protected_open(*a, **kw):
    """SIGTERM protected open() function. For more details read the doc string of
    sigterm_protection()
    """
    with sigterm_protection(*a, **kw):
        yield open(*a, **kw)


def sigterm_protected(fun):
    """function decorator which creates a SIGTERM protected function. For more details read the doc
    string of sigterm_protection()
    """
    @functools.wraps(fun)
    def inner(*a, **kw):
        with sigterm_protection(*a, **kw):
...
def _fallback_save_function():
    ...
    def save_function(what, path):
        with sigterm_protected_open(path, "wb") as fp:
            dill.dump(what, fp)
...
def _fallback_load_function():
    ...
    def load_function(path):
        with sigterm_protected_open(path, "rb") as fp:
            return dill.load(fp)
...
class CacheItem(object):
    ...
    def load_result(self, cb):
        ...
        path = os.path.join(cb.root_folder, self.rel_path)
        try:
            with sigterm_protection():
                obj = load_function(path)
        except Exception:
            used = "used %s load function" % ("customized" if used_customized_load_function else "fallback")
            raise CacheLoadError("could not load cache content from %s, %s" % (path, used))
        return obj

    def dump(self, path):
        self.last_path = rel_to_cache_root(path)
        with sigterm_protected_open(path, "wb") as fh:
    ...
    def load(path):
        try:
            with sigterm_protected_open(path, "rb") as fh:
                data = fh.read()
        except Exception:
            raise IOError("reading from %s failed" % path)
        try:
    ...
    def _compute_hash(self, key, seen, keep):
        ...
        if id(key) in seen and not isinstance(key, (int, float, bool, long, str, tuple)):
            ...
            if isinstance(key, str):
                ...
                data = str(time.mktime(key.timetuple()))
                ...
                data = self._compute_hash(values, seen, keep)
            else:
                raise Exception("can not compute hash for %s %r" % (type(key), key))
...
class _CacheBuilder(object):
    ...
    def remove_unused_entries(self):
        ...
        for path in glob.glob(os.path.join(self.root_folder, "*", "*.cache_entry")):
        ...
        for path in glob.glob(os.path.join(self.root_folder, "*", "*.cache_entry")):
            ...
            referenced_path = os.path.join(self.root_folder, item.rel_path)
            used_refs.add(referenced_path)
            if isinstance(item, CacheListItem):
                for sub_item in item:
                    referenced_path = os.path.join(self.root_folder, sub_item.rel_path)
                    used_refs.add(referenced_path)

        for path in glob.glob(os.path.join(self.root_folder, "*", "*")):
...
class _CachedFunction(object):
    ...
    def remove_injected_result(self, item):
        ci = self._cache.get(item.hash_code_args)
        if ci is not None:
            os.remove(os.path.join(self.root_folder, ci.last_path))
    ...
    def _write_cache_item(self, hash_code_args, item):
        ...
        path = os.path.join(self.root_folder, self.function.__name__, name)
        item.dump(path)
        return item
    ...
    def _set_use_marker(self, hash_code):
        ...
        try:
            with sigterm_protected_open(marker_path, "w"):
                pass
        except IOError, e:
