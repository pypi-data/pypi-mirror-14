...
def atomic_write(save_function):
    ...
    def inner(what, where):
        ...
        shutil.move(where + ".incomplete", where)
        return result
    return inner
    ...
    def create_save_function(self, cb):
        ...
        def save(what, path):
            ...
            for i, item in enumerate(what):
                ...
                atomic_write(save_function)(item, full_path)
...
class _CacheBuilder(object):
    ...
    def remove_unused_entries(self):
        ...
        for path in glob.glob(join(self.root_folder, "*", "*.cache_entry")):
        ...
        for path in glob.glob(join(self.root_folder, "*", "*")):
