var WlFileBrowser = {
    ...
    // change this
    thumb_prefix: 'thumb_',
    no_thumb: 'wlfilebrowser/img/no_thumb.gif',
    ...
    init: function() {
        ...
        for (var i=0; i<scripts.length; i++) {
            if (scripts[i].src.match(/AddWlFileBrowser/)) {
                var idx = scripts[i].src.indexOf('wlfilebrowser/js/AddWlFileBrowser');
                WlFileBrowser.admin_media_prefix = scripts[i].src.substring(0, idx);
                break;
            }
        }
    },
...
addEvent(window, 'load', WlFileBrowser.init);
