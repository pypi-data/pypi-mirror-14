...
import skbold.data.test_data as test_data
import shutil
import glob
...
def test_fsl2mvp():
    ...
    test_dir = op.dirname(test_data.__file__)
    run1 = op.join(test_dir, 'run1.feat')
    run2 = op.join(test_dir, 'run2.feat')

    true_labels = ['actie', 'actie', 'actie',
                   'interoception', 'interoception', 'interoception',
                   'situation', 'situation', 'situation']
    mvp_dir = op.join(test_dir, 'mvp_data')
