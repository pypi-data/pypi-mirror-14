...
def to_date(val, fmt=None):
  """Convert the value to a date, using arrow."""
  return _to_datetime(val, fmt).date()


def to_datetime(val, fmt=None):
  """Convert the value to a datetime, using arrow."""
  return _to_datetime(val, fmt).datetime


def to_time(val, fmt="HH:mm:ss"):
  """Convert the value to a time, using arrow."""
  return _to_datetime(val, fmt).time()


def _to_datetime(val, fmt=None):
  """Convert the value to a datetime, using arrow."""
  if val is None:
    raise Exception("Invalid date string for format {}: {}".format(fmt, val))
  if fmt is None or not isinstance(val, (str, list, tuple)):
    return arrow.get(val)
  else:
    return arrow.get(val, fmt)
...
def formatted_datetime(fmt):
  """The same as partial(to_datetime(fmt=fmt))."""
  return lambda val: _to_datetime(val, fmt).datetime
