...
class LengthEstimator(object):
	def __init__(self,insert_mu,insert_sigma,
		...
		if trained_gmm is None:
			self._get_read_len = (lambda: self._read_len)
		else:
			self._get_read_len = (lambda: trained_gmm.sample(1).flatten()[0])
	...
	def __get_factor__(self,abs_result):
		...
		if abs_result < (total * 0.001):
			return 1
		 	...
		 	return random.sample([1,2,2,4,25,200], 1)[0]
		elif abs_result < (total * 0.5):
		 	return random.sample([20,50,100,200,1000], 1)[0]
		else:
			return int(log(abs_result)*300) / random.sample([1,2], 1)[0]
	...
	def __simulate_reads__(self,tel_len):
		...
		while total < (obs_total):
			insert_size = int(random.gauss(insert_mean,insert_sigma))
			...
			if is_complete(location,insert_size):
				...
				est_boundary += 1
				total += 1
		return (est_complete,est_boundary)
...
def estimator_process(job,insert_mu,insert_sigma,complete,boundary,read_len,trained_gmm):
	length_estimator = LengthEstimator(insert_mu,insert_sigma,complete,boundary,read_len,trained_gmm)
	results = length_estimator.start()
	return results

def run_simulator_par(insert_mu,insert_sigma,complete,
					  boundary,proc,read_len,trained_gmm,N=10):
...
if __name__ == "__main__":
	print run_simulator_par(195, 56, 9583, 250, 16, 100,N=1)
	#print run_simulator_par(473, 16, 85506, 29366, 16, 100,N=1)
