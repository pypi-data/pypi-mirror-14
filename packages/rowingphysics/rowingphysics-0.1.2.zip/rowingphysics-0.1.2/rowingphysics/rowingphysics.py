...
def main():
    print("Executing rowingphysics version %s." % __version__)
...
def testbladeforce(fhandle,rigging,vb,oarangle=0.01,aantal=10):
   ...
   lout = lscull - lin
   oarangle = oarangle*pi/180.

   Fblade = fhandle*lin/lout 
   res = blade_force(oarangle,rigging,vb,Fblade)
   phidot = res[0]
   
   print Fblade,180*phidot/pi,180.*vb*np.cos(0.01)/(pi*lout)
...
def empirical(datafile,vavg,crew,rigging,tstroke,trecovery,doplot=1):
    ...
    empdata = genfromtxt(datafile, delimiter = ',',skiprows=1)
    emptime = empdata[:,0]
    ...
    Fdrag = drag_eq((Nrowers*mc)+mb,xdot,doprint=0)
...
#      vcstroke = vhand*(1-(handlepos/d))
   ...
   if (recprofile == 1): # oude methode (sinus)
      vhandmax = -pi*d/(2*trecovery)
      vhand = vhandmax*np.sin(pi*(time-time[i])/trecovery)
...
#   Ekinc = 0.5*mc*(xdot+ydot)**2 - 0.5*mc*v0**2
   ...
   Pw = drag_eq((Nrowers*mc)+mb,xdot)*xdot
   Pwmin = drag_eq((Nrowers*mc)+mb,mean(xdot))*mean(xdot)
...
# empirical data
   ...
   if (empirical<>0):
       empdata = genfromtxt(empirical, delimiter = ',',skiprows=1)
       emptime = empdata[:,0]
       if (max(emptime)>10):
...
def energybalance_erg(ratio,crew,erg,w0=4.3801,dt=0.03,doplot=1,doprint=0,theconst=0.1):
...
#   vhandmax = pi*d/(powerconst*tdrive)
#   vhand = vhandmax*(np.sin(pi*(time)/tdrive))**(macht)

   powerconst = 3.1733259127
   vhandmax = pi*d/(powerconst*tdrive)
   vhand = vhandmax*(1-np.cos(2*pi*(time)/tdrive))
...
#	 vcrecovery[k] = vhand
   ...
   Plegdiss = theconst*(abs(Pleg)-Pleg)
   Plegsource = abs(Pleg)

   Parmdiss = theconst*(abs(Parm)-Parm)
   Parmsource = abs(Parm)
...
def energybalance_erg_old(F,crew,erg,w0=4.3801,dt=0.03,doplot=1,doprint=0,
  ...
  # stroke parameters
   ...
   while handlepos<d:
	   ...
	   ydot[i] = sqrt(2*kinerg/(Nrowers*mc))
	   ydotdot[i] = (ydot[i]-ydot[i-1])/dt
	   print ydot[i],ydotdot[i]
	   if ydot[i]>wf[i-1]*r:
	       ydot[i]=wf[i-1]*r
	   dw = dt*(-Tdrag)/inertia
	   wf[i] = wf[i-1]+dw
...
# drag power
   ...
   if (doplot==2):
      ...
      pyplot.plot(time, Phandle,'r-',label = 'Handle power')
      pyplot.plot(time, Pq,'b-',label = 'Kinetic power')
      pyplot.plot(time, Phandle+Pq,'g-',label = 'Total power')
      pylab.legend(loc='upper right')
      pyplot.xlabel("time (s)")
      pyplot.ylabel('power (W)')
      pyplot.show()
...
def constantwatt(watt,crew,rigging,timestep=0.03,aantal=5,
   ...
   for i in range(len(F)):
      ...
      vend = 4
      while (dv/vend > 0.001):
         res = energybalance(F[i],crew,rigging,vend,timestep,0,catchacceler=tcatchacceler)
         dv = res[0]
         vend = res[1]
	 tcatchacceler = res[14]
      res = stroke(F[i],crew,rigging,vend,timestep,10)
...
def constantwatt_erg(watt,crew,erg,timestep=0.03,aantal=5,
                aantal2=5,ratiomin=0.4,ratiomax=0.8,theconst=0.0):
...
def constantwatt_ergdisplay(watt,crew,erg,timestep=0.03,aantal=10,
                aantal2=10,ratiomin=0.3,ratiomax=0.8,theconst=0.0,catchacceler=5):
...
def constantvelo(velo,crew,rigging,timestep=0.03,aantal=5,aantal2=5,Fmin=100,Fmax=400,catchacceler=5):
   ...
   while (dv/vend > 0.001):
      res = energybalance(fres,crew,rigging,vend,timestep,0,catchacceler=tcatchacceler)
      vend = res[1]
      tcatchacceler = res[14]
      dv=res[0]
...
def constantratio(ratio,crew,rigging,timestep=0.03,aantal=5,aantal2=5,Fmin=100,Fmax=400,catchacceler=5):
...
def constantrecovery(trecovery,crew,rigging,timestep=0.03,aantal=5,aantal2=5,Fmin=100,Fmax=400):
...
#   kinpower = (Emaxstroke+Emaxrecovery)/timetotal
   ...
   result = constantwatt_ergdisplay(ergpower,crew,erg)
   totalpower = result[3]
