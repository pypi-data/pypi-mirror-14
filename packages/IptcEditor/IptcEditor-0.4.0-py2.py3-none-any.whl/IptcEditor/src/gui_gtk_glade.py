...
class GuiGtkGlade:
    def __init__(self):
        ...
        with open('{}/VERSION.rst'.format(self.project_root)) as in_file:
            self.app_version = engine.sanitize_filter(in_file.read())[0:5]
    ...
    def on_tagger_keyword_to_replace_input_activate(self, widget, event=None, userdata=None):
        ...
        if keywords_in_dataset:
            self.build_keyword_selector(keywords_list=keywords_list)
    ...
    def set_active_image(self):
        ...
        active_img = self.builder.get_object('active_image')
        ...
        if self.filename and not self.directory:
            # set label
            if current_label == self.builder.get_object('active_dir_img_label'):
                self.switch_label(current_label, self.builder.get_object('active_file_img_label'))
            # set image
            if data_reader.image_extension_checker(self.filename):
                ...
                active_img.set_from_pixbuf(pixbuf)
        # # if directory
        elif self.directory and not self.filename:
            # set label
            if current_label == self.builder.get_object('active_file_img_label'):
                self.switch_label(current_label, self.builder.get_object('active_dir_img_label'))
    ...
    def replace_keywords(self, dataset=None):
        new_keyword = self.builder.get_object('tagger_new_keyword_input').get_text()
        ...
        if self.engine.replace_keywords(dataset=dataset,
                                        ...
                                        new_keyword=new_keyword) is not False:
    ...
    def switch_label(self, old_label, new_label):
        active_box = self.builder.get_object('active_img_box')
        active_img = self.builder.get_object('active_image')
        active_box.remove(old_label)
        active_box.remove(active_img)
        active_box.pack_start(new_label, False, True, 0)
        active_box.pack_start(active_img, True, True, 0)
    ...
    def add_to_log(self, label, incoming=None):
        # if incoming is a list, join into a string
    ...
    def build_keyword_selector(self, keywords_list=None):
        if keywords_list:
            # ensure starting list is empty
            self.keyword_selection_list = Gtk.ListBox()
            self.keyword_selection_viewport.add(self.keyword_selection_list)
            self.keyword_selection_list.set_activate_on_single_click(False)
            self.keyword_selection_list.set_selection_mode(Gtk.SelectionMode.SINGLE)
            for keyword in keywords_list:
                ...
                new_row = Gtk.ListBoxRow()
                new_row.add(new_label)
                self.keyword_selection_list.prepend(new_row)
            self.keyword_selection_list.show_all()
        else:
            return None
