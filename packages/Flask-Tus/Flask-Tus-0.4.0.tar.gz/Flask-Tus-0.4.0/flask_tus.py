...
class tus_manager(object):
    ...
    def __init__(self, app=None, upload_url='/file-upload', upload_folder='uploads/', overwrite=True):
        self.app = app
        if app is not None:
            self.init_app(app, upload_url, upload_folder, overwrite=overwrite)

    def init_app(self, app, upload_url='/file-upload', upload_folder='uploads/', overwrite=True):
    ...
    def tus_file_upload(self):
        ...
        if request.method == 'GET':
            ...
            if metadata.get("filename", None) is not None and os.path.lexists( os.path.join( self.upload_folder, metadata.get("filename") )):
