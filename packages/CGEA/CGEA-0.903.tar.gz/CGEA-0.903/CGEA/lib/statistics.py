...
#         if self.status:
#             self.status[2].value += 1.0/(self.countLists)*3.3/10.0
...
def generatePVals(backgroundModel, scores, setSizes = None, backgroundScores = None, scaler = None, alpha = .05, FDR = None, numWorkers = mp.cpu_count()):
    if not isinstance(backgroundModel, dict):
        ...
        for index, score in enumerate(scores):
            ...
            pvals.append(map(genPval(model.weights_), zip(*model.score_samples(scaler.transform(np.array([score])))))[0])
        FDR = np.mean(FDRs)
...
#             self.status.value += 1.0/(self.numPermutations)*3.0/10.0
