...
def featureBasedCGEA(upgenes, downgenes, saveAs, outputDir, sortDir, data, status, alpha = .05, numWorkers = mp.cpu_count(), pool = None):
    #print 'Start - 0'
    status[1] = 'Started'
    status[2] = 0.01
    curTime = time.time()
    ...
    curTime = time.time()
    [backgroundModel, backgroundScores, scaler] = generateBackgroundModel(geneLists, cmap, status=status, numWorkers = numWorkers, pool = pool)
...
def geneEnrichment(drugAnnotations, data, sortDir = 'up', status = None, numWorkers = mp.cpu_count(), pool = None):
    ...
    if not mixmdls:
        maxSetSize = max(map(len, reduce(lambda x,y: x+y.values(), unifiedCompoundLevelSets.values(), [])))
        print maxSetSize
        makeMixmdl(cmap.keys(), data.getPath('mixmdl'), maxSetSize = maxSetSize)
        mixmdls = data.getModel('mixmdl')
    #print time.time() - curTime
    #curTime = time.time()
    status[2]+=.03
