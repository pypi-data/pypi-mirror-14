...
def fishersBasedCGEA(drugList, saveAs, outputDir, sortDir, data, status, alpha = .05, numWorkers = mp.cpu_count(), verbose = True):
    ...
    drugList = list(itertools.chain(*map(lambda x: re.findall(r"[\w']+", x),
                                         read_file(drugList))))
    ...
    compoundListsFilepath = 'data/CompoundLists_SizeThreshold-'+str(minCompoundSetSizeThreshold)+'.json'
