...
class LengthFramer(AbstractFramer):
    ...
    def next_msg(self) -> ProtocolMessage:
        with self._r_lock:
            ...
            body = self.ioStream.read(body_length)  # type: bytes
            # print("finish msg: %s" % body)
            return ProtocolMessage(fc, req_id, msg_type, body.decode())  # bytes -> str

    def frame_msg(self, message: ProtocolMessage):
        with self._w_lock:
            if message.body is not None and len(message.body.encode()) > protocol.MAX_SIZE:
