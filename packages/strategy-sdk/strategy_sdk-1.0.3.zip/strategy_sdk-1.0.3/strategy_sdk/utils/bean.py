...
class BaseBean(object):
    ...
    def to_dict(self):
        def _get_v_from_type(t, v):
            v_to_dict = getattr(t, "to_dict", None)
        ...
        types = getattr(self.__class__, "_types", None)
        if not types:
            return d
        for k, t in types.items():
            ...
            if issubclass(t, list):
                params = getattr(t, "__parameters__", None)
                if params is None:
                    continue
    ...
    def from_json(self, s: str):
        """use json content to fulfill current instance
        @:param s json str
    ...
    def from_dict(self, d):
        def _get_v_from_constructor(c, v):
            params = getattr(c, "__parameters__", None)
            if params:
                if issubclass(c, list):
                    return [_get_v_from_constructor(params[0], vi) for vi in v]
            handler = None
            if hasattr(c, "from_value"):
                handler = getattr(c, "from_value")
            elif hasattr(c, "from_dict"):
                handler = getattr(c, "from_dict")
            if handler is None:
                return c(v)
            else:
                res = c()
...
def json_load_bean_list(s: str, t, l=None):
    """
