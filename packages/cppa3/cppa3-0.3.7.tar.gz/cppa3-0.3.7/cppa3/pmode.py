...
def payload_profile(cpa, actionbinding):
    payload_profile_id = actionbinding.get('payloadProfileId')
    if payload_profile_id != None:
        payload_profile_list = cpa.xpath(xp_payload_profile.format(payload_profile_id),
                                         namespaces=NSMAP)
...
xp_pullchannel = 'child::cppa:PullChannelId'
...
xp_pullchannel_id = 'child::cppa:PullChannelId/text()'
...
def persistduration(duplicatehandling_config):
    #logging.error(etree.tostring(duplicatehandling_config))
...
xp_encryptionalgorithm = 'child::cppa:DataEncryption/cppa:EncryptionAlgorithm/text()'
...
def return_if_present_else_none(element, xpathexpr, default=None):
    ...
    if len(exprL) == 0:
        if default != None:
            ...
            #logging.debug('No {} and no default, returning None'.format(xpathexpr))
            return None
    else:
        #logging.debug('Found {} returning content'.format(xpathexpr))
        return exprL[0]
...
def process_sender_error_channel(cpa, pmode, leg, errhandl):
    ...
    if sendererrchannelid != None:
        ...
        pmode[leg]['ErrorHandling']['Report']['SenderErrorsTo']
...
def retrieve_certificate(cpa, certid):
    return cpa.xpath('//cppa:Certificate[@id="{}"]//ds:X509Certificate/text()'.format(certid),
                     namespaces=NSMAP)
