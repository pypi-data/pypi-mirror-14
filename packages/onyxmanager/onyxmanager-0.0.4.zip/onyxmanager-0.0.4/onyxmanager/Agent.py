...
def returnOSSlash(module):
    return "\\" if module.getDevice().getFact(module.getDevice().OS, "isWindows") else "/"
...
class Agent:
    def __init__(self, deviceName="", workingDir="", logDir=""):
        ...
        self.workingDir = workingDir if workingDir != "" else (r"C:\onyxmanager" if self.getDevice().getFact(self.getDevice().OS, "isWindows") else "/etc/OnyxManager")
        self.logDir = logDir if logDir != "" else (r"C:\onyxmanager\logs" if self.getDevice().getFact(self.getDevice().OS, "isWindows") else "/var/log/OnyxManager")
        ...
        logging.info("")
        logging.info("OnyxManager v{0} - Started".format("0.0.4"))
    ...
    def loadModules(self):
        ...
        for file in os.listdir(self.getWorkingDir()):
            if file.endswith(".json"):
                try:
                    self.modules[file[:-5]] = Module(file[:-5], self.getWorkingDir()  + returnOSSlash(self) + file, self.getDevice())
    ...
    def dumpDeviceFactsToJSON(self):
        self.getDevice().dumpFactsToJSON(str(self.getWorkingDir()) + returnOSSlash(self) + "device.facts")
        logging.info("Device facts dumped to '{0}'".format(str(self.getWorkingDir()) + returnOSSlash(self) + "device.facts"))