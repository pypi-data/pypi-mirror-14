...
### UTILITY FUNCTIONS  #########################################################
################################################################################
def fast_delete_eids(cnx, etype, eids, chunksize=10000):
    ...
    for i in range(0, len(eids), chunksize):
        ...
        cnx.system_sql('DELETE FROM cw_%s WHERE cw_eid IN (%s)'
        ...
        for table in ('is_relation', 'is_instance_of_relation',
            ...
            cnx.system_sql("DELETE FROM %s WHERE eid_from IN (%s)"
                           % (table, str_eids))
        cnx.system_sql("DELETE FROM entities WHERE eid IN (%s)" % str_eids)
    cnx.commit()

def check_if_table_exists(cnx, table_name, pg_schema='public'):
    sql = "SELECT * from information_schema.tables WHERE table_name=%(t)s AND table_schema=%(s)s"
    crs = cnx.system_sql(sql, {'t': table_name, 's': pg_schema})
    res = crs.fetchall()
    if res:
        return True
    return False

def check_if_primary_key_exists_for_table(cnx, table_name, pg_schema='public'):
...
def get_index_query(cnx, name, pg_schema='public'):
    """Get the request to be used to recreate the index"""
    return cnx.system_sql("SELECT pg_get_indexdef(c.oid) "
...
def get_constraint_query(cnx, name, pg_schema='public'):
    """Get the request to be used to recreate the constraint"""
    return cnx.system_sql("SELECT pg_get_constraintdef(c.oid) "
...
def get_application_indexes(cnx, tablename, pg_schema='public'):
    ...
    indexes_list = cnx.system_sql(sql).fetchall()
    indexes = {}
    for name, type, table in indexes_list:
        indexes[name] = get_index_query(cnx, name, pg_schema)
    return indexes

def get_application_constraints(cnx, tablename, pg_schema='public'):
    ...
    constraints = {}
    for name, type, table in indexes_list:
        query = get_constraint_query(cnx, name, pg_schema)
        constraints[name] = 'ALTER TABLE %s ADD CONSTRAINT %s %s' % (tablename, name, query)
    return constraints

def get_application_indexes_constraints(cnx, tablename, pg_schema='public'):
    """ Get all the indexes/constraints for a given tablename """
    indexes = get_application_indexes(cnx, tablename, pg_schema)
    constraints = get_application_constraints(cnx, tablename, pg_schema)
    _indexes = {}
...
class RelationStoreMixin(object):
    ...
    def __init__(self, cnx, replace_sep='',
                 commit_at_flush=True, iid_maxsize=1024, **kwargs):
        """ Mixin for stores to implement the behavior of relation
        based on external id (e.g. uri)

        - cnx: CubicWeb connection
        ...
        """
        super(RelationStoreMixin, self).__init__(cnx, **kwargs)
    ...
    def init_rtype_table(self, etype_from, rtype, etype_to):
        ...
        if rtype not in self._initialized['uri_rtypes']:
            ...
            if not self._cnx.repo.schema.rschema(rtype).inlined:
                sql = ('CREATE TABLE IF NOT EXISTS %(r)s_relation_iid_tmp '
                       '(uri_from character varying(%(s)s), '
                       ' uri_to character varying(%(s)s))')
    ...
    def _init_uri_eid_table(self, etype):
        ...
        """
        sql = ('CREATE TABLE IF NOT EXISTS uri_eid_%(e)s '
    ...
    def cleanup(self):
        ...
        for rtype in self._initialized['uri_rtypes']:
            if not self._cnx.repo.schema.rschema(rtype).inlined:
                ...
                self.logger.warning("inlined relation %s: no cleanup to be done for it",
                                    rtype)
        self.commit()
...
### MASSIVE STORE  #############################################################
################################################################################
class MassiveObjectStore(RelationStoreMixin, cwdi.RQLObjectStore):
    """
   ...
   It should be used as follows:
    ...
    def __init__(self, cnx, autoflush_metadata=True,
        ...
        - build_entities: Boolean. If True, create_entity returns a CW etype object
          (but WITHOUT eid !).
        """
        super(MassiveObjectStore, self).__init__(cnx, iid_maxsize=iid_maxsize,
    ...
    def _init_dataio_metatables(self):
        ...
        cu = self.sql('SELECT tablename FROM pg_tables WHERE schemaname=%(s)s',
                      {'s': self.pg_schema})
    ...
    def _drop_table_constraints_indexes(self, tablename):
        ...
        indexes, constraints = get_application_indexes_constraints(self._cnx, tablename, self.pg_schema)
    ...
    def reapply_constraint_index(self, tablename):
        if not check_if_table_exists(self._cnx, 'dataio_constraints', self.pg_schema):
    ...
    def init_etype_table(self, etype):
        ...
        if etype not in self._etype_eclass_map_cache:
            ...
            self._etype_eclass_map_cache[etype] = self._cnx.vreg['etypes'].etype_class(etype)
    ...
    def create_entity(self, etype, **kwargs):
        ...
        entity = self._etype_eclass_map_cache[etype](self._cnx)
    ...
    def flush_meta_data(self):
        ...
        if not check_if_table_exists(self._cnx, 'dataio_initialized', self.pg_schema):
    ...
    def cleanup(self):
        ...
        if check_if_table_exists(self._cnx, 'dataio_initialized', self.pg_schema):
        ...
        for table_name in ('dataio_initialized', 'dataio_constraints', 'dataio_metadata'):
            if check_if_table_exists(self._cnx, table_name, self.pg_schema):
    ...
    def insert_massive_meta_data(self, etype):
        ...
        # Meta relation
        source_eid = self._cnx.execute('Any X WHERE X is CWSource')[0][0]
        self.metagen_push_relation(etype, source_eid, 'cw_source_relation')
        source_eid = self._cnx.execute('Any X WHERE X is CWEType, X name %(n)s',
                                       {'n': etype})[0][0]
