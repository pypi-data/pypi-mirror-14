...
<https://docs.python.org/3.4/library/argparse.html#sub-commands>`_: one has to
...
class _SubCommandsDispatch(_SubParsersAction):
    ...
    def add_pattern_executables(self, pattern, *, path=None, ondouble=None):
    ...
    def add_prefix_executables(self, prefix, *, path=None, ondouble=None):
    ...
    def add_function(self, function, command=None, *, help=None, ondouble=None):
    ...
    def add_module(self, module, command=None, *, path=None, help=None, ondouble=None, onerror=None): # pylint: disable=line-too-long
        ...
        :param str help: A brief description of what the subcommand does. If
            `None`, use the first non-empty line of the module docstring.
        :param ondouble: See :ref:`ondouble`. Default is :data:`ERROR`.
        ...
        try:
            with _update_sys(path=path):
                ispkg = _is_package(module)
                if ispkg:
                    module = "{}.__main__".format(module)
        ...
        if command is None:
            if ispkg:
                command = imported.__name__[:-len(".__main__")]
            else:
                command = imported.__name__
        if help is None:
    ...
    def add_submodules(self, module, *, path=None, ondouble=None, onerror=None):
        ...
        for __finder, name, ispkg in pkgutil.iter_modules(subpath):
            if ispkg:
                if importlib.util.find_spec("{}.{}.__main__".format(module, name)) is not None:
                    self.add_module(
                        "{}.{}".format(module, name),
                        command=name,
                        ondouble=ondouble,
