...
try:
    AUTOTASK_IS_ACTIVE = settings.AUTOTASK_IS_ACTIVE
except AttributeError:
    AUTOTASK_IS_ACTIVE = False

# time in seconds to check for the worker:
try:
    WORKER_MONITOR_INTERVALL = settings.AUTOTASK_WORKER_MONITOR_INTERVALL
except AttributeError:
    WORKER_MONITOR_INTERVALL = 5

# path to the executable:
try:
    WORKER_EXECUTABLE = settings.AUTOTASK_WORKER_EXECUTABLE
except AttributeError:
...
class Supervisor(object):
    ...
    def start_worker(self):
        ...
        process = subprocess.Popen(
            [WORKER_EXECUTABLE, 'manage.py', 'run_autotask'],
            env=os.environ,
