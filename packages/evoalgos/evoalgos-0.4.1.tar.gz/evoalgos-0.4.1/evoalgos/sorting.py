...
class CrowdingDistanceSorting(SortingComponent):
    ...
    def identify_groups(self, population):
        """Identify groups of equal fitness.
        ...
        ----------
        population : list of Individual
            The individuals to group.

        Returns
        -------
        groups : list of list
            The population partitioned into fitness levels, sorted
        ...
        for front in fronts:
            ...
            prev_key = cd_dict[front[0]]
            for individual in front:
                curr_key = cd_dict[individual]
                if curr_key == prev_key:
                    groups[-1].append(individual)
                else:
    ...
    def identify_best_group(self, population):
        """Identify the group of best fitness.
        ...
        -------
        best_group : list of Individual
            The group of individuals with highest fitness in the population.

        """
        front = self.non_dom_sorting.identify_best_group(population)
        # groups of individuals with equal objective value
        group = []
        cd_dict = dict()
        self.sort_front(front, cd_dict)
        prev_key = cd_dict[front[0]]
        for individual in front:
            curr_key = cd_dict[individual]
            if curr_key == prev_key:
...
class HyperVolumeContributionSorting(SortingComponent):
    ...
    def sort_front_2d(self, front, hv_contributions=None):
        ...
        if prefer_boundary_points:
            ...
            for individual in front:
                if non_dom_sorting.weakly_dominates(individual, reference_point):
    ...
    def identify_groups(self, population):
        ...
        for front in fronts:
            ...
            prev_key = hv_dict[front[0]]
            for individual in front:
                curr_key = hv_dict[individual]
                if curr_key == prev_key:
                    groups[-1].append(individual)
                else:
                    groups.append([individual])
...
class NearestBetterSorting(SortingComponent):
    ...
    A simple distance function is provided as an example.
    ...
    class DistMatrixFunction:
        ...
        (real-valued, integer, binary). The root is not extracted to save
        some time.
        ...
        def __call__(self, individuals1, individuals2):
            ...
            for individual1 in individuals1:
                ...
                for individual2 in individuals2:
                    ...
                    dist = sum(abs(p1 - p2) ** exponent for p1, p2 in zip(phenome1, phenome2))
                    distances[-1].append(dist)
            return distances


    def __init__(self, dist_matrix_function,
                 archive=None,
                 sorting_component=None,
                 use_neighbor_count=True):
        """Constructor.

        Parameters
        ----------
        dist_matrix_function : callable
            A function which takes two iterables of individuals as input
            (say, sizes n and m), and returns an :math:`(n \\times m)`-matrix
            of distances as output.
        ...
        use_neighbor_count: bool, optional
            ...
            which are closer than the nearest-better neighbor, are counted.
            The sort order is then descending by this number. In any case,
            the order of the sorting component is used as secondary
    ...
    def calc_nearest_better_distances(self, population,
        ...
        equiv_classes = self.sorting_component.identify_groups(population + others)
        sorted_individuals = []
        for i, cls in enumerate(equiv_classes):
            for individual in cls:
                order_dict[individual] = i
        ...
        for i, individual in enumerate(sorted_individuals):
            if individual not in population_set:
                continue
            current_order = order_dict[individual]
            maybe_better_inds = sorted_individuals[:i]
            if len(maybe_better_inds) > 0:
                distances = dist_matrix_function([individual], maybe_better_inds)[0]
                min_dist = INFINITY
                for other_ind, dist in zip(maybe_better_inds, distances):
                    if dist < min_dist and order_dict[other_ind] < current_order:
                        min_dist = dist
    ...
    def count_neighbors_closer_than_nbn(self, population,
        ...
        all_distances = self.dist_matrix_function(population, population + others)
    ...
    def _process(self, population, others=None):
        ...
        others : list of Individual, optional
            An archive of individuals. By default empty.

        Returns
        -------
        min_distances : dict
            A dictionary containing the nearest-better distances
        neighbor_counts : dict
            A dictionary containing the neighbor counts.
        order : dict
            A dictionary containing the order given by the internal
            sorting component.
        sort_key : callable
            A function for obtaining a sort key from an individual.
        ...
        if self.use_neighbor_count:
            neighbor_counts = self.count_neighbors_closer_than_nbn(population,
                                                                   others,
                                                                   order,
                                                                   min_distances)

            def sort_key(individual):
                return -neighbor_counts[individual], order[individual]
        else:
            min_distances = self.calc_nearest_better_distances(population,
                                                               others,
                                                               order)
            ...
            def sort_key(individual):
                return -min_distances[individual], order[individual]

        return min_distances, neighbor_counts, order, sort_key


    def sort(self, population):
        """Sort the population in place.

        The sort order depends on the options chosen at construction time.
        ...
        """
        result = self._process(population, self.archive)
        _, _, _, sort_key = result
    ...
    def identify_groups(self, population):
        ...
        """
        population_copy = population[:]
        result = self._process(population_copy, self.archive)
        _, _, _, sort_key = result
