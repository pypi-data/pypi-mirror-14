...
class QualityIndicator(object):
    ...
    def assess_non_dom_front(self, front):
        ...
        Parameters
        ----------
        front : list of Individual
            A list of individuals with the special property that no
            individual is dominated by any other regarding Pareto-dominance.
...
class FonsecaHyperVolume(HyperVolumeIndicator):
    ...
    def hv_recursive(self, dim_index, length, bounds):
        ...
        In contrast to [Fonseca2006]_, the code assumes that the reference
        point is [0, ..., 0]. This allows the avoidance of a few operations.
