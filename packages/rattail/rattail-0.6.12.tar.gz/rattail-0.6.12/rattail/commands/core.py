    ...
    * ``rattail datasync stop``
    ...
    def run(self, args):
        ...
        if args.subcommand == 'wait':
            ...
            count = changes.count()
            log.debug("there are {0} changes in the queue".format(count))
            while count:
                try:
                    now = datetime.datetime.utcnow()
                    if (now - last_logged).seconds >= 60:
                        log.debug("still waiting, {0} changes in the queue".format(count))
                        last_logged = now
                    time.sleep(1)
                    count = changes.count()
                except KeyboardInterrupt:
...
class ImportRattail(NewImportSubcommand):
    ...
    def get_handler_factory(self, args):
        spec = self.config.get('rattail.importing', 'rattail.handler',
                               default='rattail.db.newimporting.rattail:RattailImportHandler')
