...
def test_choice_types_contain_choices(complete_parser):
  results = convert(complete_parser)['widgets']['primary']['contents']
  for item in results:
    if item['type'] == 'Dropdown':
      assert len(item['data']['choices']) > 0
      assert item['data']['choices'] == ['yes', 'no']
