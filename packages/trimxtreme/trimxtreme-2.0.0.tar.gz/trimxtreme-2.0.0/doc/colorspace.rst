...
Trimxtreme was designed to work with colorspace reads from the ABi SOLiD
...
3. Example input files are in the trimxtreme distribution at
``tests/data/solid.*``. The ``.csfasta``/``.qual`` file format is
automatically assumed if two input files are given to trimxtreme.
...
``solid.qual``, use this command-line::
    ...
    trimxtreme -c -a CGCCTTGGCCGTACAGCAG solid.csfasta solid.qual > output.fastq
...
trimming the adapter ``AACTC``, trimxtreme searches for its
...
output.fastq::
    ...
    trimxtreme --bwa -a CGCCTTGGCCGTACAGCAG solid.csfasta solid.qual > output.fastq
...
how to use a different error rate::
    ...
    trimxtreme --bwa -e 0.15 -a 330201030313112312 -o output.fastq solid.csfasta solid.qual
...
them::
    ...
    trimxtreme -c -e 0.15 -a 330201030313112312 -x abc: --strip-f3 solid.csfasta solid.qual > output.fastq
...
BWA also has a problem with such data. Trimxtreme therefore converts
