...
* Renamed trimxtreme to keep fork separate upon request by author.
...
* Support single-pass paired-end trimming with the new ``-A``/``-G``/``-B``/``-U``
  ...
  documentation <http://trimxtreme.readthedocs.org/en/latest/guide.html#paired-end>`_
  for details.
...
* Support for demultiplexing was added. This means that reads can be written to
  ...
  documentation <http://trimxtreme.readthedocs.org/en/latest/guide.html#demultiplexing>`_
...
* Options ``--minimum-length``/``--maximum-length`` (``-m``/``-M``) can be used
  standalone. That is, trimxtreme can be used to filter reads by length without
  trimming adapters.
...
* Extensive work on the documentation. It's now available at
  https://trimxtreme.readthedocs.org/ .
...
* Galaxy support files are now in `a separate
  repository <https://bitbucket.org/lance_parsons/trimxtreme_galaxy_wrapper>`_.
...
* When the new option ``--quiet`` is used, no report is printed after all reads
  have been processed.
* When processing paired-end reads, trimxtreme now checks whether the reads are
  properly paired.
...
v1.4
----
* This release of trimxtreme reduces the overhead of reading and writing files.
  On my test data set, a typical run of trimxtreme (with a single adapter) takes
...
* When using trimxtreme without installing it, you now need to run ``bin/trimxtreme`` due to
...
* Due to trimxtreme being published in EMBnet.journal, I found it appropriate
...
v0.9.2
------
* Install a single ``trimxtreme`` Python package instead of multiple Python
...
v0.5
----
* add ``--discard`` option which makes trimxtreme discard reads in which an adapter occurs
