...
class ChangePhoneView(APIView, FormView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if self.form.is_valid():
            ...
            if QuickPhoneNumber.objects.filter(user=request.user, phone=phone, verified=True).exists():
                return Response(ApiStatus(CHANGE_PHONE_NUMBER_EXIST).get_json(), status=status.HTTP_400_BAD_REQUEST)

            phone_number, created = self.get_or_create_object(request.user, phone)
    ...
    def get_or_create_object(self, user, phone):
        queryset = QuickPhoneNumber.objects.filter(user=user, primary=True)

        if queryset and len(queryset) > 0:
            return queryset[0], False
        else:
            obj = QuickPhoneNumber.objects.create(user=user, phone=phone)
            return obj, True
