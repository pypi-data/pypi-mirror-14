...
class PhaseUpdate(hook.Hook):
    ...
    def __call__(self):
        ...
        if not getattr(repo, 'has_review', False): # not there during migration
            return
        rev = self.entity
...
class RevisionAdded(hook.Hook):
    ...
    __select__ = hook.Hook.__select__ & hook.match_rtype('from_repository')
    events = ('after_add_relation',)

    def __call__(self):
        repo = self._cw.entity_from_eid(self.eidto)
        if getattr(repo, 'has_review', False): # not there during migration
            operation = AttachToPatchesOp.get_instance(self._cw)
            operation.add_data((self.eidto, self.eidfrom))
    ...
    def precommit_event(self):
        ...
        for eid_repo, eid_patch, eid_rev in sorted(self.get_data()):
            ...
            if visible_tips:
                ...
                def ask_review(msg, msg_format=u'text/plain'):
                    """Trigger ask-review transition only if patch is not WIP."""
                    if not re.search(r'\b(wip|WIP)\b', rev.description):
                        wfable.fire_transition('ask review', msg, msg_format)

                # defaults to auto ask review
                autoreview = auto and auto[0][0] == "1"
                new_version_msg = u'new version %s' % rev.changeset
                if autoreview:
                    if wfable.state != 'pending-review':
                        ask_review(new_version_msg)
                else:
                    if wfable.state == 'outdated':
                        wfable.fire_transition('reopen', new_version_msg)
                    elif wfable.state == 'reviewed':
                        if ancestor is not None:
                            new_version_msg += u' (different from :eid:`%d:%s`)' % (
                                ancestor.eid, ancestor.changeset)
                        ask_review(new_version_msg, u'text/rest')
