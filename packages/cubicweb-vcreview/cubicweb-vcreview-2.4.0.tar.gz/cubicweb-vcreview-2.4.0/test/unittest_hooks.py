...
class PatchLifeCycleTC(HgHelperTC):
    ...
    def test_no_askreview_for_wip_patch(self):
        with self.admin_access.repo_cnx() as cnx:
            babar = self.create_user(cnx, 'Babar', ('users', 'reviewers'),
                                     email=u'babar@ratax.es')
            cnx.create_entity('CWProperty', pkey=u"vcreview.autoreview", value=u"1",
                              for_user=babar)
            cnx.commit()
        ...
        a = self.hg_append_to
        a('jungle', 'Babar')
        hg.commit('Initial jungle content', user='babar <babar@ratax.es>')
        a('jungle', 'Celestine or Celeste?')
        hg.commit('Add the queen (WIP)', user='babar <babar@ratax.es>')
        self.refresh()
        with self.admin_access.client_cnx() as cnx:
            rset = cnx.execute('Any D,S WHERE P patch_revision REV, '
                               'REV description D, P in_state ST, ST name S')
        self.assertItemsEqual(rset,
                              [[u'Initial jungle content', u'pending-review'],
                               [u'Add the queen (WIP)', u'in-progress']])
...
class RepositoryNotificationTC(HgHelperTC):
    ...
    def test_repo_nosy_list(self):
        ...
        hg.commit('refresh', user='Cornelius', date='2012-01-01 00:00 +0300')
        self.refresh()
        with self.admin_access.client_cnx() as cnx:
            self.repoeid = cnx.find('Repository')[0][0]
            self.patcheid = cnx.find('Patch', patch_name='refresh')[0][0]
    ...
    def _check_new_patch_notification(self, mail):
        ...
        self.assertEqual(mail.subject,
                         '[mercurial repository #%s] new patch: refresh' % self.repoeid)
        self.assertMultiLineEqual(
            mail.message.get_payload(decode=True),
            '\n'.join([
                'url: http://testing.fr/cubicweb/patch/%s' % self.patcheid,
                '',
                '# HG changeset patch',
                '# User Cornelius',
...
class PatchNotificationTC(HGRCMixin, CubicWebTC):
    ...
    def test_no_notification_from_outdated(self):
        with self.admin_access.client_cnx() as cnx:
            rql = 'Any R ORDERBY R DESC WHERE R is Revision'
            rev = cnx.execute(rql).get_entity(1, 0)
            assert rev.description == 'Add more elephants children'
            ...
            self.assertEqual(iwf.state, 'outdated')
            MAILBOX[:] = []
            patch.cw_adapt_to('IWorkflowable').fire_transition('ask review')
            cnx.commit()
