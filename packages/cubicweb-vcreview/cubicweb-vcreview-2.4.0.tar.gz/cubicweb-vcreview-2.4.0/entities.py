...
if cwversion >= (3, 22):
    import pytz
    min_creation_date = datetime.min.replace(tzinfo=pytz.UTC)
else:
    min_creation_date = datetime.min
...
class PatchReviewBehaviour(EntityAdapter):
    ...
    def set_reviewers(self):
        ...
        for ueid,_ in sorted(self.reviewers_rset(), key=lambda row: row[1] or min_creation_date):
            if ueid in authors:
                continue
