...
class is_in_state_not_from(is_in_state):
    """Like `cubicweb.predicates.is_in_state` filtering on the previous state
    using `not_from` parameter.
    """
    def __init__(self, *expected, **kwargs):
        super(is_in_state_not_from, self).__init__(*expected)
        self.not_from = kwargs.get('not_from', ())

    def _score(self, adapted):
        score = super(is_in_state_not_from, self)._score(adapted)
        if score:
            trinfo = adapted.latest_trinfo()
            if trinfo.previous_state.name in self.not_from:
                return 0
        return score


class PatchStatusChangeView(StatusChangeMixIn, NotificationView):
    __select__ = is_instance('Patch') & is_in_state_not_from('pending-review',
                                                             'in-progress',
                                                             'reviewed',
                                                             'applied',
                                                             'rejected',
