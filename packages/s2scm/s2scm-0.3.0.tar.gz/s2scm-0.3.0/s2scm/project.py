...
def env_to_args(env):
    ...
    for i, env_item in enumerate(env):
        if i == len(env) - 1:
            lst.append(env_item[1])
        else:
            lst.append(kurt.Block('getParam', env_item[0].name))
    return lst
...
def special_form_define(sexpr, sprite, script = None):
    ...
    cont_call = kurt.Block(cont_type, *env_to_args(current_env))
...
def compile_sexpr(sexpr, sprite, script = None):
    ...
    if isinstance(sexpr, list) and sexpr != []:
        ...
        return kurt.Block('getParam', sexpr.str)
    else:
        return sexpr
