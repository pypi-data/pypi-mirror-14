...
from PyQt5 import QtCore, QtGui, Qt, QtWidgets, QtQuickWidgets
import numpy as np
...
class QmlCanvasWidget(QtQuickWidgets.QQuickWidget):

    resized = QtCore.pyqtSignal(int, int)

    def __init__(self, parent, verticalScaleTransform, horizontalScaleTransform):
        super(QmlCanvasWidget, self).__init__(parent)

        self.setSource(QtCore.QUrl("qrc:/qml/canvas.qml"))
    ...
    def paintGL(self):
        ...
        print(painter.font().name)
        painter.setFont(QtGui.QFont("Calibri"))
