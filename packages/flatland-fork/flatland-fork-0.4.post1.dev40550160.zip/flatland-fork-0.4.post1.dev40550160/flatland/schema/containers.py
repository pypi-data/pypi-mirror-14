...
class Mapping(Container, OrderedDict):
    ...
    def _reset(self):
        ...
        for member_schema in self.field_schema:
            ...
            super(Mapping, self).__setitem__(key, member_schema(parent=self))
    ...
    def _set_flat(self, pairs, sep):
        ...
        for schema in self.field_schema:
            ...
            if accum:
                if super(Mapping, self).__contains__(field):
    ...
    def value(self):
        ...
        return OrderedDict((key, value.value)
                           for key, value in self.iteritems())
    ...
    def field_schema_mapping(instance, cls):
        ...
        return OrderedDict((schema.name, schema) for schema in field_schema)
...
class Dict(Mapping):
                ...
                if policy != 'duck':
                    raise KeyError(
                        'Dict %r schema does not allow key %r' % (
                            self.name, key))
                continue
            if super(Dict, self).__contains__(key):
                converted &= self[key].set(value)
            else:
    ...
    def set_by_object(self, obj, include=None, omit=None, rename=None):
        ...
        *obj*.  Object attributes that do not correspond to field names are
        ...
        final = OrderedDict((key, value) for key, value in sliced
                            if key in fields)
        self.set(final)
    ...
    def slice(self, include=None, omit=None, rename=None, key=None):
        ...
        return OrderedDict(keyslice_pairs(((key, element.value)
                                           for key, element in
                                           self.iteritems()), include=include,
                                          omit=omit, rename=rename, key=key))
...
class SparseDict(Dict):
    ...
    def _reset(self):
        super(SparseDict, self).clear()
        for member_schema in self.field_schema:
            key = member_schema.name
            if self.minimum_fields is None or member_schema.optional:
                continue
            super(SparseDict, self).__setitem__(key,
                                                member_schema(parent=self))

    def __setitem__(self, key, value):
        schema = self._field_schema_for(key)
        if not super(SparseDict, self).__contains__(key):
            if schema is None:
                raise TypeError('May not set unknown key %r on %s %r' %
                                (key, type(self).__name__, self.name))
                ...
                return
            super(SparseDict, self).__setitem__(key, schema(value,
                                                             parent=self))
        elif isinstance(value, schema):
            value.parent = self
            super(SparseDict, self).__setitem__(key, value)
    ...
    def __delitem__(self, key):
        if self.minimum_fields is None:
            try:
                super(SparseDict, self).__delitem__(key)
                return
            except KeyError:
                if not self.may_contain(key):
                    raise TypeError(
    ...
    def pop(self, key):
        ...
        if self.minimum_fields == 'required' and not self[key].optional:
            raise TypeError('May not pop required key %r on %s %r' %
                            (key, type(self).__name__, self.name))
        return super(SparseDict, self).pop(key)
