...
def rv(t_obs, radius_1, radius_2, sbratio, incl, 
       t_zero = 0, period = 1,
       a = None,
       q = 1,
       f_c = None, f_s = None,
  ...
  ipar = np.array([n1,n2,n_spots_1,n_spots_2,l1,l2,s1,s2,rvflux_flag],dtype=int)
  ...
  if vsini_2 is not None : par[36] = vsini_1
  ...
  for j in range(0,len(t_calc)):
    rv_1[i_calc[j]] += lc_rv_flags[j,3]*w_calc[j]
  ...
  if n_obs_0 > 0 :
    ...
    rv_1[n_int_array == 0 ] = np.interp(t_obs_0,t_int,f_int)

  rv_2 = np.zeros(n_obs)
  for j in range(0,len(t_calc)):
    rv_2[i_calc[j]] += lc_rv_flags[j,4]*w_calc[j]
  t_obs_0 = t_obs_array[n_int_array == 0 ] # Points to be interpolated
  n_obs_0 = len(t_obs_0)
  if n_obs_0 > 0 :
    i_sort = np.argsort(t_calc)
    t_int = t_calc[i_sort]
    f_int = lc_rv_flags[i_sort,0]
    rv_2[n_int_array == 0 ] = np.interp(t_obs_0,t_int,f_int)
