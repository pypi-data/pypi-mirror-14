...
def _lnlike(varpar_v, lcdata, varpar_l, fixpar_d, priors, ldy_, 
  ...
  # Use simplified heating model with same coeff for both stars
  heat_1 = A_1
  heat_2 = A_2
...
"""
 ...
 The output file is a numpy array containing the parameters at each step of
 the chain with the lnprobability value at 
"""
...
def main():
  ...
  print('\n Bands: uS,vS,bS,yS (Stromgren)')
  print('        UJ, BJ, VJ, RJ, IJ, JJ, HJ, KJ (Johnson)')
  print('        u_, g_, r_, i_ ,z_ (SDSS)')
 ...
 # Could probably tidy this up by iterating over list of varaibles - oh well..
  ...
  t_fit = (lcdata[lcdata['flag'] >= 0])['time']
  n_fit = len(t_fit)
  n_df = n_fit + n_prior - ndim
  print(' Degrees of freedom (including priors) =  ',n_df)
