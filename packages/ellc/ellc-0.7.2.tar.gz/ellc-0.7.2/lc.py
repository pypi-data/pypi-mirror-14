...
def lc(t_obs, radius_1, radius_2, sbratio, incl, 
       ...
       spots_1=None, spots_2=None, verbose=1,debug=None):
  """
  ...
  if debug :
    ...
    t=Table()
    t.add_column(Column(t_calc,name='t_calc'))
    t.add_column(Column(i_calc,name='i_calc'))
    t.add_column(Column(w_calc,name='w_calc'))
    t.add_column(Column(lc_rv_flags[:,0],name='f_calc'))
    t.write('debug.csv')
