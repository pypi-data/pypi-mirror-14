...
class MainNotebook(Gtk.Window):
	...
	def updateLendList(self):
		...
		try:
			self.lendingwindow.add(treeview)
		except:
			pass	
...
class MainBox(Gtk.Window):
	...
	def __init__(self):
		...
		self.label = Gtk.Label("Please enter valid Roll No.")
...
dpath = os.path.dirname(os.path.abspath(notes.__file__))+'/'

if os.path.exists(dpath+".info"):
	fi=open(dpath+".info",'r+')
else:
	fi=open(dpath+".info","w+")	
line=fi.readline()
if os.path.exists(dpath+".download"):
	fd=open(dpath+".download", "r+")
else:
	fd=open(dpath+".download","w+")
def mainProject():

	global dpath = os.path.dirname(os.path.abspath(notes.__file__))+'/'
	if line == '':
