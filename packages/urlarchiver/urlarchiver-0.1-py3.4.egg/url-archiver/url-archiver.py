#!/usr/bin/env python
"""url-archiver is a simple library to fetch and archive URL."""
...
class Archive:
    ...
    """url-archive Archive is base class to fetch and store URL in a raw format.
    ...
    def __init__(self, archive_path=False, debug=False, meta=True):
        if not archive_path:
            self.archive_path = os.path.join(os.path.split(os.path.realpath(__file__))[0], ".url-archiver")
        else:
            self.archive_path = os.path.join(archive_path, ".url-archive")
        if debug:
            self.debug = True
    ...
    def fetch(self, url=False):
        ...
        if not self._archived(v=urlhash):
            ...
            if fetcher.status_code == 200:
                ...
                meta['url-archiver:url'] = normalizedurl
                meta['url-archiver:urlhash'] = urlhash
                meta['url-archiver:version'] = __version__
