...
def address_to_mangopay_address(address, city, region, postal_code, country):
    if address in [None, ""] or city in [None, ""] or postal_code in [0, None] or country in [None, ""]:
        return None
    mangopay_address = Address()
    mangopay_address.AddressLine1=address
    mangopay_address.City=city
    mangopay_address.Region=region
    mangopay_address.PostalCode=postal_code
    mangopay_address.Country=country
...
class MangoPayNaturalUser(MangoPayUser):
    ...
    def _build(self):
        ...
        mangopay_user.Address = address_to_mangopay_address(self.address, self.city, self.region, self.postal_code, self.country_code)
...
class MangoPayLegalUser(MangoPayUser):
    ...
    def _build(self):
        ...
        mangopay_user.HeadquartersAddress = address_to_mangopay_address(self.headquarters_address, self.headquarters_city, self.headquarters_region, self.headquarters_postal_code, self.headquarters_country_code)
        mangopay_user.LegalRepresentativeFirstName = self.first_name
        mangopay_user.LegalRepresentativeLastName = self.last_name
        mangopay_user.LegalRepresentativeAddress = address_to_mangopay_address(self.address, self.city, self.region, self.postal_code, self.country_code)
...
class MangoPayBankAccount(models.Model):
    ...
    def create(self):
        ...
        mangopay_bank_account.OwnerAddress = address_to_mangopay_address(self.address, self.city, self.region, self.postal_code, self.country_code)
        ...
        if self.country_code:
            mangopay_bank_account.Details.Country = self.country_code
        else:
            if self.account_type != BA_BIC_IBAN:
...
class MangoPayWallet(models.Model):
    ...
    def balance(self):
        wallet = self._get()
        return (wallet.Balance.Amount / 100, wallet.Balance.Currency)
...
class MangoPayPayIn(models.Model):
    ...
    def create(self, tag=None):
        ...
        pay_in.DebitedFunds = amount_and_currency_to_mangopay_money(self.debited_funds, self.currency)
        pay_in.Fees = amount_and_currency_to_mangopay_money(self.fees, self.currency)
    ...
    def _update(self, pay_in):
        ...
        self.result_message = pay_in.ResultMessage
        self.save()
        return self
...
class MangoPayPayInBankWire(MangoPayPayIn):
    ...
    def _get_payment_details(self):
        ...
        payment_details.DeclaredDebitedFunds = amount_and_currency_to_mangopay_money(self.debited_funds, self.currency)
        payment_details.DeclaredFees = amount_and_currency_to_mangopay_money(self.fees, self.currency)
...
class MangoPayPayOut(models.Model):
    ...
    def create(self, tag=''):
        ...
        pay_out.DebitedFunds = amount_and_currency_to_mangopay_money(self.debited_funds, self.currency)
        pay_out.Fees = amount_and_currency_to_mangopay_money(self.fees, self.currency)
...
class MangoPayCardRegistration(models.Model):
    ...
    def create(self, currency, card_type=None):
        client = get_mangopay_api_client()
...
class MangoPayRefund(models.Model):
    ...
    def create_advanced(self):
        ...
        refund = Refund()
        refund.InitialTransactionId = pay_in_id
        refund.DebitedFunds = amount_and_currency_to_mangopay_money(self.debited_funds, self.mangopay_pay_in.currency)
        refund.Fees = amount_and_currency_to_mangopay_money(self.fees, self.mangopay_pay_in.currency)
        refund.AuthorId = self.mangopay_user.mangopay_id
        client = get_mangopay_api_client()
        created_refund = client.payIns.CreateRefund(pay_in_id, refund)
        self.status = created_refund.Status
...
class MangoPayTransfer(models.Model):
    ...
    def create(self):
        ...
        transfer.DebitedFunds = amount_and_currency_to_mangopay_money(self.debited_funds, self.currency)
        transfer.Fees = amount_and_currency_to_mangopay_money(self.fees, self.currency)
        client = get_mangopay_api_client()
    ...
    def __unicode__(self):
        return "Debited wallet's owner %s %s" % (self.mangopay_debited_wallet.mangopay_user.first_name, self.mangopay_debited_wallet.mangopay_user.last_name)
