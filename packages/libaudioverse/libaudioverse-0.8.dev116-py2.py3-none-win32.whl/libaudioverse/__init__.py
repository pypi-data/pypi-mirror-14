...
class GenericNode(_HandleComparer):
    ...
    def init_with_handle(self, handle):
        with _object_states_lock:
            ...
            self._property_instances[_libaudioverse.Lav_NODE_CHANNEL_INTERPRETATION] = IntProperty(handle = self.handle, slot = _libaudioverse.Lav_NODE_CHANNEL_INTERPRETATION, enum = ChannelInterpretations)
            ...
            self._property_instances[_libaudioverse.Lav_NODE_STATE] = IntProperty(handle = self.handle, slot = _libaudioverse.Lav_NODE_STATE, enum = NodeStates)
...
Any functionality to change a property on all sources needs to be implemented by the app, and is not offered by Libaudioverse."""
    ...
    def init_with_handle(self, handle):
        with _object_states_lock:
            ...
            self._property_instances[_libaudioverse.Lav_ENVIRONMENT_DISTANCE_MODEL] = IntProperty(handle = self.handle, slot = _libaudioverse.Lav_ENVIRONMENT_DISTANCE_MODEL, enum = DistanceModels)
            ...
            self._property_instances[_libaudioverse.Lav_ENVIRONMENT_PANNING_STRATEGY] = IntProperty(handle = self.handle, slot = _libaudioverse.Lav_ENVIRONMENT_PANNING_STRATEGY, enum = PanningStrategies)
...
If you are in a garbage collected language, failure to hold on to the source nodes will cause them to go silent."""
    ...
    def init_with_handle(self, handle):
        with _object_states_lock:
            ...
            self._property_instances[_libaudioverse.Lav_SOURCE_DISTANCE_MODEL] = IntProperty(handle = self.handle, slot = _libaudioverse.Lav_SOURCE_DISTANCE_MODEL, enum = DistanceModels)
            ...
            self._property_instances[_libaudioverse.Lav_SOURCE_PANNING_STRATEGY] = IntProperty(handle = self.handle, slot = _libaudioverse.Lav_SOURCE_PANNING_STRATEGY, enum = PanningStrategies)
...
Called once per block and outside the audio thread when there is less than the specified threshold audio remaining."""
        with self._lock:
            ...
            wrapper = _CallbackWrapper(self, callback, additional_args, additional_kwargs)
            ctypes_callback = _libaudioverse.LavPushNodeLowCallback(wrapper)
...
Called exactly once and outside the audio thread when the node runs out of audio completely."""
        with self._lock:
            ...
            if additional_kwargs is None:
                additional_kwargs = dict()
            wrapper = _CallbackWrapper(self, callback, additional_args, additional_kwargs)
            ctypes_callback = _libaudioverse.LavPushNodeUnderrunCallback(wrapper)
...
http://www.musicdsp.org/files/Audio-EQ-Cookbook.txt"""
    ...
    def init_with_handle(self, handle):
        with _object_states_lock:
            ...
            self._property_instances[_libaudioverse.Lav_BIQUAD_FILTER_TYPE] = IntProperty(handle = self.handle, slot = _libaudioverse.Lav_BIQUAD_FILTER_TYPE, enum = BiquadTypes)
...
Capabilities include switching from HRTF to stereo and back, a useful property for games wherein the user might or might not be using headphones."""
    ...
    def init_with_handle(self, handle):
        with _object_states_lock:
            ...
            self._property_instances[_libaudioverse.Lav_PANNER_STRATEGY] = IntProperty(handle = self.handle, slot = _libaudioverse.Lav_PANNER_STRATEGY, enum = PanningStrategies)
...
class NoiseNode(GenericNode):
    ...
    def init_with_handle(self, handle):
        with _object_states_lock:
            ...
            self._property_instances[_libaudioverse.Lav_NOISE_NOISE_TYPE] = IntProperty(handle = self.handle, slot = _libaudioverse.Lav_NOISE_NOISE_TYPE, enum = NoiseTypes)
...
Called outside the audio threads every time the buffer reaches the end of the audio data."""
        with self._lock:
            ...
            if additional_kwargs is None:
                additional_kwargs = dict()
            wrapper = _CallbackWrapper(self, callback, additional_args, additional_kwargs)
            ctypes_callback = _libaudioverse.LavBufferNodeEndCallback(wrapper)
...
This node is equivalent to the delay line in the Karplus-strong algorithm."""
    ...
    def init_with_handle(self, handle):
        with _object_states_lock:
            ...
            self._property_instances[_libaudioverse.Lav_FILTERED_DELAY_FILTER_TYPE] = IntProperty(handle = self.handle, slot = _libaudioverse.Lav_FILTERED_DELAY_FILTER_TYPE, enum = BiquadTypes)
...
Called outside the audio thread when the currently scheduled crossfade finishes."""
        with self._lock:
            ...
            if additional_kwargs is None:
                additional_kwargs = dict()
            wrapper = _CallbackWrapper(self, callback, additional_args, additional_kwargs)
            ctypes_callback = _libaudioverse.LavCrossfaderNodeFinishedCallback(wrapper)
...
When called, ended will be set to true,."""
        with self._lock:
            ...
            if additional_kwargs is None:
                additional_kwargs = dict()
            wrapper = _CallbackWrapper(self, callback, additional_args, additional_kwargs)
            ctypes_callback = _libaudioverse.LavFileStreamerNodeEndCallback(wrapper)
