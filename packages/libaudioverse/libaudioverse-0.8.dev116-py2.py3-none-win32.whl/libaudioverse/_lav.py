...
def is_initialized():
    ...
    err = _libaudioverse.Lav_isInitialized(
...
def error_get_message():
    ...
    err = _libaudioverse.Lav_errorGetMessage(
        ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return 
    destination.value.decode('utf8') #All strings are contractually UTF8 when returned to us.
...
def error_get_file():
    ...
    err = _libaudioverse.Lav_errorGetFile(
        ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return 
...
def error_get_line():
    ...
    err = _libaudioverse.Lav_errorGetLine(
        ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return getattr(destination, 'value', destination)
...
def handle_get_and_clear_first_access(handle):
    ...
    err = _libaudioverse.Lav_handleGetAndClearFirstAccess(handle, 
        ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return getattr(destination, 'value', destination)
...
def handle_get_ref_count(handle):
    ...
    err = _libaudioverse.Lav_handleGetRefCount(handle, 
        ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return getattr(destination, 'value', destination)
...
def handle_get_type(handle):
    ...
    err = _libaudioverse.Lav_handleGetType(handle, 
        ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return getattr(destination, 'value', destination)
...
def get_logging_callback():
    ...
    err = _libaudioverse.Lav_getLoggingCallback(
        ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return getattr(destination, 'value', destination)
...
def get_logging_level():
    ...
    err = _libaudioverse.Lav_getLoggingLevel(
        ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return getattr(destination, 'value', destination)
...
def device_get_count():
    ...
    err = _libaudioverse.Lav_deviceGetCount(
        ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return getattr(destination, 'value', destination)
...
def device_get_name(index):
    ...
    err = _libaudioverse.Lav_deviceGetName(index, 
        ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return 
...
def device_get_channels(index):
    ...
    err = _libaudioverse.Lav_deviceGetChannels(index, 
        ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return getattr(destination, 'value', destination)
...
def create_simulation(sr, blockSize):
    ...
    err = _libaudioverse.Lav_createSimulation(sr, blockSize, 
        ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
...
def simulation_get_block_size(simulationHandle):
    ...
    err = _libaudioverse.Lav_simulationGetBlockSize(simulationHandle, 
        ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return getattr(destination, 'value', destination)
...
def simulation_get_block(simulationHandle, channels, mayApplyMixingMatrix, buffer):
    ...
    err = _libaudioverse.Lav_simulationGetBlock(simulationHandle, channels, mayApplyMixingMatrix, buffer)
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
...
def simulation_get_sr(simulationHandle):
    ...
    err = _libaudioverse.Lav_simulationGetSr(simulationHandle, 
        ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return getattr(destination, 'value', destination)
...
def simulation_set_output_device(simulationHandle, index, channels, minLatency, startLatency, maxLatency):
    ...
    err = _libaudioverse.Lav_simulationSetOutputDevice(simulationHandle, index, channels, minLatency, startLatency, maxLatency)
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
...
def simulation_set_block_callback(simulationHandle, callback, userdata):
    ...
    err = _libaudioverse.Lav_simulationSetBlockCallback(simulationHandle, callback, userdata)
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
...
def simulation_write_file(simulationHandle, path, channels, duration, mayApplyMixingMatrix):
    ...
    err = _libaudioverse.Lav_simulationWriteFile(simulationHandle, path, channels, duration, mayApplyMixingMatrix)
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
...
def simulation_get_threads(simulationHandle):
    ...
    err = _libaudioverse.Lav_simulationGetThreads(simulationHandle, 
        ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return getattr(destination, 'value', destination)
...
def simulation_call_in(simulationHandle, when, inAudioThread, cb, userdata):
    ...
    err = _libaudioverse.Lav_simulationCallIn(simulationHandle, when, inAudioThread, cb, userdata)
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
...
def create_buffer(simulationHandle):
    ...
    err = _libaudioverse.Lav_createBuffer(simulationHandle, 
        ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return reverse_handle(destination.value)
...
def buffer_get_simulation(bufferHandle):
    ...
    err = _libaudioverse.Lav_bufferGetSimulation(bufferHandle, 
        ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return reverse_handle(destination.value)
...
def buffer_get_duration(bufferHandle):
    ...
    err = _libaudioverse.Lav_bufferGetDuration(bufferHandle, 
        ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return getattr(destination, 'value', destination)
...
def buffer_get_length_in_samples(bufferHandle):
    ...
    err = _libaudioverse.Lav_bufferGetLengthInSamples(bufferHandle, 
        ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return getattr(destination, 'value', destination)
...
def node_get_simulation(nodeHandle):
    ...
    err = _libaudioverse.Lav_nodeGetSimulation(nodeHandle, 
        ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return reverse_handle(destination.value)
...
def node_get_input_connection_count(nodeHandle):
    ...
    err = _libaudioverse.Lav_nodeGetInputConnectionCount(nodeHandle, 
        ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return getattr(destination, 'value', destination)
...
def node_get_output_connection_count(nodeHandle):
    ...
    err = _libaudioverse.Lav_nodeGetOutputConnectionCount(nodeHandle, 
        ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return getattr(destination, 'value', destination)
...
def node_get_int_property(nodeHandle, propertyIndex):
    ...
    err = _libaudioverse.Lav_nodeGetIntProperty(nodeHandle, propertyIndex, 
        ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return getattr(destination, 'value', destination)
...
def node_get_float_property(nodeHandle, propertyIndex):
    ...
    err = _libaudioverse.Lav_nodeGetFloatProperty(nodeHandle, propertyIndex, 
        ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return getattr(destination, 'value', destination)
...
def node_get_double_property(nodeHandle, propertyIndex):
    ...
    err = _libaudioverse.Lav_nodeGetDoubleProperty(nodeHandle, propertyIndex, 
        ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return getattr(destination, 'value', destination)
...
def node_get_string_property(nodeHandle, propertyIndex):
    ...
    err = _libaudioverse.Lav_nodeGetStringProperty(nodeHandle, propertyIndex, 
        ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return 
...
def node_get_float3_property(nodeHandle, propertyIndex):
    ...
    err = _libaudioverse.Lav_nodeGetFloat3Property(nodeHandle, propertyIndex, 
        ctypes.byref(destination1), ctypes.byref(destination2), ctypes.byref(destination3))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
...
def node_get_float6_property(nodeHandle, propertyIndex):
    ...
    err = _libaudioverse.Lav_nodeGetFloat6Property(nodeHandle, propertyIndex, 
        ctypes.byref(destinationV1), ctypes.byref(destinationV2), ctypes.byref(destinationV3), ctypes.byref(destinationV4), ctypes.byref(destinationV5), ctypes.byref(destinationV6))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
...
def node_get_int_property_range(nodeHandle, propertyIndex):
    ...
    err = _libaudioverse.Lav_nodeGetIntPropertyRange(nodeHandle, propertyIndex, 
        ctypes.byref(destinationMin), ctypes.byref(destinationMax))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
...
def node_get_float_property_range(nodeHandle, propertyIndex):
    ...
    err = _libaudioverse.Lav_nodeGetFloatPropertyRange(nodeHandle, propertyIndex, 
        ctypes.byref(destinationMin), ctypes.byref(destinationMax))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return getattr(destinationMin, 'value', destinationMin), getattr(destinationMax, 'value', destinationMax)
...
def node_get_double_property_range(nodeHandle, propertyIndex):
    ...
    err = _libaudioverse.Lav_nodeGetDoublePropertyRange(nodeHandle, propertyIndex, 
        ctypes.byref(destinationMin), ctypes.byref(destinationMax))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return getattr(destinationMin, 'value', destinationMin), getattr(destinationMax, 'value', destinationMax)
...
def node_get_property_name(nodeHandle, propertyIndex):
    ...
    err = _libaudioverse.Lav_nodeGetPropertyName(nodeHandle, propertyIndex, 
        ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return 
...
def node_get_property_type(nodeHandle, propertyIndex):
    ...
    err = _libaudioverse.Lav_nodeGetPropertyType(nodeHandle, propertyIndex, 
        ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return getattr(destination, 'value', destination)
...
def node_get_property_has_dynamic_range(nodeHandle, propertyIndex):
    ...
    err = _libaudioverse.Lav_nodeGetPropertyHasDynamicRange(nodeHandle, propertyIndex, 
        ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return getattr(destination, 'value', destination)
...
def node_read_float_array_property(nodeHandle, propertyIndex, index):
    ...
    err = _libaudioverse.Lav_nodeReadFloatArrayProperty(nodeHandle, propertyIndex, index, 
        ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return getattr(destination, 'value', destination)
...
def node_get_float_array_property_length(nodeHandle, propertyIndex):
    ...
    err = _libaudioverse.Lav_nodeGetFloatArrayPropertyLength(nodeHandle, propertyIndex, 
        ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return getattr(destination, 'value', destination)
...
def node_read_int_array_property(nodeHandle, propertyIndex, index):
    ...
    err = _libaudioverse.Lav_nodeReadIntArrayProperty(nodeHandle, propertyIndex, index, 
        ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return getattr(destination, 'value', destination)
...
def node_get_int_array_property_length(nodeHandle, propertyIndex):
    ...
    err = _libaudioverse.Lav_nodeGetIntArrayPropertyLength(nodeHandle, propertyIndex, 
        ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return getattr(destination, 'value', destination)
...
def node_get_array_property_length_range(nodeHandle, propertyIndex):
    ...
    err = _libaudioverse.Lav_nodeGetArrayPropertyLengthRange(nodeHandle, propertyIndex, 
        ctypes.byref(destinationMin), ctypes.byref(destinationMax))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return getattr(destinationMin, 'value', destinationMin), getattr(destinationMax, 'value', destinationMax)
...
def node_get_buffer_property(nodeHandle, propertyIndex):
    ...
    err = _libaudioverse.Lav_nodeGetBufferProperty(nodeHandle, propertyIndex, 
        ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return reverse_handle(destination.value)
...
def create_sine_node(simulationHandle):
    ...
    err = _libaudioverse.Lav_createSineNode(simulationHandle, 
        ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return reverse_handle(destination.value)
...
def create_additive_square_node(simulationHandle):
    ...
    err = _libaudioverse.Lav_createAdditiveSquareNode(simulationHandle, 
        ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return reverse_handle(destination.value)
...
def create_additive_triangle_node(simulationHandle):
    ...
    err = _libaudioverse.Lav_createAdditiveTriangleNode(simulationHandle, 
        ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return reverse_handle(destination.value)
...
def create_additive_saw_node(simulationHandle):
    ...
    err = _libaudioverse.Lav_createAdditiveSawNode(simulationHandle, 
        ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return reverse_handle(destination.value)
...
def create_noise_node(simulationHandle):
    ...
    err = _libaudioverse.Lav_createNoiseNode(simulationHandle, 
        ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return reverse_handle(destination.value)
...
def create_hrtf_node(simulationHandle, hrtfPath):
    ...
    err = _libaudioverse.Lav_createHrtfNode(simulationHandle, hrtfPath, 
        ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return reverse_handle(destination.value)
...
def create_hard_limiter_node(simulationHandle, channels):
    ...
    err = _libaudioverse.Lav_createHardLimiterNode(simulationHandle, channels, 
        ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return reverse_handle(destination.value)
...
def create_crossfading_delay_node(simulationHandle, maxDelay, channels):
    ...
    err = _libaudioverse.Lav_createCrossfadingDelayNode(simulationHandle, maxDelay, channels, 
        ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return reverse_handle(destination.value)
...
def create_dopplering_delay_node(simulationHandle, maxDelay, channels):
    ...
    err = _libaudioverse.Lav_createDoppleringDelayNode(simulationHandle, maxDelay, channels, 
        ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return reverse_handle(destination.value)
...
def create_amplitude_panner_node(simulationHandle):
    ...
    err = _libaudioverse.Lav_createAmplitudePannerNode(simulationHandle, 
        ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return reverse_handle(destination.value)
...
def create_multipanner_node(simulationHandle, hrtfPath):
    ...
    err = _libaudioverse.Lav_createMultipannerNode(simulationHandle, hrtfPath, 
        ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return reverse_handle(destination.value)
...
def create_push_node(simulationHandle, sr, channels):
    ...
    err = _libaudioverse.Lav_createPushNode(simulationHandle, sr, channels, 
        ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return reverse_handle(destination.value)
...
def create_biquad_node(simulationHandle, channels):
    ...
    err = _libaudioverse.Lav_createBiquadNode(simulationHandle, channels, 
        ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return reverse_handle(destination.value)
...
def create_pull_node(simulationHandle, sr, channels):
    ...
    err = _libaudioverse.Lav_createPullNode(simulationHandle, sr, channels, 
        ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return reverse_handle(destination.value)
...
def create_graph_listener_node(simulationHandle, channels):
    ...
    err = _libaudioverse.Lav_createGraphListenerNode(simulationHandle, channels, 
        ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return reverse_handle(destination.value)
...
def create_custom_node(simulationHandle, inputs, channelsPerInput, outputs, channelsPerOutput):
    ...
    err = _libaudioverse.Lav_createCustomNode(simulationHandle, inputs, channelsPerInput, outputs, channelsPerOutput, 
        ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return reverse_handle(destination.value)
...
def custom_node_set_processing_callback(nodeHandle, callback, userdata):
    ...
    err = _libaudioverse.Lav_customNodeSetProcessingCallback(nodeHandle, callback, userdata)
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
...
def create_ringmod_node(simulationHandle):
    ...
    err = _libaudioverse.Lav_createRingmodNode(simulationHandle, 
        ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return reverse_handle(destination.value)
...
def create_feedback_delay_network_node(simulationHandle, maxDelay, channels):
    ...
    err = _libaudioverse.Lav_createFeedbackDelayNetworkNode(simulationHandle, maxDelay, channels, 
        ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return reverse_handle(destination.value)
...
def create_iir_node(simulationHandle, channels):
    ...
    err = _libaudioverse.Lav_createIirNode(simulationHandle, channels, 
        ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return reverse_handle(destination.value)
...
def create_gain_node(simulationHandle, channels):
    ...
    err = _libaudioverse.Lav_createGainNode(simulationHandle, channels, 
        ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return reverse_handle(destination.value)
...
def create_channel_splitter_node(simulationHandle, channels):
    ...
    err = _libaudioverse.Lav_createChannelSplitterNode(simulationHandle, channels, 
        ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return reverse_handle(destination.value)
...
def create_channel_merger_node(simulationHandle, channels):
    ...
    err = _libaudioverse.Lav_createChannelMergerNode(simulationHandle, channels, 
        ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return reverse_handle(destination.value)
...
def create_buffer_node(simulationHandle):
    ...
    err = _libaudioverse.Lav_createBufferNode(simulationHandle, 
        ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return reverse_handle(destination.value)
...
def create_buffer_timeline_node(simulationHandle, channels):
    ...
    err = _libaudioverse.Lav_createBufferTimelineNode(simulationHandle, channels, 
        ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return reverse_handle(destination.value)
...
def create_recorder_node(simulationHandle, channels):
    ...
    err = _libaudioverse.Lav_createRecorderNode(simulationHandle, channels, 
        ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return reverse_handle(destination.value)
...
def create_convolver_node(simulationHandle, channels):
    ...
    err = _libaudioverse.Lav_createConvolverNode(simulationHandle, channels, 
        ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return reverse_handle(destination.value)
...
def create_fft_convolver_node(simulationHandle, channels):
    ...
    err = _libaudioverse.Lav_createFftConvolverNode(simulationHandle, channels, 
        ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return reverse_handle(destination.value)
...
def create_three_band_eq_node(simulationHandle, channels):
    ...
    err = _libaudioverse.Lav_createThreeBandEqNode(simulationHandle, channels, 
        ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return reverse_handle(destination.value)
...
def create_filtered_delay_node(simulationHandle, maxDelay, channels):
    ...
    err = _libaudioverse.Lav_createFilteredDelayNode(simulationHandle, maxDelay, channels, 
        ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return reverse_handle(destination.value)
...
def create_crossfader_node(simulationHandle, channels, inputs):
    ...
    err = _libaudioverse.Lav_createCrossfaderNode(simulationHandle, channels, inputs, 
        ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return reverse_handle(destination.value)
...
def create_one_pole_filter_node(simulationHandle, channels):
    ...
    err = _libaudioverse.Lav_createOnePoleFilterNode(simulationHandle, channels, 
        ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return reverse_handle(destination.value)
...
def create_first_order_filter_node(simulationHandle, channels):
    ...
    err = _libaudioverse.Lav_createFirstOrderFilterNode(simulationHandle, channels, 
        ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return reverse_handle(destination.value)
...
def create_allpass_node(simulationHandle, channels, maxDelay):
    ...
    err = _libaudioverse.Lav_createAllpassNode(simulationHandle, channels, maxDelay, 
        ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return reverse_handle(destination.value)
...
def create_nested_allpass_network_node(simulationHandle, channels):
    ...
    err = _libaudioverse.Lav_createNestedAllpassNetworkNode(simulationHandle, channels, 
        ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return reverse_handle(destination.value)
...
def nested_allpass_network_node_begin_nesting(nodeHandle, delay, coefficient):
    ...
    err = _libaudioverse.Lav_nestedAllpassNetworkNodeBeginNesting(nodeHandle, delay, coefficient)
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
...
def nested_allpass_network_node_append_allpass(nodeHandle, delay, coefficient):
    ...
    err = _libaudioverse.Lav_nestedAllpassNetworkNodeAppendAllpass(nodeHandle, delay, coefficient)
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
...
def nested_allpass_network_node_append_one_pole(nodeHandle, frequency, isHighpass):
    ...
    err = _libaudioverse.Lav_nestedAllpassNetworkNodeAppendOnePole(nodeHandle, frequency, isHighpass)
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
...
def nested_allpass_network_node_append_biquad(nodeHandle, type, frequency, dbGain, q):
    ...
    err = _libaudioverse.Lav_nestedAllpassNetworkNodeAppendBiquad(nodeHandle, type, frequency, dbGain, q)
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
...
def create_fdn_reverb_node(simulationHandle):
    ...
    err = _libaudioverse.Lav_createFdnReverbNode(simulationHandle, 
        ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return reverse_handle(destination.value)
...
def create_blit_node(simulationHandle):
    ...
    err = _libaudioverse.Lav_createBlitNode(simulationHandle, 
        ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return reverse_handle(destination.value)
...
def create_dc_blocker_node(simulationHandle, channels):
    ...
    err = _libaudioverse.Lav_createDcBlockerNode(simulationHandle, channels, 
        ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return reverse_handle(destination.value)
...
def create_leaky_integrator_node(simulationHandle, channels):
    ...
    err = _libaudioverse.Lav_createLeakyIntegratorNode(simulationHandle, channels, 
        ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return reverse_handle(destination.value)
...
def create_file_streamer_node(simulationHandle, path):
    ...
    err = _libaudioverse.Lav_createFileStreamerNode(simulationHandle, path, 
        ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return reverse_handle(destination.value)
...
def create_environment_node(simulationHandle, hrtfPath):
    ...
    err = _libaudioverse.Lav_createEnvironmentNode(simulationHandle, hrtfPath, 
        ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return reverse_handle(destination.value)
...
def environment_node_add_effect_send(nodeHandle, channels, isReverb, connectByDefault):
    ...
    err = _libaudioverse.Lav_environmentNodeAddEffectSend(nodeHandle, channels, isReverb, connectByDefault, 
        ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return getattr(destination, 'value', destination)
...
def create_source_node(simulationHandle, environmentHandle):
    ...
    err = _libaudioverse.Lav_createSourceNode(simulationHandle, environmentHandle, 
        ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return reverse_handle(destination.value)
