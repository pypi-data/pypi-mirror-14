...
def bammarkduplicates(uuid, bam_path, engine, logger):
    step_dir = os.getcwd()
    ...
    if pipe_util.already_step(work_dir, 'md', logger):
        logger.info('already completed step `bammarkduplicates` of: %s' % bam_path)
    else:
        logger.info('running step `bammarkduplicates` of %s: ' % bam_path)
        os.makedirs(work_dir, exist_ok=True)
        cpu_count = pipe_util.get_cpu_count()
        cmd = ['bammarkduplicates', 'I=' + bam_path, 'O=' + out_bam_path, 'M=' + out_metrics_path, 'verbose=0', 'level=-1', 'index=1',
        ...
        logger.info('completed running step `bammarkduplicates` of: %s' % bam_path)
    return
