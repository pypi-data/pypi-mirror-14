...
def main():
    ...
    if tool_name == 'bamfixmateinformation':
        ...
        bamfixmateinformation.bamfixmateinformation(uuid, bam_path, engine, logger, be_lenient)
    elif tool_name == 'bamindex':
        bam_path = pipe_util.get_param(args, 'bam_path')
        bamindex.bamindex(uuid, bam_path, engine, logger)
    elif tool_name == 'bammarkduplicates':
        bam_path = pipe_util.get_param(args, 'bam_path')
        bammarkduplicates.bammarkduplicates(uuid, bam_path, engine, logger)
    elif tool_name == 'bammarkduplicates2':
        bam_path = pipe_util.get_param(args, 'bam_path')
        bammarkduplicates2.bammarkduplicates2(uuid, bam_path, engine, logger)
    elif tool_name == 'bammdnm':
        bam_path = pipe_util.get_param(args, 'bam_path')
        reference_fasta_path = pipe_util.get_param(args, 'reference_fasta_path')
        bammdnm.bammdnm(uuid, bam_path, reference_fasta_path, engine, logger)
    elif tool_name == 'bammerge':
        bam_path = pipe_util.get_param(args, 'bam_path')
        bammerge.bammerge(uuid, bam_path, engine, logger)
    elif tool_name == 'bamsort':
        bam_path = pipe_util.get_param(args, 'bam_path')
        reference_fasta_path = pipe_util.get_param(args, 'reference_fasta_path')
        bamsort.bamsort(uuid, bam_path, reference_fasta_path, engine, logger)
    elif tool_name == 'bamtofastq':
        bam_path = pipe_util.get_param(args, 'bam_path')
        bamtofastq.bamtofastq(uuid, bam_path, engine, logger)
    elif tool_name == 'bamvalidate':
        bam_path = pipe_util.get_param(args, 'bam_path')
        bamvalidate.bamvalidate(uuid, bam_path, engine, logger, be_lenient)
    else:
