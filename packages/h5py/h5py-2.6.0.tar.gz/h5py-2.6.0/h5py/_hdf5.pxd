...
cdef extern from "hdf5.h":
  ...
  hid_t H5Aopen(hid_t obj_id, char *attr_name, hid_t aapl_id) except *
  hid_t H5Aopen_by_name( hid_t loc_id, char *obj_name, char *attr_name, hid_t aapl_id, hid_t lapl_id) except *
  hid_t H5Aopen_by_idx(hid_t loc_id, char *obj_name, H5_index_t idx_type, H5_iter_order_t order, hsize_t n, hid_t aapl_id, hid_t lapl_id) except *
  ...
  IF HDF5_VERSION >= (1, 9, 178):
      herr_t H5Dflush(hid_t dataset_id) except *
  IF HDF5_VERSION >= (1, 9, 178):
      herr_t H5Drefresh(hid_t dataset_id) except *
  herr_t H5Eset_auto(void* a, void* b) except *
  ...
  IF HDF5_VERSION >= (1, 9, 178):
      herr_t H5Fstart_swmr_write(hid_t file_id) except *
  ...
  herr_t H5Lunpack_elink_val(void *ext_linkval, size_t link_size, unsigned *flags, const char **filename, const char **obj_path) except *
  ...
  hid_t H5Pget_class(hid_t plist_id) except *
  ...
  herr_t H5Pset_fapl_multi(hid_t fapl_id, H5FD_mem_t *memb_map, hid_t *memb_fapl, const char * const *memb_name, haddr_t *memb_addr, hbool_t relax) except *
  herr_t H5Pset_cache(hid_t plist_id, int mdc_nelmts, int rdcc_nelmts,  size_t rdcc_nbytes, double rdcc_w0) except *
  herr_t H5Pget_cache(hid_t plist_id, int *mdc_nelmts, size_t *rdcc_nelmts, size_t *rdcc_nbytes, double *rdcc_w0) except *
  ...
  IF HDF5_VERSION >= (1, 9, 233):
      ssize_t H5Pget_virtual_dsetname(hid_t dcpl_id, size_t index, char *name, size_t size) except *
  IF HDF5_VERSION >= (1, 9, 233):
      ssize_t H5Pget_virtual_filename(hid_t dcpl_id, size_t index, char *name, size_t size) except *
  IF HDF5_VERSION >= (1, 9, 233):
      herr_t H5Pget_virtual_count(hid_t dcpl_id, size_t *count) except *
  IF HDF5_VERSION >= (1, 9, 233):
      herr_t H5Pset_virtual(hid_t dcpl_id, hid_t vspace_id, const char *src_file_name, const char *src_dset_name, hid_t src_space_id) except *
  IF HDF5_VERSION >= (1, 9, 233):
      hid_t H5Pget_virtual_vspace(hid_t dcpl_id, size_t index) except *
  IF HDF5_VERSION >= (1, 9, 233):
      hid_t H5Pget_virtual_srcspace(hid_t dcpl_id, size_t index) except *
  ...
  IF HDF5_VERSION >= (1, 9, 233):
      herr_t H5Pset_virtual_view(hid_t plist_id, H5D_vds_view_t view) except *
  IF HDF5_VERSION >= (1, 9, 233):
      herr_t H5Pget_virtual_view(hid_t plist_id, H5D_vds_view_t *view) except *
  IF HDF5_VERSION >= (1, 9, 233):
      herr_t H5Pset_virtual_printf_gap(hid_t plist_id, hsize_t gap_size) except *
  IF HDF5_VERSION >= (1, 9, 233):
      herr_t H5Pget_virtual_printf_gap(hid_t plist_id, hsize_t *gap_size) except *
  IF MPI:
      herr_t H5Pset_dxpl_mpio( hid_t dxpl_id, H5FD_mpio_xfer_t xfer_mode ) except *
  IF MPI:
      herr_t H5Pget_dxpl_mpio( hid_t dxpl_id, H5FD_mpio_xfer_t* xfer_mode ) except *
  ...
  herr_t H5Sselect_elements(hid_t space_id, H5S_seloper_t op,  size_t num_elements, const hsize_t *coord) except *
  ...
  IF HDF5_VERSION >= (1, 9, 233):
      htri_t H5Sis_regular_hyperslab(hid_t spaceid) except *
  IF HDF5_VERSION >= (1, 9, 233):
      htri_t H5Sget_regular_hyperslab(hid_t spaceid, hsize_t* start, hsize_t* stride, hsize_t* count, hsize_t* block) except *
