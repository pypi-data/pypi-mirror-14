...
class TypingMeta(type):
    ...
    def __new__(cls, name, bases, namespace, **_3to2kwargs):
        if '_root' in _3to2kwargs: _root = _3to2kwargs['_root']; del _3to2kwargs['_root']
        else: _root = False
        if not _root:
            raise TypeError("Cannot subclass %s" %
                            (', '.join(map(_type_repr, bases)) or '()'))
        return super(cls.__class__, cls).__new__(cls, name, bases, namespace)
    ...
    def __new__(cls, arg):
        ...
        try:
            ...
            raise SyntaxError('ForwardRef must be an expression -- got %r' %
                              (arg,))
        self = super(cls.__class__, cls).__new__(cls, arg, (), {}, _root=True)
...
class AnyMeta(TypingMeta):
    ...
    def __new__(cls, name, bases, namespace, _root=False):
        self = super(cls.__class__, cls).__new__(cls, name, bases, namespace, _root=_root)
        return self
    ...
    def __subclasscheck__(self, cls):
        if not isinstance(cls, type):
            return super(self.__class__, self).__subclasscheck__(cls)  # To TypeError.
        return True


class Any(Final, _root=True):
...
class TypeVar(TypingMeta, _root=True):
    ...
    def __new__(cls, name, *constraints, **_3to2kwargs):
        if 'contravariant' in _3to2kwargs: contravariant = _3to2kwargs['contravariant']; del _3to2kwargs['contravariant']
        else: contravariant = False
        if 'covariant' in _3to2kwargs: covariant = _3to2kwargs['covariant']; del _3to2kwargs['covariant']
        else: covariant = False
        if 'bound' in _3to2kwargs: bound = _3to2kwargs['bound']; del _3to2kwargs['bound']
        else: bound = None
        self = super(cls.__class__, cls).__new__(cls, name, (Final,), {}, _root=True)
...
class UnionMeta(TypingMeta):
    ...
    def __new__(cls, name, bases, namespace, parameters=None, _root=False):
        if parameters is None:
            return super(cls.__class__, cls).__new__(cls, name, bases, namespace, _root=_root)
        ...
        self = super(cls.__class__, cls).__new__(cls, name, bases, {}, _root=True)
...
class Optional(Final, _root=True):
...
class TupleMeta(TypingMeta):
    ...
    def __subclasscheck__(self, cls):
        ...
        if not isinstance(cls, TupleMeta):
            return super(self.__class__, self).__subclasscheck__(cls)  # False.
...
class CallableMeta(TypingMeta):
    ...
    def __subclasscheck__(self, cls):
        ...
        if not isinstance(cls, CallableMeta):
            return super(self.__class__, self).__subclasscheck__(cls)
        if self.__args__ is None and self.__result__ is None:
            return True
...
class Callable(Final, _root=True):
...
class GenericMeta(TypingMeta, abc.ABCMeta):
    ...
    def __new__(cls, name, bases, namespace,
        ...
        self = super(cls.__class__, cls).__new__(cls, name, bases, namespace, _root=True)
    ...
    def __subclasscheck__(self, cls):
        ...
        if super(self.__class__, self).__subclasscheck__(cls):
            return True
...
class AbstractSet(Sized, Sequentiable[T_co], Container[T_co],
...
class Mapping(Sized, Sequentiable[KT], Container[KT], Generic[VT_co],
...
class Sequence(Sized, Sequentiable[T_co], Container[T_co],
...
class MappingView(Sized, Sequentiable[T_co], extra=collections_abc.MappingView):
    pass
...
class Generator(Iterator[T_co], Generic[T_co, T_contra, V_co],
    ...
    def __new__(cls, *args, **kwds):
        ...
        return super(cls.__class__, cls).__new__(cls, *args, **kwds)
