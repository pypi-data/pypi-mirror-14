...
class TCellAgent(object):
    ...
    def __init__(self):
        ...
        LOGGER.debug("making initial connection.")
        self.call_update(None, delays=False)
    ...
    def call_update(self, last_timestamp, delays=True):
        ...
        try:
            if CONFIGURATION.version == 1:
                ...
                result = self.background_tcell_api.v0api('/csp/poll', last_timestamp)
            if self.shutdown:
            ...
            LOGGER.error("result general exception, sleeping 120 seconds")
            LOGGER.debug(general_exception)
            if delays:
                time.sleep(120)
        return last_timestamp
    ...
    def run_event_handler(self):
        """ Method that runs forever """
        self.events_to_dispatch = [ServerAgentDetailsEvent(
    ...
    def flush_events(self):
        ...
        try:
            ...
            LOGGER.error("Error sending events, will try again laster")
            LOGGER.debug(connection_exception)
            self.filter_all_ensure_events()
    ...
    def send(cls, event):
        try:
            ...
            LOGGER.error("could not send event, no agent initialized")
            LOGGER.debug(e)
    ...
    def get_policy(cls, policy_type):
        try:
            ...
            LOGGER.error("could not get policy, no agent initialized")
            LOGGER.debug(e)
    ...
    def get_route_table(cls):
        try:
            ...
            LOGGER.error("could not get route_table, no agent initialized")
            LOGGER.debug(e)
        ...
        try:
            if cls.get_agent().in_fork() and cls.get_agent().fork_pipe_broken == False:
                ...
                try:
                    ...
                    LOGGER.error("Exception adding login metric")
            ...
            LOGGER.error("could not add metric")
            LOGGER.debug(e)
    ...
    def discover_route(cls, route_url, route_method, route_target, route_id):
        try:
            ...
            LOGGER.error("could not add route discovery")
            LOGGER.debug(e)
    ...
    def discover_database_fields(cls, database, schema, table, fields, route_id=None):
        try:
            ...
            LOGGER.error("could not add database discovery fields")
            LOGGER.debug(e)