            ...
            for track_idx in track_idxes:
                ...
                for action in actions:
                    try:
                        ...
                        print(e)
                        pass
    elif count == CONFIGURATION.max_data_ex_db_records_per_request:
        logger.warn("Retrieved too many records")

    return row

def multicallback2(rows, track_idxes, request, i):
    j = i * len(rows) - 1
    for row in rows:
        j += 1
        yield singlecallback(row, track_idxes, request, j)

def multicallback(results, track_idxes, request):
    for counter, rows in enumerate(results):
        yield multicallback2(rows, track_idxes, request, counter)
                ...
                if col_supported:
                    if self.select:
                        try:
                            ...
                            if results and request and len(track_idxes.keys()) > 0:
                                if result_type == MULTI:
                                    return multicallback(results, track_idxes, request)
