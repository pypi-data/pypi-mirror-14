...
class ContentSecurityHeader:
    ...
    def header_value(self, transaction_id=None, route_id=None, session_id=None, user_id=None):
        ...
        try:
            ...
            if route_id:
                params["rid"] = route_id
            #url_parts = list(urlparse(unicode(self.report_uri)))
            url_parts = list(urlparse(self.report_uri)) 
            query = dict(parse_qsl(url_parts[4]))
            query.update(params)
            url_parts[4] = urlencode(query)
            report_url = urlunparse(url_parts)
            ...
            logger.error("error creating csp: %s: (%s, %s), falling back" % (e, self.value, self.report_uri))
