...
def singlecallback(row, track_idxes, request):
...
def multicallback2(rows, track_idxes, request):
    for row in rows:
        yield singlecallback(row, track_idxes, request)

def multicallback(results, track_idxes, request):
    for rows in results:
        yield multicallback2(rows, track_idxes, request)
                        ...
                        try:
                            ...
                            if results and request and len(track_idxes.keys()) > 0:
                                if result_type == MULTI:
                                    ...
                                    return singlecallback(results, track_idxes, request)
