...
class TutorHome(Frame):
	def __init__(self, parent, controller):								#Home window for TA/Tutor 
...
#		self.geometry('500x500')
		...
		for F in (assignment, student, marks, question, stud_pro, ansassign):
			...
			fram.grid(row = 0, column = 1, sticky = "nsew")
																	#tabs for assignment, student and marks
	...
	def load(self, info):										#loads assignment frame by default
		self.course = info
		self.show_frame("assignment")

	def back(self):											#back button customised for internal frames
		if(len(self.array) > 1):
