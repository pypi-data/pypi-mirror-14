...
class StudentHome(Frame):
	def __init__(self, parent, controller):								#Course frame for students
...
#		self.geometry('500x500')
	...
	def show_frame(self, page_name, assign = ""):				#displays separate frames according to buttons in frame1
	...
	def back(self):												#back button for internal frames
		if(len(self.array) > 1):
			frame = self.array[len(self.array) - 2]
			self.array = self.array[:len(self.array) - 1]
			frame.tkraise()

	def load(self, info):											#default frame is assignment
		self.course = info
