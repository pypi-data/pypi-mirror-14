...
class assignment(Frame):
	def __init__(self, parent, controller):								#assignment window 
		...
		# 	listt.insert(i, y[i])
		# listt.pack()
		# but0 = Button (self,fg="black",text="Assignment 1",height="4", command = lambda: self.show_frame("assignment"))
		# but0.pack(side = TOP,fill=X)
		# but2 = Button (self,fg="black",text="Assignment 2",height="4", command = lambda: self.show_frame("student"))
		# but2.pack(side = TOP,fill=X)
		# lab0 = Label(self,text="Add Assignments: ")
		# lab0.pack(side=TOP)
...
class student(Frame):
	def __init__(self, parent, controller):								#student window 
		...
		# 	listt.insert(i, y[i])
		# listt.pack()
		# self.students = students = Menubutton(self,text = "Student Names")
		# students.grid()
		# students.menu = Menu(students)
		# students["menu"] = students.menu
		# stud=["sfd","sdgfs"]
		# for studd in stud:
		# 	students.menu.add_checkbutton(label = studd,variable=studd, command= partial(self.add_selection,studd))
		# 	#students.menu.add_checkbutton(label = "stud2")
		# students.pack()
		# # but0 = Button (self,fg="black",text="Kuldeep",height="4", command = lambda: self.show_frame("assignment"))
		# # but0.pack(side = TOP,fill=X)
		# # but2 = Button (self,fg="black",text="Parewa",height="4", command = lambda: self.show_frame("student"))
		# # but2.pack(side = TOP,fill=X)
	def add_selection(self, roll_no, studd):
	...
	def load(self):
		...
		for stud in all_list:
			if stud not in ta_students:
				students.menu.add_checkbutton(label = all_list[stud]["name"] + ":" + all_list[stud]["roll_no"], variable = all_list[stud]["roll_no"],width=15, command = partial(self.add_selection, all_list[stud]["roll_no"], all_list[stud]["name"] + ":" + all_list[stud]["roll_no"]))
		students.pack()
...
class marks(Frame):
	def __init__(self, parent, controller):								#marks frame 
		Frame.__init__(self, parent)
		self.controller = controller									
...
class question(Frame):
	def __init__(self, parent, controller):								#marks window 
		Frame.__init__(self, parent)
		self.controller = controller									#8donvdratruqwbrblduqb
		self.canvas = canvas = Canvas(self)#, width = 600, height = 600)
