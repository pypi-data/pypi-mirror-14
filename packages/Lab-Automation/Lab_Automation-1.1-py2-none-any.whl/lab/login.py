...
class Login(Frame):
	def __init__(self, parent, controller):
		...
		self.grid_columnconfigure(0, weight = 3)				#dividing the frame into columns
		self.grid_columnconfigure(1, weight = 1)
		self.grid_columnconfigure(2, weight = 1)
		self.grid_columnconfigure(3, weight = 3)
		self.grid_rowconfigure(0, weight = 7)					#dividing the frame into rows
		...
		#User Nam and Password entries for Login
		...
		self.ent1.bind('<Control-x>', lambda e: 'break') #disable cut for password
		self.ent1.bind('<Control-c>', lambda e: 'break') #disable copy for password
		self.ent1.bind('<Control-v>', lambda e: 'break') #disable paste for password
		self.ent1.bind('<Button-3>', lambda e: 'break') #disable right click for password
	...
	def login(self, controller):				#Function for Logging in user
		...
		if username == "":
			...
			# print(csrf)
			message = "Success"
			#sends username and password for checking the database
