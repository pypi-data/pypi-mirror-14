...
class Def(Bot):
    ...
    def search_word(self, *args, **kwargs):
        ...
        if wx:
            if len(wx) > 1:
                ...
                cached_word = self._get_word_from_cache(wx[0].word)
                if cached_word:
                    self.reply({'text': cached_word.text})
                else:
    ...
    def define_word(self, *args, **kwargs):
        ...
        if self._word_in_choices(word, self._message['chat']['id']):
            cached_word = self._get_word_from_cache(word)
            if cached_word:
                meaning = cached_word.text
            else:
