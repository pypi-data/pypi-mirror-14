...
class Checker(object):
    ...
    def _check(self, curr_key, arr):
        ...
        for k2, v in arr.items():
            ...
            if not _ or not all([x in _.keys() for x in [u"c"]]): #-t默认str  但-c一定要有
            ...
            if isinstance(v, list):
                ...
                for i, x in enumerate(v):
                    if "[%s]" % k1!=_.get("t"):
                        print '[ERROR] key值: %s 声明的类型为:%s,与key值不符合,正确值为[%s],请检查 "%s"' % (k,_.get("t"),k, k2)
                        self.is_ok = False
                    # print k,k1
                    # if k=="category_list":
                    # print k1,i,x
    ...
    def check(self, obj_name, txt):
        try:
            arr = json.loads(txt)
            self._check(obj_name, arr)
        except ValueError, e:
