...
def criterion_function(neighbors, edge_distances, point_data):
    ...
    if type(neighbors) is not collections.defaultdict:
        ...
        raise ValueError("input must be of type scipy.spatial.qhull.Delaunay.")
...
def global_sta_dev(edge_distances):
    ...
    Usage
    -----
    >>> global_std_distance = tri_calc.global_sta_dev(edge_distances)
...
def global_mean_distance(edge_distances):
    ...
    Usage
    -----
    >>> global_mean_distance = tri_calc.global_mean_distance(edge_distances)
...
def local_mean_distance(neighbors, points):
    ...
    Usage
    -----
    >>> local_mean_distance = tri_calc.local_mean_distance(neighbors,\
 point_data)
    """
