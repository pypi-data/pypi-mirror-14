...
def GetLogFile(physical=False, filename=None):
    ...
    if flog_static.store_log_values["__log_file"] is None:
        if physical:
            ...
            if not isinstance(flog_static.store_log_values["__log_file_name"], unicode
                              ):
...
def guess_type_value(x, none=None):
    ...
    try:
        ...
        if x[0] == '0' and len(x) > 1:
            ...
            return int if len(x) < 9 else unicode
    except:
        try:
            x = float(x)
            return float
...
def get_default_value_type(ty, none=True):
    ...
    if ty is None and none:
        ...
        raise PQHException("type expected in " + unicode
...
def guess_type_list(l, tolerance=0.01, none=True):
    ...
    """
    defa = None if none else unicode
    length = 0
    typstr = unicode
