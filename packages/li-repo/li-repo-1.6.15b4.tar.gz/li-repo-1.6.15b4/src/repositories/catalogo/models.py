# -*- coding: utf-8 -*-
#
from repositories.domain.models import MyObjects, remover_acentos
...
#     imagem = models.ForeignKey('repositories_plataforma.Imagem', related_name='produtos_imagem', on_delete=models.CASCADE)
...
"""
#########################################################################################################
                        abaixo catalogo/models.py que estava em LI-AppPainel
#########################################################################################################
"""
# -*- coding: utf-8 -*-
from repositories.domain.models import MyObjects, remover_acentos
    ...
    END"""
        ...
        if listagem == 'alfabetica':
            ...
            queryset = queryset.extra(
                select={'vendas':  "select count(pedido_venda_item_id) from " \
                                   "pedido.tb_pedido_venda_item, pedido.tb_pedido_venda where " \
                                   "tb_pedido_venda_item.produto_id = tb_produto.produto_id" \
                                   " AND tb_pedido_venda_item.pedido_venda_id = " \
                                   "tb_pedido_venda.pedido_venda_id and tb_pedido_venda.pedido_venda_situacao_id not in " \
                                   "(6, 7, 8)"},
                order_by=['-vendas']
            )
        elif listagem == 'ativo':
...
class Produto(models.Model):
    ...
    imagens = models.ManyToManyField('domain.Imagem', through='ProdutoImagem', related_name='produtos')
    grades = models.ManyToManyField('catalogo.Grade', through='ProdutoGrade', related_name='produtos')
    categorias = models.ManyToManyField('catalogo.Categoria', through='ProdutoCategoria', related_name='produtos')
    destaque = models.BooleanField(db_column='produto_destaque', null=False, default=False)

    # Dados em cache que são atualizados pelas triggers.
    # cache_preco_sob_consulta = models.NullBooleanField(db_column='produto_cache_preco_sob_consulta', null=True, default=None)
    # cache_preco_cheio = models.DecimalField(db_column='produto_cache_preco_cheio', max_digits=16, decimal_places=4, null=True)
    # cache_preco_promocional = models.DecimalField(db_column='produto_cache_preco_promocional', max_digits=16, decimal_places=4, null=True)
    # cache_preco_custo = models.DecimalField(db_column='produto_cache_preco_custo', max_digits=16, decimal_places=4, null=True)
    # cache_preco_varia = models.NullBooleanField(db_column='produto_cache_preco_varia', null=True, default=None)
    # cache_preco_vazio = models.NullBooleanField(db_column='produto_cache_preco_vazio', null=True, default=None)
    # cache_tem_venda = models.NullBooleanField(db_column='produto_cache_tem_venda', null=True, default=None)
    # cache_url = models.CharField(db_column='produto_cache_url', max_length=255, null=True, default=None)
    # cache_imagem_principal = models.ForeignKey('domain.Imagem', db_column='produto_cache_imagem_principal_id', null=True, on_delete=models.SET_NULL)
    # cache_categoria_principal = models.ForeignKey('catalogo.Categoria', db_column='produto_cache_categoria_principal_id', null=True, on_delete=models.SET_NULL)
    ...
    def disponivel(self):
        ...
        # if self.preco.sob_consulta:
        #     return True
        #
        # if self.tipo in ('normal', 'atributo'):
        #     return self.estoque.disponivel()
        # else:
        #     if hasattr(self, '_disponivel'):
        #         return self._disponivel
        #
        #     self._disponivel = (self.estoque.disponivel() and
        #                         self.preco.preenchido())
        #     return self._disponivel
...
def produto_post_save(sender, instance, created, raw, **kwargs):
    ...
    # Cria as relações que o produto precisa para funcionar.
    if created and not getattr(instance, '_copia', False):
        ProdutoEstoque.objects.get_or_create(produto_id=instance.id, conta_id=instance.conta_id)
        ProdutoPreco.objects.get_or_create(produto_id=instance.id, conta_id=instance.conta_id)

    if created:
        SEO.objects.get_or_create(linha_id=instance.id, tabela='tb_produto', conta_id=instance.conta_id)


@receiver(post_save, sender=ProdutoCategoria)
def produto_categoria_post_save(sender, instance, created, raw, **kwargs):
    # Caso não exista nenhuma categoria principal, nomeia uma.
    total_produto_categoria = ProdutoCategoria.objects.filter(
        produto=instance.produto, principal=True).count()
    if not total_produto_categoria:
        produto_categorias = ProdutoCategoria.objects.filter(produto=instance.produto)
        if produto_categorias.count():
            produto_categoria = produto_categorias[0]
            produto_categoria.principal = True
