...
import repositories.libs.utils
...
class Usuario(models.Model):
    ...
    contas = models.ManyToManyField('plataforma.Conta', related_name="usuarios", through="ContaUsuario")
    ...
    def autenticar(self, email=None, senha=None, contrato_id=None, conta_id=None, by_token=None):
        ...
        try:
            if by_token is not None and by_token is True:
                usuario = self.objects.get(email=email, contrato_id=contrato_id)
            else:
                usuario = self.objects.get(email=email, _senha=hash(senha),
...
class Conta(models.Model):
    ...
    dominio = models.CharField(db_column="conta_loja_dominio", max_length=128, null=True, default=None)
    nome_loja = models.CharField(db_column="conta_loja_nome", max_length=128, null=True, default=None)
...
class Parceiro(models.Model):
    ...
    data_contrato = models.DateField(db_column="parceiro_data_contrato", null=True)
...
class Contrato(models.Model):
    ...
    url_termo = models.CharField(u'URL Termo de uso', db_column="contrato_url_termo", max_length=255)
