    ...
    return {
      ...
      link: function($scope, element, attrs) {
        ...
        $scope.fullReload()["catch"](function() {
          $scope.disabled = true;
          $scope.onReady();
          return notify.info(gettext('Terminal was closed'));
        });
        $scope.scheduleResize = function(w, h) {
          $timeout.cancel($scope.resizeTimeout);
          return $scope.resizeTimeout = $timeout(function() {
        ...
        $scope.autoResize = function() {
          ...
          return $scope.scheduleResize(cols, rows);
        };
  ...
  angular.module('ajenti.terminal').controller('ScriptWidgetController', function($scope, $location, notify, terminals, gettext) {
    return $scope.run = function() {
      ...
      if ($scope.widget.config.terminal) {
        ...
        return terminals.script({
          script: script,
          input: input
        }).then(function(data) {
          if (data.code === 0) {
            return notify.success(gettext('Script has finished'), data.output + data.stderr);
          } else {
            return notify.warning(gettext('Script has failed'), data.stderr + data.output);
          }
        })["catch"](function(err) {
...
(function() {
  angular.module('ajenti.terminal').service('terminals', function($http, $q, $location) {
    this.script = function(options) {
      var q;
      q = $q.defer();
      $http.post('/api/terminal/script', options).success(function(data) {
