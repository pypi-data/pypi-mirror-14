...
from ...backends import BLEBackend, Characteristic
from ...util import uuid16_to_uuid
...
class BGAPIBackend(BLEBackend):
    ...
    def disconnect_all(self):
        """
        disconnect all - see
        http://community.silabs.com/t5/Wireless/BGAPI-BGLib-How-to-close-all-opened-connections-at-once-on/m-p/161279/highlight/true#M10001
        :return:
        """
        MAX_CONNECTIONS = 10 # yy - TODO - what is the acutal max connection
        try:
            for i in range(0, MAX_CONNECTIONS + 1):
                self.send_command(CommandBuilder.connection_disconnect(i))
        except Exception as ex:
            log.error('EXCEPTION when trying to disconnect from MyMe device.'
    ...
    def start(self):
        ...
        receiver thread.
        """
        if self._running and self._runnperson_iding.is_set():
            self.stop()

        self._ser = serial.Serial(self._serial_port, baudrate=256000,
                                  timeout=0.25, write_timeout=0.5)
        ...
        # yy - patch start
        self.disconnect_all()
        print ('Disconnected from all connected devices')
        # yy - patch end
    ...
    def expect_any(self, expected_packet_choices, timeout=None,
        ...
        """
        # YL Patch: changed default timeout to 2
        if not timeout:
            timeout = 15
        ...
        while True:
            ...
            try:
                ...
                # YL Patch: changed timeout from 0.1 to 0.5
                packet = self._receiver_queue.get(timeout=0.1)
            except queue.Empty:
                if timeout is not None:
                    if time.time() - start_time > timeout:
                        log.error('got timeout while waiting for response '
                              'from one of: {}'.format(expected_packet_choices))
                        # YL: can we get rid of this exception? it's annoying...:)
    ...
    def _ble_evt_attclient_find_information_found(self, args):
        ...
        elif uuid_type in  [UUIDType.custom, UUIDType.characteristic]:
            log.info("Found characteristic {} of type: {}".format(uuid,uuid_type))
