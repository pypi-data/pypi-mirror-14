...
if sys.platform.startswith("darwin"):
    libfile = os.path.join(path_prefix, "osx/mujoco/libmujoco.dylib")
elif sys.platform.startswith("linux"):
    libfile = os.path.join(path_prefix, "linux/mujoco/libmujoco.so")
elif sys.platform.startswith("win"):
    libfile = os.path.join(path_prefix, "win/mujoco/mujoco.lib")
else:
...
try:
    ...
    target = os.path.realpath(os.path.join(path_prefix, 'osx/gcc'))
    raise OSError, """Was not able to load mjlib.

Original error message: {message}

HINT: this generally means MuJoCo could not find the relevant GCC dynamic libraries. To fix this, try doing:

  export DYLD_FALLBACK_LIBRARY_PATH="{target}"

Note that if you are running on El Capitan, System Integrity
Protection may prevent DYLD_FALLBACK_LIBRARY_PATH from being picked up
(see https://github.com/BVLC/caffe/issues/3227 for a similar
issue). In this case, the best workaround is probably to globally
install the GCC libraries needed by MuJoCo:

  sudo cp -vi {target}/* /usr/local/lib

You can always remove these libraries later:

  cd {target} && for i in *; do sudo rm -i /usr/local/lib/$i; done
""".format(message=e.message, target=target)
