...
def _load_library():
    ...
    path_prefix = os.environ['MUJOCO_PY_BUNDLE_PATH']
    if sys.platform.startswith("darwin"):
        libfile = os.path.join(path_prefix, "osx/mjpro/bin/libglfw.3.dylib")
    elif sys.platform.startswith("linux"):
        libfile = os.path.join(path_prefix, "linux/mjpro/bin/libglfw.so.3")
    elif sys.platform.startswith("win"):
        libfile = os.path.join(path_prefix, "windows/mjpro/bin/glfw3.dll")
    else:
        raise RuntimeError("Unrecognized platform %s" % sys.platform)

    if not os.path.exists(libfile):
        raise RuntimeError("Missing path: %s. (HINT: you should install a mujoco-bundle directory provided by OpenAI. If you've done so, then this is a bug and please let us know.)" % libfile)
