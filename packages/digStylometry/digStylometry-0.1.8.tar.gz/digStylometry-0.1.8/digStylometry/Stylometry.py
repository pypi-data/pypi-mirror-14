...
class ComputeSignature:
    def __init__(self,**options):
        ...
        self.minLength = options.get("minLength")

    def check_less_min_sig(self,x):
        return len(x[1][self.field_name]) > self.minLength

    def perform(self,rdd):
        rdd = rdd.mapValues(lambda x: self.addsignature(x))
        rdd = rdd.filter(lambda x : self.check_less_min_sig(x))
