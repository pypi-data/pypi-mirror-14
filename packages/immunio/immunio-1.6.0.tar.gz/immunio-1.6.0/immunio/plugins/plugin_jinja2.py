...
def add_hooks(run_hook, get_agent_func=None, timer=None):
    ...
    add_compiler_hooks(timer)
    # Hook the actual rendering
    add_template_render_hook(run_hook, timer)
...
def add_template_render_hook(run_hook, timer):
    """
    ...
    def _from_namespace(orig, template_cls, environment, namespace,
        ...
        @monkeypatch(template, "root_render_func")
    ...
    def _macro_call(orig, macro_self, *args, **kwargs):
        ...
        sha = getattr(macro_self, "_immunio_sha")
        filename = getattr(macro_self, "_immunio_filename")
