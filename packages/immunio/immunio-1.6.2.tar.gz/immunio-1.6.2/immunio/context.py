...
def get_context(additional_data=None, log_context_data=False, offset=0):
    ...
    """
    strict_context, loose_context, stack, loose_stack = _build_context(
        get_stack(),
        log_context_data,
        offset,
    )
...
def _is_immunio_agent_file(filename):
    ...
    the cache decorator.
    """
    path_part = filename.split("/immunio/")[-1]
    path = os.path.join(THIS_DIR, path_part)
    return os.path.exists(path)


@cachetools.func.lfu_cache(maxsize=1000)
def _build_context(stack, log_context_data, offset):
    ...
    for frame in stack:
        ...
        # determines our loose context key. This includes filenames
        # for usability -- just method names not being very good
        # for display purposes...
        loose_stack_rope.append("%s:%s" % (filename, name))
    ...
    strict_context = hashlib.sha1(strict_context_stack).hexdigest()
