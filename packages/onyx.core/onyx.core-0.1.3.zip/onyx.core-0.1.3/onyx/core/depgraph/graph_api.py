...
from ..database.ufo_base import UfoBase, get_base_classes
from ..database.objdb import ObjNotFound
from ..database.objdb_api import DelObj
from .graph import DependencyGraph, CreateNode, GraphNodeSettable
from .graph import GraphError, GraphNodeError
...
class GraphNodeVt(object):
    """
    This decorator trasforms methods of classes derived from UfoBase in a
    on-graph Value Type.
    """
    # -------------------------------------------------------------------------
...
def GetVal(obj, VT, *args, **kwds):
    ...
    if not isinstance(obj, str):
        raise ValueError("First argument should be the target object. "
                         "{0!s} was passed instead".format(obj.__class__))
    # --- return node value. we make a deepcopy to make sure we never return a
    #     reference to a mutable value
    return copy.deepcopy(GetNode((obj, VT)).get_value(*args, **kwds))
...
# -----------------------------------------------------------------------------
def ChildrenSet(obj, VT, child_VT, obj_type=None, graph=None):
    """
    Description:
...
# -----------------------------------------------------------------------------
def leaves_iterator(obj_name, VT, get_val):
    stored = get_val(obj_name, "StoredAttrs")

    for name, vt in onyx_dg.graph[(obj_name, VT)].children:
        if vt in stored:
            yield name, vt
        else:
            for leaf in leaves_iterator(name, vt, get_val):
                yield leaf


# -----------------------------------------------------------------------------
def LeafNodes(obj, VT, graph=None):
    """
    Description:
    ...
    return set(leaves_iterator(obj, VT, get_val))
...
# -----------------------------------------------------------------------------
def SettableNodes(obj, VT, graph=None):
    """
    Description:
