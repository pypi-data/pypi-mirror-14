...
class PluginClientProtocol(WebSocketClientProtocol):
    """ Websocket linking the paps_settings to the webserver """
...
class SettingsPlugin(Plugin):
    """ Class handling paps_settings paps_settings to webserver transmission """
    ...
    def __init__(self, settings=None):
        ...
        """ Handle to paps_settings client protocol
    ...
    def _plugin_get(self, plugin_name):
        """
        Find paps_settings in controller

        :param plugin_name: Name of the paps_settings to find
        :type plugin_name: str | None
        :return: Plugin or None and error message
        :rtype: (audience.crowd.pluginInterface.Plugin | None, str)
        """
        if not plugin_name:
        ...
        for plugin in self.controller.plugins:
            if plugin.name == plugin_name:
                return plugin, ""
        return None, u"Plugin '{}' not found".format(plugin_name)

    def handle_msg(self, payload):
        """
        Handle message for network paps_settings protocol
        ...
        if msg == "plugin_list":
            ...
            res['paps_settings'] = plugin.get_info()
            return res
            ...
            try:
                ...
                if str(e) == "No resource path set":
                    ...
                    self.exception(
                        u"Failed to get resource list for paps_settings '{}'".format(
                            plugin.name
                        )
                    )
            return res
