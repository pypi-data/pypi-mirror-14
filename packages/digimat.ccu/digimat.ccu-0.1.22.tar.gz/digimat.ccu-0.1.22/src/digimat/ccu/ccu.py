...
class CCUFlag(object):
	...
	def value(self, value):
		with self._lock:
			value=bool(value)
			self._stampLastActivity=time.time()
	...
	def elapsed(self, delay):
        ...
        def ageLastActivity(self):
                with self._lock:
                        return time.time()-self._stampLastActivity

        def elapsedSinceLastActivity(self, delay):
                with self._lock:
                        return self.ageLastActivity()>=delay
	...
	def wasTriggered(self, sinceDelay=3.0):
		for channel in self.channels():
			for flag in channel.values():
				if not flag.elapsedSinceLastActivity(sinceDelay):
					return True
