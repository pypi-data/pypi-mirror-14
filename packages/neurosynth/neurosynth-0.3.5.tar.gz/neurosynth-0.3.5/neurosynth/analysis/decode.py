...
class Decoder:
    ...
    def plot_polar(self, data, n_top=3, overplot=False, labels=None,
                   palette='husl', size=6, ax=None, features=None,
                   color=None, ylim=None, yticks=None):
        ...
        for i in range(n_panels):
            ...
            ax.fill(theta, d, color=col, alpha=alpha, ec='k',
                    linewidth=0)
            ax.fill(theta, d, alpha=1.0, ec=col,
            ...
            if ylim is not None:
                ax.set_ylim(ylim)
        # plt.tight_layout()
        return ax if overplot else axes
