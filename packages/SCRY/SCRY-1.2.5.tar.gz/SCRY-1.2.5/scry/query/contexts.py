...
from scry.utility                  import SCRY, SCRYError, EmptyListError

# Superclass of OrbHandler, ProcedureHandler, VarProcedureHandler, ValuesHandler, FilterHandler and BindHandler
class ContextHandler(object):
    ...
    # Used by some context handlers to combine valid inputs from the outputs of their dependencies
...
# The most commonly used class in this file -- executes calls to procedures specified by its PAU
class ProcedureHandler(ContextHandler):
    def __init__(self,query_handler,proc,pau):
        super(ProcedureHandler,self).__init__(query_handler)
...
### END OF ProcedureHandler


class VarProcedureHandler(ProcedureHandler):
    def __init__(self,query_handler,var):
        super(VarProcedureHandler,self).__init__(query_handler,None,None)
        self.subject = var
        self.input_vars.add(var)
    
    def execute(self):
        dep_binds  = [dep.bindings for dep in self.dependencies]
        procedures = self.query_handler.procedures
        ...
        for pau in paus:
            ...
            proc      = procedures[short_uri]
            
            known_in, var_in, default_in = self.get_input_specifiers(proc)
            out_spec, default_out        = self.get_output_specifiers(proc)
            var_nodes                    = [var_in[k] for k in var_in]
            var_in_values                = self.merge_and_filter(var_nodes,dep_binds)
            
            constants  = Graph()
            constants += self.get_descriptions(proc,pau)
            for spec in known_in:
                t = known_in[spec]
                constants.add((pau,t[1],t[2]))
            ...
            for d in var_in_values:
                ...
                solution_dicts = proc.execute(in_dict,out_spec.keys(),self.query_handler)
...
class OrbHandler(ContextHandler):
    ...
    def describe_orb(self):
        ...
        return self.query_handler.orb_desc
...
class BindHandler(ContextHandler):
    ...
    def execute(self):
        ...
        if self.dependencies:
            ...
            for d in inputs:
                ctx = DummyContext(d)
                self.bindings.append({self.algebra.var:exp.eval(ctx)})

        else:
            if hasattr(exp,'eval'):
                exp = exp.eval(DummyContext({}))
