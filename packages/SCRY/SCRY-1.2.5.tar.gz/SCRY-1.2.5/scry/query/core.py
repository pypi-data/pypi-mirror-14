...
class QueryHandler(object):
    ...
    def parse_query(self):
        ...
        def parse_service_clauses(qry): # CAUTION: This breaks if any curly closing brackets } occur in Literals (or URIs)...
            
            sqry = qry.split('\n')
            for line in sqry:
                if line.strip().startswith('#'): sqry.remove(line)
            qry = '\n'.join(sqry)
            
            def pp_unparse(hit):
                hit = list(hit)
                for i, frag in enumerate(hit):
                    if isinstance(frag,pp.ParseResults):
                        hit[i] = '{%s}' % pp_unparse(hit[i])
                return ' '.join(hit)
            
            service_pattern      = pp.CaselessLiteral('SERVICE') + pp.Word(pp.printables) + pp.nestedExpr('{','}')
            self.service_clauses = [pp_unparse(hit[0][2]) for hit in service_pattern.scanString(qry)]
            prefix_pattern       = pp.CaselessLiteral('PREFIX') + pp.Word(pp.alphanums) + pp.Literal(':') + pp.nestedExpr('<','>')
            self.prefixes        = '\n'.join(['PREFIX %s: <%s>' % (p[0][1],p[0][3][0]) for p in prefix_pattern.scanString(qry)])
