...
import dcmpi.custom_info as dpci
        ...
        if method == 'pydicom':
            ...
            try:
                ...
                info.update(dpc.postprocess_info(
                    dcm, dpci.SESSION, lambda x, p: x.value, verbose))
            ...
            for group_id, group in sorted(groups.items()):
                ...
                try:
                    ...
                    info.update(dpc.postprocess_info(
                        dcm, dpci.ACQUISITION, lambda x, p: x.value,
                        verbose))
                    # information from protocol
                    if dpc.DCM_ID['hdr_nfo'] in dcm:
                        prot_src = dcm[dpc.DCM_ID['hdr_nfo']].value
                        prot = dpc.parse_protocol(dpc.get_protocol(prot_src))
                    else:
                        prot = {}
                    info.update(dpc.postprocess_info(
                        prot,
                        dpci.get_sequence_info(info, prot),
                        None, verbose))
                    # additional information: duration
                    field_id = 'Duration'
                    try:
            ...
            for src_id, in_filepath_list in sorted(sources.items()):
                ...
                for acq, series in groups.items():
                    ...
                    try:
                        ...
                        info.update(dpc.postprocess_info(
                            dcm, dpci.SERIES, lambda x, p: x.value,
                            verbose))
                if verbose > VERB_LVL['none']:
                    out_subpath = out_filepath[len(out_dirpath):]
                    print('Info:\t{}'.format(out_subpath))
