...
class Block(object):
    ...
    def getModelFromClass(self):
        # "BlockTest" will return "Test"
        model=self.__class__.__name__[5:]
        if not model:
            return 'rblock'
        return model
    ...
    def register(self, bclass):
        try:
            if bclass and issubclass(bclass, Block):
                with self._lock:
                    block=bclass(self, 999)
                    self._models[block.model.lower()]=bclass
                    self.logger.debug('block<%s> registered' % block.model)
                    return True
        except:
