...
from macropy.core import ast_repr, util, Captured
...
class ClassMatcher(Matcher):
    ...
    def var_names(self):
        return (util.flatten([matcher.var_names() 
            for matcher in self.positionalMatchers + list(self.kwMatchers.values())]))
...
def _rewrite_if(tree, var_name=None, **kw_args):
    ...
    macroed_match = ast.With(items=[ast.withitem(ast.Name('_matching',
                                                          ast.Load()), None)], body=[ast.Expr(tree.test)])
