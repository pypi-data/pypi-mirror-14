...
``--ssl [CERTIFICATE]`` Run the HTTP server with TLS encryption.
Optionally provide the full path to your SSL certificate, which may be
PGP encrypted. The file should contain the entire certificate chain,
including the CA certificate, if any. If no SSL certificate path is
provided, a temporary self-signed certificate will be generated for the
current curlbomb session and ``--pin`` will be turned on implicitly.
