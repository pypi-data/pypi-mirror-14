...
@click.option('--interval_days',default=1, help='interval_days to backup courses')
...
def main(studio,sessionid,csrftoken,courses_data_path,config_file,start,night,interval_days):
    if not start:
    ...
    if night:
        click.echo("将于夜里两点备份")
        schedule.every(int(interval_days)).days.at("02:00").do(studioConn.course_backup)
    else:
        print u"正在备份..."
