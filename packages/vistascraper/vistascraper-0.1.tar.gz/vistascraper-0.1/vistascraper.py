...
# Needs to be installed for click to function correctly:
# Create a virtualenv:
#   virtualenv venv
# Activate it:
#   source ./venv/bin/activate
# Install vistascraper:
#   pip install -e .
...
warn_continuous = (ostype == "Windows" and "SHELL" in os.environ) # Doesn't handle SIGINT. 'os.environ' is sometimes case-sensitive
...
def errorQuit(string):
	click.echo(string)
	sys.exit(1)

# How many different channel name halves occur the same number of times? Alphabetical sorting
# Expects: List using channel name halves and how many times used: (TEXT, occurences)
# Lists are mutable so I don't need to return anything
def sortEqualCommon(lst):
	sublst = [] # In here we will store a list with elements to be sorted alphabetically
	if (lst[0][0] is None): start = 2 # Don't stumble on the most common [(None, 10+), ...] first element
	else: start = 1
	for i in range(start, len(lst)):
		if (lst[i-1][1] == lst[i][1]): sublst.append(lst[i-1])
		else:
			sublst.append(lst[i-1]) # We might have forgotten this one
			break # Only does immediate one, the rest will be checked later
	sublst = sorted(sublst, key=lambda x: x[0]) # Sorted by the name field (e.g. "NAME, 3"")
	start -= 1 # To accomodate for 0-index not 1-index
	lst[start:start+len(sublst)] = sublst # Replace that subsection with alphabetically sorted name parts
...
def mostCommon(lst):
	...
	except Exception as error: errorQuit(error)
	indexes = []
	while (True): # Records and removes most names with a side which is most common until all done
		l = Counter(tr[0]).most_common() # Sorted list by most common, left side
		sortEqualCommon(l) # Removes any ambiguity in channel name sorting
		...
		sortEqualCommon(r) # Removes any ambiguity in channel name sorting
...
@click.option("-n", "--no-overtype", is_flag=True, help="Disables overwriting text when --times is greater than 1 or --continuous is set")
...
def cli(version, url, ip, verbose, quiet, times, continuous, append, out, delay, no_overtype): # Any dashes are converted to underscores here
	# Arguments can't have helptext, so should be documented here TODO
	'''ISIS Vista scraper help description'''
	...
	if (times == 1) and not (continuous): no_overtype = True # We shouldn't truncate the output
	...
	def echoTemp(*args):
		...
		if (winY > lines+2) or (no_overtype):
			stringBuffer += " ".join(map(str, args))+"\n" # Emulates 'print()' behaviour for comma separated args
		elif (winY == lines+2):
			stringBuffer += "~~~ Output Truncated. Perhaps enable '--no-overtype' or allow more rows in your terminal ~~~\n"
	...
	if (ip):
		if not (url):
			...
			errorQuit("URL and IP address both supplied")
	if (warn_continuous) and (continuous or times > 1):
		errorQuit("WARNING: Running in continuously in Cygwin using Windows Python can not be quit internally!")
	iteration = 0
	...
	while (continuous or iteration < times):
		...
		try:
			...
			errorQuit("No URL or IP address supplied")
		except requests.exceptions.InvalidSchema:
			errorQuit("Protocol unsupported")
		except requests.ConnectionError:
			errorQuit("Connection error\nURL: "+url)
		except requests.HTTPError:
			errorQuit("HTTPError\nURL: "+url)
		except requests.Timeout:
			errorQuit("Request timed out\nURL: "+url)
		except requests.TooManyRedirects:
			errorQuit("TooManyRedirects\nURL: "+url)
		except requests.RequestException as e:
			errorQuit(e+"\nURL: "+url)
		except:
			errorQuit("Requests module: Something went wrong\nURL: "+url)
		if (response.status_code != 200):
			errorQuit("HTTP code "+str(response.status_code)+"\nURL: "+url)
		...
		indexes = mostCommon(codes)
		for i in indexes:
			echoTemp(click.wrap_text(codes[i]+" "+values[i], width=winX, subsequent_indent=indent))
			row.append(values[i])
		...
		for csvfile in out:
			try:
				...
				click.echo(e)
				errorQuit("Error: \'"+csvfile.name+"\' may be open in another program")
				break
		if (iteration > 0):
			if (no_overtype): stringBuffer = "\n"+stringBuffer # Seperate each scrape
			flushBuffer() # Prints stringBuffer after clearing screen
