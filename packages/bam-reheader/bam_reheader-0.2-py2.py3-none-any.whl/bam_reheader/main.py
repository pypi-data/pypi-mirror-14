...
def get_bam_header(bam_path, step_dir, engine, logger):
    ...
    cmd = ['samtools', 'view', '-H', bam_path, '>', header_path]
    shell_cmd = ' '.join(cmd)
    pipe_util.do_shell_command(shell_cmd, logger)
    return header_path

def reheader_bam(uuid, bam_path, engine, logger):
    step_dir = os.getcwd()
    bam_name = os.path.basename(bam_path)
    if pipe_util.already_step(step_dir, bam_name + '_reheader', logger):
        ...
        gdc_sq_header_path = os.path.join(home_dir, '.virtualenvs', 'p2_' + uuid, 'lib', 'python'+python_version, 'site-packages', 'bam-reheader' , 'GRCh38.d1.vd1.dict')
        ...
        #reheader BAM
        cmd = [ 'samtools', 'reheader', reheader_path, bam_path, '>', bam_name ]
        shell_cmd = ' '.join(cmd)
...
def main():
    ...
    if db_cred_s3url is not None: #db server case
        ...
        sqlite_name =  bam_name + '_samtools_reheader.db'
    ...
    bam_name = reheader_bam(case_id, bam_path, engine, logger)
