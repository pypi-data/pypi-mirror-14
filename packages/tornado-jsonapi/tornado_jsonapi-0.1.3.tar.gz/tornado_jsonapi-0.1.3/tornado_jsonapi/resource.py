...
class SQLAlchemyResource(Resource):
    class ResourceObject:
        def _json_attr(self, ob, name, type_, registry):
            if isinstance(type_[0], abc.ABCMeta):
                return None
            else:
                return alchemyjsonschema.dictify.jsonify_of(ob, name, type_,
                                                            registry)

        def _json_rel(self, ob, name, type_, registry):
            if not isinstance(type_[0], abc.ABCMeta):
                return None
            print(ob, name, type_)
            return 'http://rawr'
        ...
        def attributes(self):
            return alchemyjsonschema.dictify.jsonify(self.model,
                                                     ...
                                                     convert=self._json_attr)

        def relationships(self):
            return alchemyjsonschema.dictify.jsonify(self.model,
                                                     self.resource.schema,
                                                     convert=self._json_rel)
