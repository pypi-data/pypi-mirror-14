...
class Interval(_InternalInterval):
    ...
    Intervals with the same starting and ending time are considered empty, and are represented with
    EMPTY_INTERVAL. The boolean value of EMPTY_INTERVAL is false. All other intervals evaluate to True.
    ...
    def __new__(cls, begin, end):
        ...
        assert (isinstance(begin, datetime.datetime))  # TODO: generalize for any ordered type
...
EMPTY_INTERVAL = Interval(UTC_ZERO, UTC_ZERO) # TODO: how generalize this for any ordered type?


class IntervalSet(object):
    """A set of datetime intervals. Immutable.

    To create a new set:

        IntervalSet( interval1, interval2, interval3... )

    Arguments should be Interval objects. The order is insigificant - they will automatically be unified and ordered
    by the constructor. Empty intervals will be simply ignored (and absent from the set). Sets can be iterated
    over. Operators on interval sets: "+" union, "*" intersection and "-" difference.
        ...
        try:
            while True:
                ...
                if b1 < b2 <= e2 <= e1:  # i1 contains i2
                    new_items.append(Interval(b1, e2))
                    b1 = e2
    ...
    def __contains__(self, interval):
        """Tell if the given interval is contained *completely* in this set."""
