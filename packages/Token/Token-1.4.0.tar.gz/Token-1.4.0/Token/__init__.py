...
class ProviderService(CPA):
    def __init__(self, env='prod'):
        if env == 'dev':
            self.host = 'http://provider.dev.api.token.io:80'
        else:
            self.host = 'http://provider.api.token.io:80'
...
class BankService(CBA):
    def __init__(self, env='prod'):
        if env == 'dev':
            self.host = 'http://bank.dev.api.token.io:80'
        else:
            self.host = 'http://bank.api.token.io:80'
        super(BankService, self).__init__(self.host)
    def extra(self):
        print("extra")
    def create_access(self, client, account, provider_code):
        request = {
          'sessionId': '',
...
with open('config.json') as json_data_file:
    keys = json.load(json_data_file)["keys"]
