...
def help():
    ...
    print("+-------------------------------------------------------------+")
    print("| ACCOUNTS                                                    |")
    print("| client: 'alice', account: 'alice'                           |")
    print("| client: 'bob', account: 'bob'                               |")
    print("+-------------------------------------------------------------+")
    print("| COMMANDS - Token                                            |")
    print("| Token.ProviderService()                                     |")
    print("| Token.BankService()                                         |")
...
class ProviderService(CPA):
    ...
    def create_member(self, request):
        return self.create_member_route(request)
    def create_alias(self, alias_code, request):
        auth = create_authorization('POST', 'http://provider.dev.api.token.io:80/aliases/' + \
        alias_code, request)
        return self.create_alias_route(alias_code, request, authorization=auth)
    def get_accounts(self):
        auth = create_authorization('GET', 'http://provider.dev.api.token.io:80/accounts')
    ...
    def create_account(self, request):
        auth = create_authorization('POST', 'http://provider.dev.api.token.io:80/accounts',
            request)
        return self.create_account_route(request, authorization=auth)
    def get_tokens(self):
        auth = create_authorization('GET', 'http://provider.dev.api.token.io:80/tokens')
        return self.get_tokens_route(0, 100, authorization=auth)
    def get_token(self, token_id):
        auth = create_authorization('GET', 'http://provider.dev.api.token.io:80/tokens/' + \
            quote(token_id))
        return self.get_token_route(token_id, authorization=auth)
    def create_token(self, request):
        auth = create_authorization('POST', 'http://provider.dev.api.token.io:80/tokens', request)
        return self.create_alias_token_route(request, authorization=auth)
    def endorse_token(self, token_id, request):
        auth = create_authorization('PUT', 'http://provider.dev.api.token.io:80/tokens/' + \
        quote(token_id) + '/endorse', request)
        return self.endorse_token_route(token_id, request, authorization=auth)
    def create_payment(self, token_id, request):
        auth = create_authorization('POST', 'http://provider.dev.api.token.io:80/tokens/' + \
...
class BankService(CBA):
    def extra(self):
        print("extra")
    def create_access(self, client, account, request):
        auth = create_authorization('POST', 'http://bank.dev.api.token.io:80/clients/' +
...
keys = {
    "bank": "19604397d19692010ef1dd8eee1f50978f98268b6ca18c7a6aae700d7329446a",
    "provider" : "cc735fe6f304fb8f5c6a4f24e6095d68ca2b015ff7f421c89308102f894840a2",
}
...
def set_context(data, sk):
    ...
    if hasattr(data, 'device'):
        subject_id = "provider-test-member-" + data.id + "-device-" + data.device.id
    else:
        subject_id = data
    context = {
