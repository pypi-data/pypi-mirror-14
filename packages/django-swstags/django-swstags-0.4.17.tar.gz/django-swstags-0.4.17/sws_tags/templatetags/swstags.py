...
def tag_individual_filters(view_name, name, filters, URL_INDIVIDUAL_FILTERS):
	...
	for f in filters:
		new_filters.append((f[3], f[0]))
		# new_filters[f[3]] = f[0]
		if f[2] != '':
			default_value_filters[f[0]] = f[2]

	# new_filters = {'Routetable': 'routetable', 'Client': 'client','Destination': 'destination', 'Provider': 'provider'}
	new_filters = SortedDict(new_filters)
	...
	if col_format:
		...
		for col in col_model:
			...
			for k,v in col.iteritems():
				col_aux[k]=v
			if col['name'] in col_format:
				if col_format[col['name']]:
					for k,v in col_format[col['name']].iteritems():
						col_aux[k]=v
			col_model_aux.append(col_aux)
		col_model = col_model_aux
...
def tag_grid_dict(view_name,
	...
	conf_option_default['datatype'] = 'json'
...
def tag_highcharts(view_name,data_highcharts, plot_options = {},chart={}):
	...
	if 'v_dashstyle' in data_highcharts:
		v_dashstyle = data_highcharts['v_dashstyle']
	else:
		v_dashstyle = []

	if 'v_marker' in data_highcharts:
		v_marker = data_highcharts['v_marker']
	else:
		v_marker = []

	series = get_series(v_name, v_type, v_yAxis, v_data, v_color, v_stack, v_dashstyle, v_marker)
