...
def processMessages(message, typeMessage, timeOut=10000, rsyslog=False):
	...
	if rsyslog:
		try:
			...
			getattr(logger, typeMessage)(' view: ' + called_from_view + ', user: ' + str(ThreadLocal.get_current_user()) + ', message: ' + message)
		except:
			pass
...
def sendMessages(channel, connection_redis, message='No messages', level='broadcast', time=5000, rsyslog=False):
	...
	if rsyslog:
		try:
			called_from_view = getTraceBackInfo()
			getattr(logger, typeMessage)(' channel: ' + channel + ', user: ' + str(ThreadLocal.get_current_user()) + ', message: ' + message)
		except:
			pass
...
def sendEvent(channel, connection_redis, event='', data='', rsyslog=False):
