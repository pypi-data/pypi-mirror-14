...
class Tracker(object):
    ...
    def query_isbn(self, isbn, tools_server=None):
        if "000" in isbn:
            raise BadUploadException("query_isbn: %s is not a valid ISBN" % isbn)
    ...
    def _validate_book_author(self, book_author, validation=None):
        if validation and 'book_author' in validation.keys():
            if book_author != validation['book_author']:
                raise ValidationException("_validate_book_author: %s is not equal to %s" % (book_author, validation['book_author']))

        # Avoid blank author names
        if not book_author:
            raise BadUploadException("all_ebook_format: book_author is blank")

        # ISBN of 0000000000 will be Steve Young,
        # [size=3][b]Book Details:[/b][/size]
        # [quote]
        # [b]ASIN:[/b] B00RF1ZFTS
        # [b]Publisher:[/b] Deseret Book
        # [b]Publication Date:[/b] 2014-12-23
        # [b]Number of Pages:[/b] 12
        # [b]Website:[/b] [url=http://amzn.com/B00RF1ZFTS]Amazon[/url],
        # [url=http://www.librarything.com/isbn/B00RF1ZFTS]LibraryThing[/url],
        # [url=http://books.google.com/books?vid=ISBNB00RF1ZFTS]Google
        # Books[/url]
        # [/quote]
        # [size=3][b]Synopsis from Amazon:[/b][/size]
        # [quote]For the past two years I've helped Steve Young research and
        # write his life story for his family. I asked him if I could post an
        # excerpt. This is from a chapter called, "Ten Thousand Spirals." -Jeff
        # Benedict[/quote]
        if "Steve Young" in book_author:
            raise BadUploadException("all_ebook_format: book_author is %s, likely due to bad ISBN" % book_author)

        return True

    def add_ebook_format(self, url, isbn, torrent_file, format='MOBI', debug_write_to_file=False, validation=None):
        if url and not self.logged_in:
            raise NotLoggedInException("url requires login: %s" % url)
        ...
        try:
        ...
        except BadUploadException:
            self.logger.debug("BAD ISBN: %s, but will attempt validation" % isbn)

            if not validation:
                raise ValidationException("ISBN not valid and no validation passed: %s -> %s" % (isbn, validation))

            # pull these from the validation dict if it exists
            book_author = None
            book_publisher = "Unknown"
            book_year = str(datetime.date.today().year - random.randint(1,2))

            # Author
            if 'book_author' in validation.keys():
                self.br.form['book_author'] = validation['book_author']
            else:
                raise BadUploadException("No author available for this upload")

            # Publisher
            if 'book_publisher' in validation.keys():
                self.br.form['book_publisher'] = validation['book_publisher']
            else:
                self.br.form['book_publisher'] = book_publisher

            # Year
            if 'book_year' in validation.keys():
                self.br.form['book_year'] = validation['book_year']
            else:
            ...
            # Author
            book_author = query['Items'][0]['Author']

            if self._validate_book_author(book_author, validation=validation):
                self.br.form['book_author'] = book_author

            # Publisher
            book_publisher = query['Items'][0]['Publisher']
            if not book_publisher:
                # Avoid blank publisher names
                book_publisher = "Unknown"
            self.br.form['book_publisher'] = book_publisher
            # Year
            book_year = query['Items'][0]['ReleaseDate'].split('-')[0]

            # Use the publication date if year is not found
            if not book_year:
                book_year = query['Items'][0]['PublicationDate'].split('-')[0]

            if validation and 'book_year' in validation.keys():
                book_year = validation['book_year']

            # Don't allow blank years.
            # Alternatively, use a random previous year 1-2 years ago if the year
            # is blank or not a number:
            # book_year=str(datetime.date.today().year - random.randint(1,2))
            try:
                book_year = str(int(book_year))
            except ValueError:
                raise BadUploadException("all_ebook_format: book_year is bad")
    ...
    def parse_torrent_page(self, filename=None, doc=None, url=None):
        ...
        for line in bbcode:
            ...
            if "ISBN" in line:
                ...
                # Not good to use the ASIN as the ISBN because the author
                # lookups are incorrect. "0000000000" recommended otherwise
                #- mydict['isbn'] = asin
                mydict['isbn'] = "0000000000"
                self.logger.debug("ISBN not found, found ASIN (%s), using %s" % (asin, mydict['isbn']))
                break
        else:
            mydict['isbn'] = None
        ...
        for line in bbcode:
            if "Publisher" in line:
                ...
                self.logger.debug("Found publisher: %s" % publisher)
                break
        else:
            mydict['publisher'] = "Unknown"
