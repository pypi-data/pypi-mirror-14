...
import helpers
import urlparse
# Monkey patch httplib to fix unicode issues
# Apply this patch: https://bugs.python.org/file21747/python-2.7.1-fix-httplib-UnicodeDecodeError.patch
...
class Tracker(object):
    ...
    def login(self, invalidate_cookies=False):
        ...
        #dict((k.encode('ascii') if isinstance(k, unicode) else k,
                             #v.encode('ascii') if isinstance(v, unicode) else v)
                                                 #for k,v in self.headers.items())
        #resp.headers = dict((k.encode('ascii') if isinstance(k, unicode) else k,
                     #v.encode('ascii') if isinstance(v, unicode) else v)
        ...
        if os.path.isfile(os.path.join(self.wdir, self.cookies_file)) and not invalidate_cookies:
            ...
            print("Login headers are %s" % br.headers)
    ...
    def add_ebook_format(self, url, isbn, torrent_file, format='MOBI', debug_write_to_file=False):
        ...
        resp = self.br.open(url)
        self.logger.debug("HEADERS ARE %s" % self.br.request.headers)
        #dict((k.encode('ascii') if isinstance(k, unicode) else k,
                             #v.encode('ascii') if isinstance(v, unicode) else v)
                                                 #for k,v in self.headers.items())
        #resp.headers = dict((k.encode('ascii') if isinstance(k, unicode) else k,
        ...
        for form in self.br.forms():
            if form.attrs.get('id') == 'upload_table':
                self.br.form = form
                break
        self.logger.info("Get the right form headers are %s" % resp.info())
        ...
        # fill out the form
        #self.logger.debug(str(self.br.form).decode("utf8"))
        self.br.form['book_isbn'] = isbn.replace('-','')
        self.br.form['book_format'] = [format.upper(),]
        self.logger.debug("headers are: %s" % self.br.request.headers)

        # Convert the torrent file to unicode
        #my_target_fname = self._convert_file_to_new_encoding(source_encoding="iso-8859-1",
                                                             #target_encoding="utf-8",
                                                             #source_file=torrent_file)
        # now you may attach it
        self.br.form.add_file(open(torrent_file, 'r'), 'text/plain', torrent_file)
        #self.br.form.add_file(open(my_target_fname), 'text/plain', my_target_fname)
        #
        ...
        for info in resp.info():
            self.logger.info("info %s value %s" % (info, len(info)))
            #self.logger.info("info %s value %s" % (info, value))
            self.logger.info("info %s type is %s" % (info, type(info)))
        #headers = dict((k.encode('ascii') if isinstance(k, unicode) else k, v.encode('ascii') if isinstance(v, unicode) else v) for k,v in self.br.request.headers.items())
