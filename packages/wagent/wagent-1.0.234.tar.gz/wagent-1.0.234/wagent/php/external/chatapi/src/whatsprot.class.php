...
class WhatsProt
{
    ...
    protected $deliveredReceipts = true;
    public $retryNodes = [];
    ...
    public function __construct($number, $nickname, $debug = false, $log = false, $datafolder = null)
    {
        ...
        if ($datafolder !== null && file_exists($datafolder)) {
            ...
            $this->dataFolder = getcwd() . DIRECTORY_SEPARATOR . Constants::DATA_FOLDER . DIRECTORY_SEPARATOR;
        }
    ...
    private function ensureDataFolderExists()
    {
      $mediaFolder = $this->dataFolder . Constants::MEDIA_FOLDER;
      $picturesFolder = $this->dataFolder . Constants::PICTURES_FOLDER;

      if (!file_exists ($mediaFolder))
          mkdir ($mediaFolder, 0777, true);

      if (!file_exists ($picturesFolder))
          mkdir ($picturesFolder, 0777, true);
    }
    ...
    public function connect()
    {
        ...
        if ($socket === false)
            $socket = socket_create(AF_INET, SOCK_STREAM, 0); // if socket creation failed, try without protocol flag.

        if ($socket !== false) {
            $host = 'e' . rand(1, 16) . '.whatsapp.net';
            $result = socket_connect($socket, $host, Constants::PORT);
            if ($result === false) {
                $socket = false;
                $error = 'Fail to connecting to host socket_connect (' .$host. ')';
            }
        } else {
            $error = 'Fail to create socket socket_create(AF_INET, SOCK_STREAM, SOL_TCP);';
        }
    ...
    /**
     *
     * Enable / Disable automatic delivered receipt
     * This is enabled by default
     */
    public function enableDeliveredReceipt($enable)
    {
        $this->deliveredReceipts = $enable;
    }
     ...
     * @return bool
     */
    public function pollMessage($timeout_ms = 0)
    {
        if (!$this->isConnected()) {
        ...
        if ($timeout_ms == 0)
            ...
            $s = socket_select($r, $w, $e, 0, $timeout_ms);
     ...
     */
    public function sendBroadcastAudio($targets, $path, $storeURLmedia = false, $fsize = 0, $fhash = '', $id = null)
    {
        if (!is_array($targets)) {
            $targets = [$targets];
        }
        // Return message ID. Make pull request for this.
        return  $this->sendMessageAudio($targets, $path, $storeURLmedia, $fsize, $fhash, $id);
    }
     ...
     * @return string|null Message ID if successfully, null if not.
     */
    public function sendBroadcastImage($targets, $path, $storeURLmedia = false, $fsize = 0, $fhash = '', $caption = '', $id = null)
    {
        if (!is_array($targets)) {
            $targets = [$targets];
        }
        // Return message ID. Make pull request for this.
        return  $this->sendMessageImage($targets, $path, $storeURLmedia, $fsize, $fhash, $caption, $id);
    }
     ...
     */
    public function sendBroadcastLocation($targets, $long, $lat, $name = null, $url = null, $id = null)
    {
        if (!is_array($targets)) {
            $targets = [$targets];
        }
        // Return message ID. Make pull request for this.
        return $this->sendMessageLocation($targets, $long, $lat, $name, $url, $id);
    }
     ...
     * @return string Message ID
     */
    public function sendBroadcastMessage($targets, $message, $id)
    {
        $bodyNode = new ProtocolNode('body', null, null, $message);
        // Return message ID. Make pull request for this.
        return $this->sendBroadcast($targets, $bodyNode, 'text', $id);
    }
     ...
     * @return string|null Message ID if successfully, null if not.
     */
    public function sendBroadcastVideo($targets, $path, $storeURLmedia = false, $fsize = 0, $fhash = '', $caption = '', $id = null)
    {
        if (!is_array($targets)) {
            $targets = [$targets];
        }
        // Return message ID. Make pull request for this.
        return $this->sendMessageVideo($targets, $path, $storeURLmedia, $fsize, $fhash, $caption, $id);
    }
     ...
     */
    public function sendDeleteBroadcastLists($lists, $msgId = null)
    {
        if (empty($msgId))
    ...
    public function sendGetClientConfig($msgId = null)
    {
        if (empty ($msgId))
            $msgId = $this->createIqId();
     ...
     */
    public function sendChangeNumber($number, $identity, $msgId = null)
    {
        if (empty($msgId))
            $msgId = $this->createIqId();
     ...
     */
    public function sendGetGroups($msgId = null)
    {
        $this->sendGetGroupsFiltered('participating', $msgId);
    }
     ...
     *    The group JID
     */
    public function sendGetGroupV2Info($groupID, $msgId = null)
    {
        if (empty($msgId)) {
     ...
     */
    public function sendGetPrivacyBlockedList($msgId = null)
    {
        if (!empty ($msgId)) {
            $this->nodeId['privacy'] = $msgId;
        } else {
     ...
     */
    public function sendGetPrivacySettings($msgId = null)
    {
        if (!empty ($msgId)) {
            $this->nodeId['privacy_settings'] = $msgId;
        } else {
     ...
     */
    public function sendSetPrivacySettings($category, $value, $msgId = null)
    {
        if (empty($msgId))
            $msgId = $this->createIqId();
     ...
     */
    public function sendGetProfilePicture($number, $large = false, $msgId = null)
    {
        if (!empty ($msgId)) {
            $this->nodeId['getprofilepic'] = $msgId;
        } else {
     ...
     * @return bool
     */
    public function sendGetProfilePhotoIds($numbers, $msgId = null)
    {
        if (!is_array($numbers)) {
            $numbers = [$numbers];
        }
     ...
     */
    public function sendGetServerProperties($id = null)
    {
        if (empty ($id))
            $id = $this->createIqId();
     ...
     *   Country
     */
    public function sendGetServicePricing($lg, $lc, $msgId = null)
    {
        if (empty($msgId))
            $msgId = $this->createIqId();
     ...
     */
    public function sendExtendAccount($msgId = null)
    {
        if (empty($msgId))
            $msgId = $this->createIqId();
     ...
     */
    public function sendGetBroadcastLists($msgId = null)
    {
        if(empty($msgId)) {
            $this->nodeId['get_lists'] = $this->createIqId();
            $msgId = $this->nodeId['get_lists'];
        } else {
            $this->nodeId['get_lists'] = $msgId;
        }
     ...
     */
    public function sendRemoveAccount($lg = null, $lc = null, $feedback = null, $msgId = null)
    {
        if (empty($msgId))
            $msgId = $this->createIqId();
     ...
     */
    public function sendGetStatuses($jids, $iqId = null)
    {
        if (!is_array($jids)) {
            $jids = [$jids];
        }
     ...
     *                The group ID.
     */
    public function sendGroupsChatCreate($subject, $participants, $id = null)
    {
     ...
     */
    public function sendSetGroupSubject($gjid, $subject, $id = null)
    {
        if (!empty($id)) {
            $this->nodeId['group_subject'] = $id;
        } else {
            $this->nodeId['group_subject'] = $this->createIqId();
            $id = $this->nodeId['group_subject'];
        }
     ...
     */
    public function sendGroupsLeave($gjids, $msgId = null)
    {
        if (!empty($msgId)) {
            $this->nodeId['leavegroup'] = $msgId;
        } else {
    ...
    public function sendGroupsParticipantsAdd($groupId, $participant, $msgId = null)
    {
        if (empty($msgId)) {
            $msgId = $this->createIqId();
            $this->nodeId['participantadd'] = $msgId;
        } else {
            $this->nodeId['participantadd'] = $msgId;
        }
     ...
     */
    public function sendGroupsParticipantsRemove($groupId, $participant, $msgId = null)
    {
        if (empty($msgId)) {
            $msgId = $this->createIqId();
            $this->nodeId['participantremove'] = $msgId;
        } else {
            $this->nodeId['participantremove'] = $msgId;
        }
     ...
     */
    public function sendPromoteParticipants($gId, $participant,  $msgId = null)
    {
         if (empty($msgId))
     ...
     */
    public function sendDemoteParticipants($gId, $participant, $msgId = null)
    {
        if (empty($msgId))
            $msgId = $this->createMsgId();
     ...
     * @return string Message ID.
     */
    public function sendMessage($to, $plaintext, $force_plain = false, $id = null)
    {
        ...
        $id = $this->sendMessageNode($to, $msgNode, $id, $plaintextNode);
     ...
     * @return string|null Message ID if successfully, null if not.
     */
    public function sendMessageAudio($to, $filepath, $storeURLmedia = false, $fsize = 0, $fhash = '', $voice = false, $id = null)
    {
        $this->voice = $voice;

        if ($fsize != 0 && $fhash != '') {
            return $this->sendRequestFileUpload($fhash, 'audio', $fsize, $filepath, $to, '', $id);
        }
        ...
        // Return message ID. Make pull request for this.
        return $this->sendCheckAndSendMedia($filepath, $size, $to, 'audio', $allowedExtensions, $storeURLmedia, '', $id);
    }
     ...
     * @return string|null Message ID if successfully, null if not.
     */
    public function sendMessageImage($to, $filepath, $storeURLmedia = false, $fsize = 0, $fhash = '', $caption = '', $id = null, $encrypted = false)
    {
        if ($fsize != 0 && $fhash != '' && !$encrypted) {
            return $this->sendRequestFileUpload($fhash, 'image', $fsize, $filepath, $to, $caption, $id);
        }

        $allowedExtensions = ['jpg', 'jpeg', 'gif', 'png'];
        $size = 5 * 1024 * 1024; // Easy way to set maximum file size for this media type.
        // Return message ID. Make pull request for this.
        return $this->sendCheckAndSendMedia($filepath, $size, $to, 'image', $allowedExtensions, $storeURLmedia, $caption, $id);
    }
     ...
     * @return string Message ID
     */
    public function sendMessageLocation($to, $long, $lat, $name = null, $url = null, $id = null)
    {
        ...
        $id = (is_array($to)) ? $this->sendBroadcast($to, $mediaNode, 'media') : $this->sendMessageNode($to, $mediaNode, $id);
     ...
     * @return string|null Message ID if successfully, null if not.
     */
    public function sendMessageVideo($to, $filepath, $storeURLmedia = false, $fsize = 0, $fhash = '', $caption = '', $id = null)
    {
        if ($fsize != 0 && $fhash != '') {
            return $this->sendRequestFileUpload($fhash, 'video', $fsize, $filepath, $to, $caption, $id);
        }

        $allowedExtensions = ['3gp', 'mp4', 'mov', 'avi'];
        $size = 70 * 1024 * 1024; // Easy way to set maximum file size for this media type.
        // Return message ID. Make pull request for this.
        return $this->sendCheckAndSendMedia($filepath, $size, $to, 'video', $allowedExtensions, $storeURLmedia, $caption, $id);
    }
     ...
     */
    public function sendSetGroupPicture($gjid, $path, $id = null)
    {
        $this->sendSetPicture($gjid, $path, $id);
    }
     ...
     */
    public function sendSetProfilePicture($path, $id = null)
    {
        $this->sendSetPicture($this->phoneNumber, $path, $id);
    }

    /*
     *	Removes the profile photo.
     */
    public function sendRemoveProfilePicture($msgId = null)
    {
        if (empty($msgId)) {
            $msgId = $this->createIqId();
            $this->nodeId['setpicture'] = $msgId;
        } else {
            $this->nodeId['setpicture'] = $msgId;
        }
     ...
     */
    public function sendStatusUpdate($txt, $nodeID = null)
    {
        $child = new ProtocolNode('status', null, null, $txt);

        if (empty($nodeID)) {
            $nodeID = $this->createIqId();
            $this->nodeId['statusupdate'] = $nodeID;
        } else {
            $this->nodeId['statusupdate'] = $nodeID;
        }
     ...
     * @return string Message ID
     */
    public function sendVcard($to, $name, $vCard, $id = null)
    {
        ...
        // Return message ID. Make pull request for this.
        return $this->sendMessageNode($to, $mediaNode, $id);
    }
     ...
     * @return string Message ID
     */
    public function sendBroadcastVcard($targets, $name, $vCard, $id = null)
    {
        $vCardNode = new ProtocolNode('vcard',
            [
                'name' => $name,
        ...
        // Return message ID. Make pull request for this.
        return $this->sendBroadcast($targets, $mediaNode, 'media', $id);
    }
    ...
    /**
     * Does the same thing of waitSever but using a smaller timeout for pollMessage
     */
    public function waitServerEvent($id, $timeout = 5)
    {
        $time = time();
        $this->serverReceivedId = false;
        do {
            $this->pollMessage(100);
            usleep(10);
        } while ($this->serverReceivedId !== $id && time() - $time < $timeout);
    }
    ...
    public function sendSync($numbers, $deletedNumbers = null, $syncType = 3, $id = null)
    {
        $users = [];
        if (!is_array($numbers)) {
            $numbers = [$numbers];
...
//          $height = $duplicate->getAttribute("height");
            ...
            //upload new file
            if (count($this->mediaFileInfo) == 0 && !empty($this->mediaQueue[$id]['filePath']))
                $this->getMediaFile($this->mediaQueue[$id]['filePath']);
...
//          $height = $json->height;
     ...
     * @return string|null Message ID if successfully, null if not.
     */
    protected function sendCheckAndSendMedia($filepath, $maxSize, $to, $type, $allowedExtensions, $storeURLmedia, $caption = '', $msgid = null)
    {
        if ($this->getMediaFile($filepath, $maxSize) == true) {
            if (in_array(strtolower($this->mediaFileInfo['fileextension']), $allowedExtensions)) {
                ...
                $id = $this->sendRequestFileUpload($b64hash, $type, $this->mediaFileInfo['filesize'], $this->mediaFileInfo['filepath'], $to, $caption, $msgid);
     ...
     * @return string
     */
    protected function sendBroadcast($targets, $node, $type, $msgId = null)
    {
        if (!is_array($targets)) {
            $targets = [$targets];
        }
     ...
     */
    protected function sendGetGroupsFiltered($type, $msgID)
    {
        if (empty($msgID)) {
            $msgID = $this->createIqId();
            $this->nodeId['getgroups'] = $msgID;
        } else {
            $this->nodeId['getgroups'] = $msgID;
        }
     ...
     */
    public function sendMessageRead($to, $id, $type = 'read', $participant = null)
    {
        $listNode = null;
        $idNode = $id;
        if (is_array($id)) {
        ...
        if (!empty($type))
            $messageHash ['type'] = $type;
        if($participant != null)
            $messageHash ['participant'] = $this->getJID($participant);

        $messageNode = new ProtocolNode('receipt', $messageHash, $listNode, null);
        $this->sendNode($messageNode);
    }
     ...
     * @return string Message ID
     */
    protected function sendRequestFileUpload($b64hash, $type, $size, $filepath, $to, $caption = '', $id = null)
    {
        if (empty($id))
            $id = $this->createIqId();
     ...
     */
    protected function sendSetPicture($jid, $filepath, $nodeID = null)
    {
        if (empty($nodeID)) {
            $nodeID = $this->createIqId();
            $this->nodeId['setpicture'] = $nodeID;
        } else {
            $this->nodeId['setpicture'] = $nodeID;
        }
