...
class AgentServer extends Server
{
    ...
    {
        $temp_file = tempnam(sys_get_temp_dir(), $this->number);
        return $temp_file;
    }
    ...
    private function GetDataContentFile($data)
    {
        $temp_file = $this->GetTempFileName();
                                     ...
                                     )
    {
        ...
        $params ['data'] = $this->GetDataContentFile($data);
    ...
    function SendGetPresences($to)
    {
        ...
        while ($total != count($numbers)) {
            ...
            $event = $this->GetEventFromIndex($index);
            $data = $event['data'];
            $total = count ($data['connected_status']);
        }

        foreach ($numbers as $number)
            $this->wa->sendPresenceUnsubscription($number);

        return $this->GetEventResult('onpresence');
    }
