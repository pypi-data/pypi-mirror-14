...
def parse_arguments(args=None):
    parser = argparse.ArgumentParser(
        ...
        'generated by the tool ``computeMatrix``. In contrast to '
                    'plotHeatmap, this option simplifies the layout'
                    'by removing boxes around individual heatmaps and'
                    'groups. ',
        epilog='An example usage is: plotHeatmap_simple -m <matrix file>',
        add_help=False)
...
def prepare_layout(hm_matrix, heatmapsize, showSummaryPlot, showColorbar, perGroup, colorbar_position):
    ...
    if showColorbar:
        if colorbar_position == 'below':
            numrows += 2 # an spacer needs to be added
                         # to avoid overlaps
...
def plotMatrix(hm, outFileName,
    ...
    for sample in range(hm.matrix.get_num_samples()):
        ...
        for group in range(numgroups):
            ...
            if group == first_group and not showSummaryPlot and not perGroup:
                ...
                if title in ['Cap-H2', 'Rad21']:
                    sub_matrix['matrix'] *= 1.5

                elif title == 'M1BP':
                    sub_matrix['matrix'] *= 1.2

                elif title == 'Mod_mdg4':
                    sub_matrix['matrix'] *= 1.8

                elif title in ['CP190', 'Beaf-32', 'CTCF']:
                    sub_matrix['matrix'] *= 0.8
    ...
    if showColorbar and colorbar_position != 'below':
        if showSummaryPlot:
            ...
            # over the profiles ans spacer top rows
            grid_start = 2
        else:
            grid_start = 0
...
def main(args=None):
    ...
    if np.any(group_len_ratio < 5.0 / 1000):
        ...
        sys.stderr.write("WARNING: Group '{}' is too small for plotting, you might want to remove it. "
                         "There will likely be an error message from matplotlib "
