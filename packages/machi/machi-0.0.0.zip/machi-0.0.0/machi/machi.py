...
####################################
#
#  should need to do this only once
...
class Machina:
    ...
    def __init__(self,username,password,serverURL='https://charts.machi.na/'):
        self.session={}
    ...
    def requestMachina(self,url,parameters={},method='POST',auth=None):
        if(self.get('modelName')!=None):
            parameters['modelName']=self.get('modelName')
        hasToken= self.get('access_token') != None
    ...
    def openModel(self,modelName='default'):
        ...
        #message={'messageType':'OpenModel'}
        #self.set('model',self.sendMessageToMW(message))
    ...
    def closeModel(self):
        ...
        #message={'messageType':'CloseModel'}
    ...
    #        if('backtest' in row):
    #            bt= row['backtest']
    #            print ' | '+bt['backtestName']+' | '+bt['pnl']+' | '+bt['ntrades']+' | '+bt['sharpe']+' | '+bt['sortino'],
    ...
    def addRow(self,query,updateRows=True,startTime='',endTime='',includeData=False):
        message={'messageType':'AddRow','query':query,'startTime':startTime,'endTime':endTime,'includeData':includeData}
        result= self.sendMessageToMW(message)
        if(updateRows):#Update the model's rows
            self.getModel()
        return result
    ...
    def getRow(self,rowIndex,startTime='',endTime='',includeData=False):
        message={'messageType':'GetRow','rowIndex':rowIndex,'startTime':startTime,'endTime':endTime,'includeData':includeData}
        return self.sendMessageToMW(message)
    

    #Get timeseries, optionally with data
    def getTimeSeries(self,query,startTime='',endTime='',includeData=False):
        message={'messageType':'GetTimeSeries','query':query,'startTime':startTime,'endTime':endTime,'includeData':includeData}
        return self.sendMessageToMW(message)
    ...
    def writeCSV(self,filename,timeSeriesData):
    ...
    def openRemoteChartPage(self,OpenOnlyIfNotAlreadyOpened=True):
        browserURL=self.buildURL('remoteChart?username='+self.get('username'))
        if OpenOnlyIfNotAlreadyOpened==False:
             webbrowser.open(browserURL)
        else:
            #check if the user already opened his browser
    ...
    def drawSeries(self,seriesDataList):
        ...
        self.requestMachina('api/user/remoteChart/sendMessagToRemoteChart',params)
        return self.openRemoteChartPage()
