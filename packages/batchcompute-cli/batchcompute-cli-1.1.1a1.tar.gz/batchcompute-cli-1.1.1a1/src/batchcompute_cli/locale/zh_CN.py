...
def __get_json():
    return {
        "login": {
            "description": u"使用阿里云AccessKey登录"
        },
        "config": {
            "description": u"配置管理",
            "detail": u"修改配置, 不带参数则查看配置",
            "option": {
                "region": u'设置区域, 如:cn-qingdao',
                "osspath": u'设置OSS路径',
                "locale": u'设置语言地域, 可选范围:[zh_CN|en]'
            }
        },
        "instance_type": {
            "description": u'显示资源类型列表.'
        },
        "cluster": {
            "description": u'获取集群列表, 获取集群详情.'
        },
        "job": {
            "description": u'获取作业列表, 获取作业,任务详情等.',
            "option": {
                'top': u'显示结果集行数, 只在获取作业列表是生效',
                'all': u'显示所有结果',
                'state': u'获取作业列表时, 按状态过滤, 取值范围: [Running|Stopped|Waiting|Finished|Failed]',
                "id": u'获取作业列表时, 按 JobId 模糊查询',
