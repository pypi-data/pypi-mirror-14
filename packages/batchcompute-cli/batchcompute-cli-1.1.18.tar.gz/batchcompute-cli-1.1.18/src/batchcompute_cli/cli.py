...
cmd_submit_job = Command('submit', alias=['sub'],
  ...
  Examples:
      ...
      3. %s sub "python main.py arg1 arg2" job_name -c cls-xxxx -p ./my_program/          # set job name, use cluster id, pack a folder and upload
...
cmd_submit_job.option("-c, --cluster [cluster]", MSG['submit']['option']['cluster'],
                      resolve=job_submit.trans_cluster)
...
cmd_submit_job.option("--file [file]",
                      MSG['submit']['option']['file'],
                      resolve=job_submit.trans_file)

cmd_submit_job.option("--show_json",
                      MSG['submit']['option']['show_json'], visible=False)
