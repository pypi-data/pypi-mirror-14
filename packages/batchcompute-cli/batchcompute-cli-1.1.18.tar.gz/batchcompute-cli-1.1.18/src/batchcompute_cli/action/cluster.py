...
def list():
    ...
    #print(gray('RequestId: %s' % result.get('RequestId')))
    ...
    arrlen = len(arr)
    cache_str =gray('(cache %s %s)' %  (arrlen, 'rows' if arrlen>1 else 'row' ) )
    print('%s %s' % ( green('Total: %s' % arrlen), cache_str))
...
def get(clusterId):
    ...
    print(gray('exec: bcs c %s' % clusterId))
    ...
    for groupName in result.Groups:
        ...
        ins = it.get_by_name(v['InstanceType'])
        if ins:
    ...
    cols = [('Starting',blue), ('Running',green), ('Stopping',yellow), ('Stopped',red),('Unallocated',gray)]
    t = []
    p = []
