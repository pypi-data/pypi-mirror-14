...
import calendar
import time
from ..util.isix import STRING
from terminal import green, gray, red, yellow, cyan
import os
import re
...
def format_date_in_arr(arr, cols):
    ...
    for m in arr:
        if 'StartTime' in cols and 'EndTime' in cols:
            m['Elapse'] = calc_elapse(m.get('StartTime'), m.get('EndTime'))
...
def calc_elapse(start, end):
    if start and end:
        return '%ds' % int(time.mktime(end.timetuple()) - time.mktime(start.timetuple()))
    else:
        return ''