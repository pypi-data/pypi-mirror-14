...
from .action import project_create,project_build,project_submit,project_status,project_update
...
def init(cmd_project):
    ...
    #cmd_project_status.option('-v, --validate', 'validate json fields')
    ...
    cmd_project_update = Command('update',
      ...
      Examples:
          1. %s p update -c img=%s:type=ecs.s3.large  # update all tasks, set AutoCluster
          2. %s p update -c cls-000010102             # update all tasks, set ClusterId
          3. %s p update find -c cls-000010102        # update find task only
          4. %s p update find -c cls-000010102        # update find task only''' % (
                             COMMAND, CMD, IMG_ID, CMD, CMD,CMD),
    ...
    #
    # # add task
    # cmd_add_task = Command('add_task', alias=['at'], description='add a task',
    #                        detail="add a task node into job.json, and create a promgram file in src folder.(%s)" % ( RUN_IN_PROJECT_FOLDER),
    #                        usage="""Usage: %s add_task|at [task_name] [option]
    #
    #   Examples:
    #       1. %s at Map                                          #add a task named Map
    #       2. %s at Map -d localhost:5000/myubuntu               #add a task named Map, use docker, you should tag you image using prefix:'localhost:5000/' first. if you forgot the port, type "docker ps" to see item named: registry_oss
    #       3. %s at Map -i img-00000000561CB0FE000061CC00000411  #add a task named Map, and set image id
    #       4. %s at Map -o oss://my-bucket/my-key/               # set oss oss_path""" % (COMMAND, CMD, CMD, CMD, CMD),
    #                        arguments=['task_name'],
    #                        func=add_task.add)
    # cmd_add_task.option('-i, --image_id [image_id]', 'set image_id for task, type "%s img" for more' % CMD,
    #                     resolve=update_task.transform_image_id)
    # cmd_add_task.option('-d, --docker_image [docker_image_name]', 'use docker image, type "docker images" for more')
    # cmd_add_task.option('-o, --oss_path [oss_path]', 'set oss path, and it will set value of PackageUri,StdoutRedirectPath and StderrRedirectPath',
    #                       resolve=update_task.transform_oss_path)
    # program.action(cmd_add_task)
    #
    #
    #
    # # remove task
