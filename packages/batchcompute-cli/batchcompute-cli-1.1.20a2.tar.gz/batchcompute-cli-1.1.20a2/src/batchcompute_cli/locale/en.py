...
def getJSON():
    return {
        ...
        'create_cluster': {
            ...
            'option': {
                'image': "optional, imageId, default: %s" % IMG_ID,
                'type': "optional, instanceType, type '%s it' for more, default: %s" % (CMD, INS_TYPE),
                'nodes': "optional, int, default: 1",
                #'groups': "optional, format: <groupName>[:type=<instanceType>][:nodes=<instanceCount>], default:group1",
                'description': "optional, description, string",
                'userData': "optional, user data k-v pairs, exp: a:b,c:e"
            }
        },
        'delete_cluster': {
        ...
        'submit': {
            ...
            'option': {
                'cluster': """optional, should be a cluster_id, or an auto_cluster string.
                                      ...
                                      (instanceType default: %s, type '%s it' for more instanceType). """ % (
                IMG_ID, INS_TYPE, CMD, INS_TYPE, CMD),
                'pack': "pack the files in the program_folder into worker.tar.gz, and upload to oss, if you don't set this option, no pack, no upload",
                ...
                'read_mount': """mount a oss path to local file system in read-only mode, kv-pairs, multi-supported,
                                      format: <oss_path>:<dir_path>[,<oss_path2>:<dir_path2>...],
                                      example: oss://bucket/key:/home/admin/ossdir """,
                'write_mount': """mount a oss path to local file system in writable mode
                                      ...
                                      example: oss://bucket/key:/home/admin/ossdir """,
                'mount': """mount a oss path to local file system in read/write mode, kv-pairs, multi-supported,
                                      format: <oss_path>:<dir_path>[,<oss_path2>:<dir_path2>...],
                                      example: oss://bucket/key:/home/admin/ossdir """,
                'docker': """optional, use docker, format: <docker_image_name>@<docker_storage_oss_path>,
                                      example: localhost:5000/myubuntu@oss://bucket/dockers/
                                      or for short: myubuntu@oss://bucket/dockers/ """,
                'file': """submit job from a config file, if you give options explicitly, options from this file will be override""",
                'show_json': 'just output job.json, don\'t submit a job, default: false'
            }
        },
        'check': {
        ...
        'oss': {
            'description': '(experimental) oss commanders: upload, download, ls, etc.',
            'pwd': {
