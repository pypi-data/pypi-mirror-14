...
def parse_cluster_cfg(s):
    ...
    if s.find('=')!=-1:
        ...
        if  not it.get_by_name(m['InstanceType']):
...
def trans_docker(docker=None):
    if docker.startswith('oss://'):
        [k,v]=docker[6:].split(':',1)
        k = 'oss://%s' % k

        if not v.startswith('localhost:5000/'):
            v = 'localhost:5000/%s' % v
        return {"BATCH_COMPUTE_DOCKER_IMAGE":v, "BATCH_COMPUTE_DOCKER_REGISTRY_OSS_PATH":k}
    else:
