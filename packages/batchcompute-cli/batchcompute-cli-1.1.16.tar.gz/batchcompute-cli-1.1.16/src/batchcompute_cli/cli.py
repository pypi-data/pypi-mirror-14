from terminal import red, blue, bold, magenta, gray
...
program = Command(COMMAND, version=VERSION,
                  ...
                  func=__main, help_footer=gray('  type "%s [command] -h" for more' % CMD))
...
cmd_log = Command('log',
  ...
  Examples:
      1. %s log <jobId>                                                 # show all instance logs for a job
      2. %s log <jobId> -e                                              # show stderr log only
      3. %s log <jobId> -d /path/to/save/logs/                          # download all instance logs for a job
      4. %s log <jobId> <taskName> -d /path/to/save/logs/               # fetch all instance logs for a task
      5. %s log <jobId> <taskName> <intanceId> -d /path/to/save/logs/   # fetch instance log
...
cmd_submit_job = Command('submit', alias=['sub'],
  ...
  Examples:
      ...
      3. %s sub "python main.py arg1 arg2" jobName1 -c cls-xxxx -p ./my_program/               # set job name, use cluster id, pack a folder and upload
      4. %s sub "python /home/admin/test/main.py" -m oss://your-bucket/a/b/:/home/admin/test/  # mount an oss path to a local path
      5. %s sub "python test.py" --docker oss://your-bucket/dockers:localhost:5000/myubuntu    # run in docker container''' % (
                         COMMAND, CMD, CMD, IMG_ID, CMD, CMD, CMD),
...
cmd_submit_job.option("-d, --description [description]",
                       MSG['submit']['option']['description'])
...
cmd_submit_job.option("-m, --mount [kv_pairs]",
                      ...
                      resolve=job_submit.trans_mount)
cmd_submit_job.option("--docker [docker]",
                      MSG['submit']['option']['docker'],
                      resolve=job_submit.trans_docker)
