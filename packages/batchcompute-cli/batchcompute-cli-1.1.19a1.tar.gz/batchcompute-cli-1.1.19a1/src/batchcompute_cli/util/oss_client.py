...
def list(oss_path):
    ...
    arr = _list(bucket_tool, key)

    return (arr, bucket_tool)

def _list(bucket_tool, key, marker=''):
    arr = []
    obj = bucket_tool.list_objects(key, marker)
    arr += obj.object_list

    if obj.next_marker:
        arr += _list(bucket_tool, key, obj.next_marker)
    return arr
