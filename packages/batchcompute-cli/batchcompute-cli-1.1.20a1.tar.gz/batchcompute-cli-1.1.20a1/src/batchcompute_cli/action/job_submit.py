...
def submit(cmd=None, job_name=None, description=None, cluster=None, timeout=None, nodes=None,
    ...
    #
    # # default value
    # read_mount =  opt.get('read_mount') or {}
    # write_mount =  opt.get('write_mount') or {}
    #
    #
    # if not cmd:
    ...
    #
    #
    # DEFAULT_CLUSTER = {'AutoCluster': {'ECSImageId':IMG_ID, 'InstanceType': INS_TYPE} }
    #
...
def fix_by_opt(desc, opt):
    ...
    if opt.get('force'):
        desc['JobFailOnInstanceFail'] = opt['force']
...
def trans_file_to_job_desc(file):
    ...
    if default_options:
        ...
        if default_options.get('force'):
            desc['JobFailOnInstanceFail'] = cf.getboolean(DEFAULT,'force')
    ...
    for sec in secs:
        ...
        if cf.has_option(sec,'force'):
            desc['JobFailOnInstanceFail'] = cf.getboolean(sec,'force')
        #################################
