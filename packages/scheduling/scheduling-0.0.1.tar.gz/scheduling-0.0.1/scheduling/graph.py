...
def dfs(node, expand=lambda n: n.all, callback=None, silent=True):
...
def explore(node):
    ...
    explored = set()
    explored.add(node)
    dfs(node, callback=lambda x: explored.add(x))
    return explored
...
def detect_cycles(nodes):
    for node in nodes:
        if not node.pruned:
            explored = []
            dfs(node,
                callback=lambda x: explored.append(x) and x.prune(),
                expand=lambda x: x.children,
                silent=False)
