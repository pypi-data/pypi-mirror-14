...
This section describes the Python API for Choco templates. If you
are using Choco within a web framework such as Pylons, the work
of integrating Choco's API is already done for you, in which case
...
template. When ``mytemplate.render()`` is called, Choco sets up a
...
The :meth:`~.Template.render` method calls upon Choco to create a
...
.. sourcecode:: python
    ...
    from choco.template import Template
    from choco.runtime import Context
...
.. sourcecode:: python
    ...
    mytemplate = Template(filename='/docs/mytmpl.txt', module_directory='/tmp/choco_modules')
    print(mytemplate.render())

When the above code is rendered, a file
``/tmp/choco_modules/docs/mytmpl.txt.py`` is created containing the
...
.. sourcecode:: python
    ...
    mylookup = TemplateLookup(directories=['/docs'], module_directory='/tmp/choco_modules')
...
generated module files in the ``/tmp/choco_modules`` directory. The
...
exception, which is a custom Choco exception.
...
module file ``/tmp/choco_modules/etc/beans/info.txt.py``.
...
.. sourcecode:: python
    ...
    mylookup = TemplateLookup(directories=['/docs'],
                    module_directory='/tmp/choco_modules', collection_size=500)
...
Note that Choco's ability to return data in any encoding and/or
...
Template errors can occur in two distinct places. One is
when you **lookup, parse and compile** the template, the other
is when you **run** the template. Within the running of a
template, errors are thrown normally from whatever Python
code originated the issue. Choco has its own set of exception
classes which mostly apply to the lookup and lexer/compiler
stages of template construction. Choco provides some library
routines that can be used to help provide Choco-specific
...
Python filenames, line numbers, and code samples into Choco
template filenames, line numbers, and code samples. All lines
within a stack trace which correspond to a Choco template module
...
.. sourcecode:: python
    ...
    try:
        ...
        print(errors.text_error_template().render())
...
.. sourcecode:: python
    ...
    try:
        ...
        print(errors.html_error_template().render())
...
.. sourcecode:: python
    ...
    print(errors.html_error_template().render(full=False))

The HTML render function is also available built-in to
:class:`.Template` using the ``format_errors`` flag. In this case, any
errors raised within the **render** stage of the template
will result in the output being substituted with the output of
:func:`.html_error_template`:

.. sourcecode:: python

    template = Template(filename="/foo/bar", format_errors=True)
    print(template.render())
...
defined. Therefore errors which occur within the
lookup/parse/compile stage will not be handled and will
propagate normally. While the pre-render traceback usually will
not include any Choco-specific lines anyway, it will mean that
errors which occur previous to rendering and those which
...
The Choco distribution includes a little bit of helper code for
the purpose of using Choco in some popular web framework
...
of files, and also displays errors using Choco's included
...
highlighting module is included under :mod:`choco.ext.pygmentplugin`.
This module is used in the generation of Choco documentation and
also contains various `setuptools` entry points under the heading
``pygments.lexers``, including ``choco``, ``html+choco``, ``xml+choco``
(see the ``setup.py`` file for all the entry points).

Babel
-----

Choco provides support for extracting `gettext` messages from
templates via a `Babel`_ extractor
entry point under ``choco.ext.babelplugin``.
...
may also be extracted from Choco templates when a comment tag is
specified to `Babel`_ (such as with
the ``-c`` option).

For example, a project ``"myproj"`` contains the following Choco
...
a Choco section in its `Babel Extraction Method Mapping
...
.. sourcecode:: cfg
    ...
    # Extraction from Choco templates

    [choco: myproj/templates/**.html]
    input_encoding = utf-8

The Choco extractor supports an optional ``input_encoding``
...
.. autoclass:: choco.template.Template
    :show-inheritance:
    :members:

.. autoclass:: choco.template.DefTemplate
    :show-inheritance:
    :members:

.. autoclass:: choco.lookup.TemplateCollection
    :show-inheritance:
    :members:

.. autoclass:: choco.lookup.TemplateLookup
    :show-inheritance:
    :members:

.. autoclass:: choco.errors.RichTraceback
    :show-inheritance:
...
.. autofunction:: choco.errors.html_error_template

.. autofunction:: choco.errors.text_error_template
