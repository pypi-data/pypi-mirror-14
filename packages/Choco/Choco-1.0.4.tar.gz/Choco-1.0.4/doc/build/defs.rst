...
which is a central Choco concept that has its own chapter in
...
Only with Choco, you don't have to use any external Python
...
syntax. This syntax, introduced in Choco 0.2.3, is functionally
equivalent to another tag known as ``%call``, which takes the form
``<%call expr='namespacename.defname(args)'>``. While ``%call``
is available in all versions of Choco, the newer style is
...
with a "custom tag" or tag library in some other system, Choco
...
``<%include file="post.choco" args="post=post" />``, the ``post``
