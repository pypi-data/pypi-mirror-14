...
# create fake nodes classes which are constructed
# exactly as the repr() of a real nodes object.
...
for cls in list(nodes.__dict__.values()):
    if isinstance(cls, type) and \
        issubclass(cls, nodes.Node):
...
"""
    ...
    def test_onlyclosed_tag(self):
        template = \
            ...
            hi.
        """
        self.assertRaises(errors.SyntaxException,
    ...
    def test_noexpr_allowed(self):
        template = \
            ...
            <%namespace name="${foo}"/>
        """
        self.assertRaises(errors.CompileException,
                          Lexer(template).parse)
