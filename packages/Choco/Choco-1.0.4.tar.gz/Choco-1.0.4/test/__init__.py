...
import unittest
import os
from choco.compat import py3k, py26, py33
from choco import compat
from choco.util import update_wrapper
import re
...
ui_template_base = os.path.join(os.path.dirname(__file__), 'templates', 'ui')
...
def requires_no_pygments_errors(fn):
    def go(*arg, **kw):
        from choco import errors
        errors._install_fallback()
        try:
            return fn(*arg, **kw)
        finally:
            errors._install_highlighting()
