...
.. note:: In current versions of Choco, usage of ``import='*'`` is
...
Choco 0.2.3, allows the usage of a "custom" Choco tag, with the
...
functions which recognize body content, which in Choco is known
...
the :func:`.supports_caller` decorator, which will ensure that Choco
will ensure the correct "caller" variable is available when your
def is called, supporting embedded content:

.. sourcecode:: python

    from choco.runtime import supports_caller
...
.. sourcecode:: python
    ...
    from choco.runtime import supports_caller, capture
...
to get at the keyword arguments, Choco recommends you define your
...
Choco. When the template is called as a top-level template (i.e.
...
The namespace is so great that Choco gives your template one (or
...
.. sourcecode:: choco
    ...
    <%namespace name="foo" file="library.choco"/>
...
Above, the file ``library.choco`` declares an attribute ``includes`` inside its global ``<%! %>`` section.
``index.choco`` includes this template using the ``<%namespace>`` tag.  The base template ``base.choco``, which is the inherited parent of ``index.choco`` and is reponsible for layout, then locates this attribute and iterates through its contents to produce the includes that are specific to ``library.choco``.
...
In this version, ``library.choco`` declares a ``<%def>`` named ``includes``.   The example works
identically to the previous one, except that ``base.choco`` looks for defs named ``include``
on each namespace it examines.

API Reference
=============

.. autoclass:: choco.runtime.Namespace
    :show-inheritance:
    :members:

.. autoclass:: choco.runtime.TemplateNamespace
    :show-inheritance:
    :members:

.. autoclass:: choco.runtime.ModuleNamespace
    :show-inheritance:
    :members:

.. autofunction:: choco.runtime.supports_caller

.. autofunction:: choco.runtime.capture
