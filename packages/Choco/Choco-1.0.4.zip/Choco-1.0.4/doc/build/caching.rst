...
any other third-party caching libraries that feature Choco integration.
...
Choco has two cache arguments available on tags that are
...
* ``cache_type`` - type of caching. ``'memory'``, ``'file'``, ``'dbm'``, or
  ...
  also accepted by the dogpile.cache Choco plugin, though not by Beaker itself).
...
includes its own Choco cache plugin -- see :mod:`dogpile.cache.plugins.choco_cache` in the
...
the rudimental methods Choco needs to implement the caching
API.   Choco includes the :class:`.BeakerCacheImpl` class to
provide the default implementation.  A :class:`.CacheImpl` class
is acquired by Choco using a ``pkg_resources`` entrypoint, using
...
the `EntryPoint` group named ``"choco.cache"``.  It can also be
...
.. sourcecode:: python
    ...
    from choco.cache import Cacheimpl, register_plugin
...
.. autoclass:: choco.cache.Cache
    :members:
    :show-inheritance:

.. autoclass:: choco.cache.CacheImpl
    :members:
    :show-inheritance:

.. autofunction:: choco.cache.register_plugin

.. autoclass:: choco.ext.beaker_cache.BeakerCacheImpl
    :members:
    :show-inheritance:
