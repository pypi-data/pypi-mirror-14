...
some built-in variables provided by Choco's runtime environment.
...
* **What happens if I reference a variable name that is not in
  ...
  variable with the class :class:`choco.runtime.Undefined`. The
...
* **Why raise an exception when you call str() on it ? Why not
  just return a blank string?** - Choco tries to stick to the
...
work as well as you'd think. The reason for this is that Choco in
...
of where Choco creates copies of the :class:`.Context` include
...
as inlined functions, Choco tries to make them act that way), and
...
* **Why can't "attributes" be a built-in feature of the
  Context?** - This is an area where Choco is trying to make as
  ...
  be passed around. Once again Choco would rather ask the user to
  be explicit.
...
a ``__len__`` method.  Prior to Choco 0.7, you might have generated a simple
...
Loop contexts can also be transparently nested, and the Choco runtime will do
...
.. versionchanged:: 0.7
   The ``loop`` name is now :ref:`reserved <reserved_names>` in Choco,
   which means a template that refers to a variable named ``loop``
   won't function correctly when used in Choco 0.7.
...
A one-stop shop for all the names Choco defines. Most of these
...
Choco has a few names that are considered to be "reserved" and can't be used
as variable names.

.. versionchanged:: 0.7
   Choco raises an error if these words are found passed to the template
...
.. autoclass:: choco.runtime.Context
    :show-inheritance:
    :members:

.. autoclass:: choco.runtime.LoopContext
    :show-inheritance:
    :members:

.. autoclass:: choco.runtime.Undefined
    :show-inheritance:
