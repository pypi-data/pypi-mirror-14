...
.. note::  Most of the inheritance examples here take advantage of a feature that's
    new in Choco as of version 0.4.1 called the "block".  This tag is very similar to
...
the way ``self`` works in a Python class, even though Choco is
not actually using Python class inheritance to implement this
functionality. (Choco doesn't take the "inheritance" metaphor too
...
to be overridden.  Before Choco 0.4.1, there wasn't any such tag -- instead
...
as the calling template, but currently Choco has no static include facility.
...
.. sourcecode:: choco
    ...
    ## parent.choco
    <%include file="partials.choco">

    ## child.choco
    <%inherit file="parent.choco">
    <%block name="header">
        Custom Header
    </%block>

Above, one might expect that the ``"header"`` block declared in ``child.choco``
might be invoked, as a result of it overriding the same block present in
``parent.choco`` via the include for ``partials.choco``.  But this is not the case.
Instead, ``parent.choco`` will invoke ``partials.choco``, which then invokes
``"header"`` in ``partials.choco``, and then is finished rendering.  Nothing
from ``child.choco`` will render; there is no interaction between the ``"header"``
block in ``child.choco`` and the ``"header"`` block in ``partials.choco``.

Instead, ``parent.choco`` must explicitly state the inheritance structure.
In order to call upon specific elements of ``partials.choco``, we will call upon
it as a namespace:
...
.. sourcecode:: choco
    ...
    ## parent.choco
    <%namespace name="partials" file="partials.choco"/>
    <%block name="header">
        ${partials.header()}
    </%block>
...
Where above, ``parent.choco`` states the inheritance structure that ``child.choco``
is to participate within.  ``partials.choco`` only defines defs/blocks that can be
used on a per-name basis.

Another scenario is below, which results in both ``"SectionA"`` blocks being rendered for the ``child.choco`` document:
...
.. sourcecode:: choco
    ...
    ## parent.choco
    <%inherit file="base.choco">
    <%include file="child.choco">
...
of ``child.choco`` using a namespace:

.. sourcecode:: choco

    ## parent.choco
    <%inherit file="base.choco">
    <%namespace name="child" file="child.choco">
