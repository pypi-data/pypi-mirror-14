...
One of Choco's central design goals is speed. To this end, all of
...
Choco provides two ways to work around this. One is by applying
...
The other way to buffer the output of a def or any Choco callable
...
callable in the ``choco.runtime`` module (available in templates
as just ``runtime``):
