...
cdef tuple alignSequence(bytes seq1, bytes seq2,
...
#    nseq   = len(readids)
    ...
    progressBar(1,nali,reset=True,head=bytes("Pairwise alignment %6d x %6d " % (0,0),encoding='ascii'))
    
    for h1 in range(nseq):
        for h2 in range(h1+1,nseq):
            
            progressBar(pos,nali,reset=False,head=bytes("Pairwise alignment %6d x %6d " % (h1,h2),encoding='ascii'))
            pos+=1
