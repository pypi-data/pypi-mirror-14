...
def extend_sloth(cls, extension):
    ...
    class Sloth(cls):
        def __init__(self, config):
            ...
            self._subjects = {
                'triggered': build_email_config.get(
                    'subject_triggered', '{listen_point}: Build Triggered'),
                'completed': build_email_config.get(
                    'subject_completed', '{listen_point}: Build Completed'),
                'partially_completed': build_email_config.get(
                    'subject_partially_completed',
                    '{listen_point}: Build Partially Completed'),
                'failed': build_email_config.get(
                    'subject_failed', '{listen_point}: Build Failed')
            }
        ...
        def _get_email_subject(self, record):
            ...
            if record.levelname == 'INFO':
                if 'Triggered' in record.getMessage():
                    return self._subjects['triggered'].format(
                        listen_point=self.listen_point)

                else:
                    return self._subjects['completed'].format(
                        listen_point=self.listen_point)

            elif record.levelname == 'WARNING':
                return self._subjects['partially_completed'].format(
                    listen_point=self.listen_point)

            elif record.levelname == 'ERROR':
                return self._subjects['failed'].format(
                    listen_point=self.listen_point)
