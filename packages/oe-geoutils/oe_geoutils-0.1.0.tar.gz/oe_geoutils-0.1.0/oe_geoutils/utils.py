...
def check_in_flanders(geojson_input):
    ...
    flanders_geometry = geometry.shape(flanders_geojson)
    if geometry.shape(geojson_input).intersects(flanders_geometry):
        return True
    return False
...
class AdminGrenzenClient:
    ...
    def _get_gemeenten_objecten(self, geojson_input, admin_grens_type, geef_geometrie=0):
        if check_in_flanders(geojson_input):
            params = {
                'geef_geometrie': geef_geometrie,
    ...
    def get_gemeenten(self, geojson_input):
        ...
        """
        results = self._get_gemeenten_objecten(geojson_input, 'gemeente')
    ...
    def get_gemeente(self, geojson_input):
        ...
        :return: list of municipalities
        """
        results = self._get_gemeenten_objecten(geojson_input, 'gemeente', 1)
        if len(results) == 0:
            return None
        elif len(results) == 1:
            ...
            for result in results:
                municipality = convert_geojson_to_geometry(result['geometrie'])
                result['intersection_area'] = municipality.intersection(input_polygon).area
