...
class APIDocumentDownloadPDFView(generics.RetrieveAPIView):
    """
    Download the latest version of a document.
    ---
    GET:
    ...
    def retrieve(self, request, *args, **kwargs):
        instance = self.get_object()
        doc_ver = instance.latest_version
        if doc_ver.mimetype == 'application/pdf':
            pdf_file = instance.latest_version.file
            save_as = instance.label
            content_type = instance.latest_version.mimetype if instance.latest_version.mimetype else 'application/octet-stream'
        else:
            pdf_file = doc_ver.get_intermidiate_file()
            pre, ext = os.path.splitext(instance.label)
            save_as = '{0}.pdf'.format(pre)
