...
class AvroConverter(Converter):
  ...
  def _map_properties_to_fields(self, props, name_cnt={}, **kw):
    """
    ...
    for name, prop in props.items():
      ...
      if types[0] == 'object':
        ...
        if name not in name_cnt:
          ...
          name_cnt[name] += 1
        print name_cnt[name], name
        if name_cnt[name] > 1:
          print "Deduping", name
          name = "{0}_{1}".format(name, name_cnt[name]-1)

        rec = self._init_obj_record(name)
        rec['type']['fields'] = self._map_properties_to_fields(prop['properties'], name_cnt)
