...
class RegularizedLogisticRegression(object) :
    def __init__(self, alpha=0.01, cv=10, num_cores=0) :
        self.alpha = alpha
        self.cv = cv
    ...
    def fit(self, examples, labels, case_weights=None) :
        if self.cv and np.sum(labels) > 50 :
    ...
    def fit_alpha(self, examples, labels, case_weights) :
        ...
        if self.weights is None :
            ...
            start_betas = np.append(self.weights, self.bias)
    ...
    def predict_proba(self, examples) :
        scores = np.dot(examples, self.weights) + self.bias
