...
def kFolds(labeled_examples, k):
    ...
    ninetieth = int(len(labels) * 0.9)
    
    for i in range(5):
        permutation = numpy.random.permutation(len(labels))
        training_indices = permutation[:ninetieth]
        validation_indices = permutation[ninetieth:]
        
        training = (examples[training_indices, :], 
                    labels[training_indices])
        validation = (examples[validation_indices, :], 
...
def scorePredictions(true_labels, predictions) :
    ...
    dupe_predictions = predictions[true_labels == 1]
    distinct_predictions = predictions[true_labels == 0]

    if (not len(dupe_predictions)
        or not len(distinct_predictions)
        or numpy.isnan(dupe_predictions).any()
        or numpy.isnan(distinct_predictions).any()) :
        return None

    true_dupes = numpy.sum(dupe_predictions > 0.5)
    false_dupes = numpy.sum(distinct_predictions > 0.5)

    true_distinct = numpy.sum(distinct_predictions <= 0.5)
    false_distinct = numpy.sum(dupe_predictions <= 0.5)
