...
class openYandex():
    ...
    def get_yandex_cache_pos(self, phrase, lr=213, normalize=True, storage=True):
        ...
        for i in YANDEX_RANGE:
            ...
            if data:
                ...
                urls.append(getSoup.s(url, self.antigate, normalize))
        if urls:
            job = group(urls)
    ...
    def load_core(self, phrase, lr=213, google=True, yandex=True):
        phrase_quote = get_normal_quote(phrase)
        if yandex:
            urls = []
            for i in YANDEX_RANGE:
