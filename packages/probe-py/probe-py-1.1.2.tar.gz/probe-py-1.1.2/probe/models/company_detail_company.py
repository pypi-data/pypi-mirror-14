# coding: utf-8

"""
Copyright 2015 SmartBear Software

    Licensed under the Apache License, Version 2.0 (the "License");
    you may not use this file except in compliance with the License.
    You may obtain a copy of the License at

        http://www.apache.org/licenses/LICENSE-2.0

    Unless required by applicable law or agreed to in writing, software
    distributed under the License is distributed on an "AS IS" BASIS,
    WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
    See the License for the specific language governing permissions and
    limitations under the License.

    Ref: https://github.com/swagger-api/swagger-codegen
"""

from pprint import pformat
from six import iteritems


class CompanyDetailCompany(object):
    """
    NOTE: This class is auto generated by the swagger code generator program.
    Do not edit the class manually.
    """
    def __init__(self):
        """
        CompanyDetailCompany - a model defined in Swagger

        :param dict swaggerTypes: The key is attribute name
                                  and the value is attribute type.
        :param dict attributeMap: The key is attribute name
                                  and the value is json key in definition.
        """
        self.swagger_types = {
            'legal_name': 'str',
            'status': 'str',
            'incorporation_date': 'str',
            'next_cin': 'str',
            'classification': 'str',
            'paid_up_capital': 'int',
            'authorized_capital': 'float',
            'registered_address': 'Address',
            'efiling_status': 'str',
            'cin': 'str'
        }

        self.attribute_map = {
            'legal_name': 'legal_name',
            'status': 'status',
            'incorporation_date': 'incorporation_date',
            'next_cin': 'next_cin',
            'classification': 'classification',
            'paid_up_capital': 'paid_up_capital',
            'authorized_capital': 'authorized_capital',
            'registered_address': 'registered_address',
            'efiling_status': 'efiling_status',
            'cin': 'cin'
        }

        self._legal_name = None
        self._status = None
        self._incorporation_date = None
        self._next_cin = None
        self._classification = None
        self._paid_up_capital = None
        self._authorized_capital = None
        self._registered_address = None
        self._efiling_status = None
        self._cin = None

    @property
    def legal_name(self):
        """
        Gets the legal_name of this CompanyDetailCompany.
        Legal name of the company

        :return: The legal_name of this CompanyDetailCompany.
        :rtype: str
        """
        return self._legal_name

    @legal_name.setter
    def legal_name(self, legal_name):
        """
        Sets the legal_name of this CompanyDetailCompany.
        Legal name of the company

        :param legal_name: The legal_name of this CompanyDetailCompany.
        :type: str
        """
        self._legal_name = legal_name

    @property
    def status(self):
        """
        Gets the status of this CompanyDetailCompany.
        Listing status

        :return: The status of this CompanyDetailCompany.
        :rtype: str
        """
        return self._status

    @status.setter
    def status(self, status):
        """
        Sets the status of this CompanyDetailCompany.
        Listing status

        :param status: The status of this CompanyDetailCompany.
        :type: str
        """
        allowed_values = ["Listed", "Unlisted", "Undefined"]
        if status not in allowed_values:
            raise ValueError(
                "Invalid value for `status`, must be one of {0}"
                .format(allowed_values)
            )
        self._status = status

    @property
    def incorporation_date(self):
        """
        Gets the incorporation_date of this CompanyDetailCompany.
        Date of incorporation of the company.

        :return: The incorporation_date of this CompanyDetailCompany.
        :rtype: str
        """
        return self._incorporation_date

    @incorporation_date.setter
    def incorporation_date(self, incorporation_date):
        """
        Sets the incorporation_date of this CompanyDetailCompany.
        Date of incorporation of the company.

        :param incorporation_date: The incorporation_date of this CompanyDetailCompany.
        :type: str
        """
        self._incorporation_date = incorporation_date

    @property
    def next_cin(self):
        """
        Gets the next_cin of this CompanyDetailCompany.
        Changed CIN

        :return: The next_cin of this CompanyDetailCompany.
        :rtype: str
        """
        return self._next_cin

    @next_cin.setter
    def next_cin(self, next_cin):
        """
        Sets the next_cin of this CompanyDetailCompany.
        Changed CIN

        :param next_cin: The next_cin of this CompanyDetailCompany.
        :type: str
        """
        self._next_cin = next_cin

    @property
    def classification(self):
        """
        Gets the classification of this CompanyDetailCompany.
        Classification of Company

        :return: The classification of this CompanyDetailCompany.
        :rtype: str
        """
        return self._classification

    @classification.setter
    def classification(self, classification):
        """
        Sets the classification of this CompanyDetailCompany.
        Classification of Company

        :param classification: The classification of this CompanyDetailCompany.
        :type: str
        """
        allowed_values = ["Public Limited Indian Non-Government Company", "Private Limited Indian Non-Government Company", "One Person Company", "Private Limited Foreign Company Incorporated in India", "Public Limited Foreign Company Incorporated in India", "Union Government Company", "State Government Company", "Guarantee & Association Public", "Guarantee & Association Private", "Not For Profit Company", "Unlimited Liabilities Public", "Unlimited Liabilities Private", "Undefined"]
        if classification not in allowed_values:
            raise ValueError(
                "Invalid value for `classification`, must be one of {0}"
                .format(allowed_values)
            )
        self._classification = classification

    @property
    def paid_up_capital(self):
        """
        Gets the paid_up_capital of this CompanyDetailCompany.
        Paid up capital

        :return: The paid_up_capital of this CompanyDetailCompany.
        :rtype: int
        """
        return self._paid_up_capital

    @paid_up_capital.setter
    def paid_up_capital(self, paid_up_capital):
        """
        Sets the paid_up_capital of this CompanyDetailCompany.
        Paid up capital

        :param paid_up_capital: The paid_up_capital of this CompanyDetailCompany.
        :type: int
        """
        self._paid_up_capital = paid_up_capital

    @property
    def authorized_capital(self):
        """
        Gets the authorized_capital of this CompanyDetailCompany.
        Authorized Capital for the company.

        :return: The authorized_capital of this CompanyDetailCompany.
        :rtype: float
        """
        return self._authorized_capital

    @authorized_capital.setter
    def authorized_capital(self, authorized_capital):
        """
        Sets the authorized_capital of this CompanyDetailCompany.
        Authorized Capital for the company.

        :param authorized_capital: The authorized_capital of this CompanyDetailCompany.
        :type: float
        """
        self._authorized_capital = authorized_capital

    @property
    def registered_address(self):
        """
        Gets the registered_address of this CompanyDetailCompany.


        :return: The registered_address of this CompanyDetailCompany.
        :rtype: Address
        """
        return self._registered_address

    @registered_address.setter
    def registered_address(self, registered_address):
        """
        Sets the registered_address of this CompanyDetailCompany.


        :param registered_address: The registered_address of this CompanyDetailCompany.
        :type: Address
        """
        self._registered_address = registered_address

    @property
    def efiling_status(self):
        """
        Gets the efiling_status of this CompanyDetailCompany.
        Status of EFiling.

        :return: The efiling_status of this CompanyDetailCompany.
        :rtype: str
        """
        return self._efiling_status

    @efiling_status.setter
    def efiling_status(self, efiling_status):
        """
        Sets the efiling_status of this CompanyDetailCompany.
        Status of EFiling.

        :param efiling_status: The efiling_status of this CompanyDetailCompany.
        :type: str
        """
        self._efiling_status = efiling_status

    @property
    def cin(self):
        """
        Gets the cin of this CompanyDetailCompany.
        Unique identifier representing the Company - Company Identification number.

        :return: The cin of this CompanyDetailCompany.
        :rtype: str
        """
        return self._cin

    @cin.setter
    def cin(self, cin):
        """
        Sets the cin of this CompanyDetailCompany.
        Unique identifier representing the Company - Company Identification number.

        :param cin: The cin of this CompanyDetailCompany.
        :type: str
        """
        self._cin = cin

    def to_dict(self):
        """
        Returns the model properties as a dict
        """
        result = {}

        for attr, _ in iteritems(self.swagger_types):
            value = getattr(self, attr)
            if isinstance(value, list):
                result[attr] = list(map(
                    lambda x: x.to_dict() if hasattr(x, "to_dict") else x,
                    value
                ))
            elif hasattr(value, "to_dict"):
                result[attr] = value.to_dict()
            else:
                result[attr] = value

        return result

    def to_str(self):
        """
        Returns the string representation of the model
        """
        return pformat(self.to_dict())

    def __repr__(self):
        """
        For `print` and `pprint`
        """
        return self.to_str()

    def __eq__(self, other): 
        """
        Returns true if both objects are equal
        """
        return self.__dict__ == other.__dict__

    def __ne__(self, other):
        """ 
        Returns true if both objects are not equal
        """
        return not self == other

