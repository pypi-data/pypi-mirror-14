...
class Configs(Jsonizable):
    ...
    def add_system_disk(self, size, type_='cloud'):
        assert isinstance(size, NUMBER) and isinstance(type, STRING), \
            'size must be number and type must be string'
        disk = {
            'Type': type,
            'Size': size,
        } 
    ...
    def add_data_disk(self, size, type_='cloud', mount_point=''):
        assert isinstance(size, NUMBER) and isinstance(type, STRING) and \
            isinstance(mount_point, STRING), 'size must be number and type must be string'
        disk = {
            'Type': type,
            'Size': size,
